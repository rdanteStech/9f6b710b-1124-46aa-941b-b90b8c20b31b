...
export default function GastroLayout() {
  ...
  //   • Step: 1px (Shift → 10px)
  //   • Zone boundary IS enforced (wallDistPx = 0 — just the polygon edge, no extra margin)
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      setZones(prev => {
        ...
        // ── Zone boundary enforcement (no wall-distance margin, just the polygon edge) ──
        if (zone.spacePath.points.length > 2) {
          ...
          // No zone drawn — only raw canvas bounds
          nx = Math.max(0, nx)
          ny = Math.max(0, ny)
        }
      ...
      <div className="flex gap-4 flex-1 min-h-0">
        ...
        <div className="flex flex-col gap-2 flex-shrink-0 w-44 overflow-y-auto">
          ...
          <div className="card-gastro p-3">
            ...
            <div className="mt-3 pt-2 border-t border-gastro-border space-y-1">
              ...
              <p className="text-xs text-gastro-subtle leading-relaxed">⌨️ <strong className="text-gastro-text">Flechas:</strong> Ajuste fino 1px respetando el límite de zona. Shift = 10px.</p>
        ...
        <div className="flex-1 rounded-2xl overflow-hidden relative" style={{ background: '#0d0d18', border: `1px solid ${activeZone.color}22` }}>
          ...
          <div className="absolute top-3 left-1/2 -translate-x-1/2 z-10 pointer-events-none flex flex-col items-center gap-2">
            ...
            {arrowMoving && selectedId && (
              <div className="px-4 py-2 rounded-xl text-xs font-semibold"
                ...
                ⌨️ Ajuste fino con flechas · respeta límite de zona · Shift = 10px
              </div>
            )}
            {pendingFuse && (
              <div className="px-4 py-2 rounded-xl text-xs font-semibold"
          ...
          >
            ...
            {/* clipPath always active — arrow keys now respect zone boundary */}
