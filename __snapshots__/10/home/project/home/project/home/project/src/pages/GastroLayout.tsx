 ...
 * Returns the point coordinates, parameter t, and distance.
 * Uses dense sampling (steps=60) for accuracy.
 */
 ...
 * Returns the split point P on the curve, and the two new control points
 * for the left sub-curve (A→cp1→P) and right sub-curve (P→cp2→B).
 */
 ...
 * All wall physics (clamp, snap, warning) use this so curved walls
 * are the actual physical boundary.
 */
...
export default function GastroLayout() {
    ...
    if ((tool === 'addPoint' || tool === 'curveEdge') && activeSpacePath.points.length > 1) {
      ...
      for (let i = 0; i < activeSpacePath.points.length; i++) {
        ...
        if (seg.controlPoint && tool === 'addPoint') {
          // For curved edges in addPoint mode: find closest point on the VISUAL curve
  ...
  const handleCanvasClick = useCallback((e: React.MouseEvent) => {
    if (tool === 'draw') {
      ...
      // Find the closest edge — for curved edges use bezier distance, for straight use segment distance
      ...
      for (let i = 0; i < activeSpacePath.points.length; i++) {
        ...
        if (seg.controlPoint) {
          // Measure distance to the VISUAL bezier curve
          const result = closestPointOnQuadBezier(pt.x, pt.y, a.x, a.y, seg.controlPoint.x, seg.controlPoint.y, b.x, b.y)
      ...
      setZones(prev => prev.map(z => {
        ...
        if (seg.controlPoint) {
          // ── Curved edge: split the bezier into two sub-curves (de Casteljau) ──
          // The new point lands exactly on the visual curve.
          // The original straight-line segment ceases to exist — only the two
          // new sub-curves remain for all physics and visual effects.
          const split = splitQuadBezier(
            a.x, a.y,
            seg.controlPoint.x, seg.controlPoint.y,
            b.x, b.y,
            bestT
          )
          newPoints.splice(bestIndex + 1, 0, split.p)
          newSegments.splice(bestIndex, 1,
            { controlPoint: split.cp1 },  // left sub-curve: A → cp1 → P
            { controlPoint: split.cp2 }   // right sub-curve: P → cp2 → B
          )
        } else {
          // ── Straight edge: insert point at linear interpolation ──
