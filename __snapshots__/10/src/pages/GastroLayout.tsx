...
export default function GastroLayout() {
  ...
  const minWallDistRef = useRef(minWallDist)
  ...
  useEffect(() => { minWallDistRef.current = minWallDist }, [minWallDist])

  // ── Arrow-key movement ────────────────────────────────────────────────────
  // Rules:
  //   • Only fires when a table is selected and no input/textarea is focused
  //   • Step: 1px (Shift → 10px)
  //   • Zone boundary IS enforced WITH wall-distance margin (wallDistPx)
  //   • Table-to-table minimum distance IS enforced
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      setZones(prev => {
        ...
        // ── Zone boundary + wall-distance margin enforcement ───────────────
        // Uses the same wallDistPx as mouse drag so the table can't get
        // closer to the wall than the configured minimum distance.
        if (zone.spacePath.points.length > 2) {
          const approxPoly = approximateSpacePath(zone.spacePath)
          const wallPx = minWallDistRef.current * METER_PX
          const clamped = clampInsidePolygon(nx, ny, moving.width, moving.height, approxPoly, wallPx)
          nx = clamped.x
          ny = clamped.y
        } else {
          nx = Math.max(0, nx)
        ...
        // ── Table-to-table minimum distance ────────────────────────────────
      ...
      <div className="flex gap-4 flex-1 min-h-0">
        ...
        <div className="flex flex-col gap-2 flex-shrink-0 w-44 overflow-y-auto">
          ...
          <div className="card-gastro p-3">
            ...
            <div className="mt-3 pt-2 border-t border-gastro-border space-y-1">
              ...
              <p className="text-xs text-gastro-subtle leading-relaxed">⌨️ <strong className="text-gastro-text">Flechas:</strong> Ajuste fino 1px respetando zona y distancia a pared. Shift = 10px.</p>
        ...
        <div className="flex-1 rounded-2xl overflow-hidden relative" style={{ background: '#0d0d18', border: `1px solid ${activeZone.color}22` }}>
          ...
          <div className="absolute top-3 left-1/2 -translate-x-1/2 z-10 pointer-events-none flex flex-col items-center gap-2">
            ...
            {arrowMoving && selectedId && (
              <div className="px-4 py-2 rounded-xl text-xs font-semibold"
                ...
                ⌨️ Ajuste fino · respeta zona y distancia a pared · Shift = 10px
              </div>
            )}
            {pendingFuse && (
              <div className="px-4 py-2 rounded-xl text-xs font-semibold"
          ...
          >
            ...
            {/* clipPath always active — arrow keys respect zone boundary + wall distance */}
