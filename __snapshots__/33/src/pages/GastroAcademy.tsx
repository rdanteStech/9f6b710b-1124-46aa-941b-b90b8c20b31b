...
const LEARNING_PATHS: LearningPath[] = [
  ...
  { id: 'lp2', name: 'Maître & Sommelier', description: 'Excelencia en sala y vinos', color: '#9E7FFF', icon: Wine, courses: 4, duration: '22h', level: 'intermediate', enrolled: 198 },
  { id: 'lp3', name: 'Gerente de Restaurante', description: 'Gestión integral del negocio', color: '#38bdf8', icon: BarChart3, courses: 5, duration: '18h', level: 'intermediate', enrolled: 287 },
  { id: 'lp4', name: 'Pastelero Profesional', description: 'Arte y técnica en pastelería', color: '#f472b6', icon: Sparkles, courses: 5, duration: '32h', level: 'advanced', enrolled: 156 },
]
...
const CAT_CFG: Record<CourseCategory, { label: string; color: string; emoji: string }> = {
  ...
  servicio:   { label: 'Servicio',        color: '#38bdf8', emoji: '🍽️' },
  gestion:    { label: 'Gestión',         color: '#9E7FFF', emoji: '📊' },
  bar:        { label: 'Bar & Bebidas',   color: '#f472b6', emoji: '🍸' },
  pasteleria: { label: 'Pastelería',      color: '#f472b6', emoji: '🧁' },
  higiene:    { label: 'Higiene & HACCP', color: '#10b981', emoji: '🛡️' },
}

const STATUS_CFG: Record<CourseStatus, { label: string; color: string; bg: string }> = {
  available: { label: 'Disponible',  color: '#8888aa', bg: 'rgba(136,136,170,0.1)' },
  enrolled:  { label: 'En curso',    color: '#9E7FFF', bg: 'rgba(158,127,255,0.12)' },
...
function CourseCard({ course, onEnroll }: { course: Course; onEnroll: (id: string) => void }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden transition-all duration-200 group"
      style={{
        background: '#1a1a26',
        border: `1px solid ${course.status === 'enrolled' ? 'rgba(158,127,255,0.3)' : course.status === 'completed' ? 'rgba(16,185,129,0.25)' : '#2a2a3d'}`,
      ...
      <div className="relative overflow-hidden" style={{ aspectRatio: '16/9' }}>
        ...
        <div className="absolute top-3 left-3 flex gap-1.5 flex-wrap">
          ...
          {course.isPro && (
            <span className="text-xs font-bold px-2 py-1 rounded-lg flex items-center gap-1"
              style={{ background: 'rgba(158,127,255,0.85)', color: '#fff' }}>
              <Zap size={9} /> PRO
            </span>
          )}
        </div>
        ...
        {isLocked && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-12 h-12 rounded-2xl flex items-center justify-center"
              ...
              <Lock size={20} style={{ color: '#9E7FFF' }} />
            </div>
          </div>
        )}
        ...
        {!isLocked && course.status !== 'available' && (
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
            <div className="w-12 h-12 rounded-full flex items-center justify-center"
              style={{ background: 'rgba(158,127,255,0.9)', backdropFilter: 'blur(4px)' }}>
        ...
        {course.status === 'enrolled' && course.progress !== undefined && (
          <div className="absolute bottom-0 left-0 right-0 h-1" style={{ background: 'rgba(255,255,255,0.1)' }}>
            <div className="h-full transition-all" style={{ width: `${course.progress}%`, background: 'linear-gradient(90deg, #9E7FFF, #f472b6)' }} />
          </div>
        )}
      ...
      <div className="p-4">
        ...
        {course.status === 'enrolled' && course.progress !== undefined && (
          <div className="mb-3">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-xs font-bold" style={{ color: '#9E7FFF' }}>{course.progress}%</span>
            </div>
            <div className="h-1.5 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.06)' }}>
              <div className="h-full rounded-full" style={{ width: `${course.progress}%`, background: 'linear-gradient(90deg, #9E7FFF, #f472b6)' }} />
            </div>
          </div>
        )}
        ...
        {course.status === 'available' && (
          <button onClick={() => onEnroll(course.id)}
            ...
            onMouseEnter={e => { e.currentTarget.style.background = 'rgba(158,127,255,0.25)' }}
            onMouseLeave={e => { e.currentTarget.style.background = 'rgba(158,127,255,0.12)' }}>
        ...
        {course.status === 'locked' && (
          <button className="w-full py-2.5 rounded-xl text-xs font-bold"
            style={{ background: 'rgba(158,127,255,0.08)', border: '1px solid rgba(158,127,255,0.15)', color: '#6666aa' }}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl p-6 relative overflow-hidden"
        style={{
          background: 'linear-gradient(135deg, rgba(245,158,11,0.08) 0%, rgba(158,127,255,0.06) 50%, rgba(56,189,248,0.04) 100%)',
        ...
        <div className="relative z-10 flex items-center gap-4 flex-wrap">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0"
            style={{ background: 'linear-gradient(135deg, #f59e0b, #f472b6)', boxShadow: '0 0 30px rgba(245,158,11,0.4)' }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: 'Cursos en curso', value: enrolledCourses.length, color: '#9E7FFF', icon: BookOpen },
          { label: 'Cursos completados', value: completedCourses.length, color: '#10b981', icon: CheckCircle },
          { label: 'Horas aprendidas', value: `${totalHoursLearned}h`, color: '#38bdf8', icon: Clock },
        ...
        <div className="space-y-5">
          ...
          {filteredCourses.length === 0 ? (
            <div className="card-gastro text-center py-16">
              <BookOpen size={32} className="mx-auto mb-3" style={{ color: '#4a4a6a' }} />
      ...
      {activeTab === 'my_learning' && (
        <div className="space-y-6">
          ...
          {enrolledCourses.length > 0 && (
            <div>
              <h3 className="font-bold text-gastro-text mb-4 flex items-center gap-2">
                <Play size={16} style={{ color: '#9E7FFF' }} /> En curso ({enrolledCourses.length})
              </h3>
          ...
          {enrolledCourses.length === 0 && completedCourses.length === 0 && (
            <div className="card-gastro text-center py-16">
              <GraduationCap size={40} className="mx-auto mb-4" style={{ color: '#4a4a6a' }} />
          ...
          {(enrolledCourses.length > 0 || completedCourses.length > 0) && (
            <div className="rounded-2xl p-5"
              style={{ background: 'linear-gradient(135deg, rgba(245,158,11,0.08), rgba(244,114,182,0.05))', border: '1px solid rgba(245,158,11,0.2)' }}>
                ...
                {['L','M','X','J','V','S','D'].map((day, i) => (
                  <div key={day} className="flex-1 flex flex-col items-center gap-1">
                    <div className="w-full h-8 rounded-lg flex items-center justify-center"
                      style={{ background: i < 7 ? 'rgba(245,158,11,0.25)' : 'rgba(255,255,255,0.04)', border: `1px solid ${i < 7 ? 'rgba(245,158,11,0.4)' : '#2a2a3d'}` }}>
            ...
            {LEARNING_PATHS.map(path => {
              ...
              return (
                <div key={path.id} className="rounded-2xl p-5 cursor-pointer transition-all duration-200 group"
                  style={{ background: '#1a1a26', border: `1px solid ${path.color}22` }}
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {CERTIFICATIONS.map(cert => (
              <div key={cert.id} className="rounded-2xl p-5 transition-all"
                style={{
                  background: cert.earned ? `${cert.color}08` : '#1a1a26',
                  border: `1px solid ${cert.earned ? cert.color + '44' : '#2a2a3d'}`,
                }}>
                <div className="flex items-start gap-4 mb-4">
                    ...
                    {cert.requirements.map(req => {
                      ...
                      return (
                        <div key={req} className="flex items-center gap-2 text-xs">
                          ...
                          <span style={{ color: isDone ? '#10b981' : '#8888aa', textDecoration: isDone ? 'none' : 'none' }}>
                            {req}
                          </span>
                        </div>
                      )
