...
const WAITERS_PERF: WaiterPerf[] = [
  ...
  { id: 'w2', name: 'Valentina', emoji: '👩‍🍳', color: '#f472b6', avgResponseTime: 2.1, alertsResolved: 28, rating: 4.7 },
  { id: 'w3', name: 'Rodrigo', emoji: '🧑‍🍳', color: '#38bdf8', avgResponseTime: 2.6, alertsResolved: 21, rating: 4.5 },
]
...
const ALERT_CONFIG: Record<AlertType, { label: string; icon: React.ComponentType<{size?:number;style?:React.CSSProperties}>; color: string; bg: string; emoji: string }> = {
  empty_glass:  { label: 'Vaso vacío',        icon: GlassWater,      color: '#38bdf8', bg: 'rgba(56,189,248,0.12)',   emoji: '🥤' },
  empty_plate:  { label: 'Plato vacío',        icon: UtensilsCrossed, color: '#9E7FFF', bg: 'rgba(158,127,255,0.12)', emoji: '🍽️' },
...
const PRIORITY_CONFIG: Record<AlertPriority, { label: string; color: string; bg: string; pulse: boolean }> = {
  ...
  medium:   { label: 'Media',    color: '#9E7FFF', bg: 'rgba(158,127,255,0.15)', pulse: false },
  low:      { label: 'Baja',     color: '#8888aa', bg: 'rgba(136,136,170,0.12)', pulse: false },
}
...
const INITIAL_ALERTS: EyeAlert[] = [
  ...
  },
  {
    ...
    assignedTo: 'w2', assignedWaiter: { name: 'Valentina', emoji: '👩‍🍳', color: '#f472b6' },
  ...
  },
  {
    ...
    assignedTo: 'w3', assignedWaiter: { name: 'Rodrigo', emoji: '🧑‍🍳', color: '#38bdf8' },
    thumbnail: 'https://images.pexels.com/photos/1267320/pexels-photo-1267320.jpeg?w=300',
  },
  {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none rounded-xl">
      <div style={{
        ...
        background: 'linear-gradient(90deg, transparent, #f472b6, #9E7FFF, transparent)',
...
function AlertCard({ alert, onAssign, onResolve, onDismiss }: {
  ...
  return (
    <div className="rounded-2xl overflow-hidden transition-all duration-300"
      style={{
        background: alert.status === 'resolved' ? 'rgba(255,255,255,0.02)' : '#1a1a26',
        border: `1px solid ${alert.status === 'new' && (alert.priority === 'critical' || alert.priority === 'high') ? priCfg.color + '44' : '#2a2a3d'}`,
      ...
      <div className="p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mb-1">
              <MapPin size={11} style={{ color: '#8888aa', flexShrink: 0 }} />
            ...
            <div className="flex items-center gap-2 p-2 rounded-xl"
              ...
              <Zap size={11} style={{ color: '#9E7FFF', flexShrink: 0 }} />
              <span className="text-xs font-semibold" style={{ color: '#9E7FFF' }}>{alert.actionSuggested}</span>
            </div>
          </div>
        ...
        {/* Actions */}
        {alert.status !== 'resolved' && (
          <div className="flex items-center gap-2 mt-3 pt-3 border-t" style={{ borderColor: '#2a2a3d' }}>
            {alert.status === 'new' && (
        ...
        {showAssign && alert.status === 'new' && (
          <div className="mt-3 p-3 rounded-xl space-y-2"
            ...
            <p className="text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: '#9E7FFF' }}>
              Asignar a mozo
            </p>
            {WAITERS_PERF.map(w => (
              <button key={w.id}
...
function CameraCard({ cam, isSelected, onClick }: {
  ...
  return (
    <button onClick={onClick}
      ...
      style={{
        border: `2px solid ${isSelected ? '#f472b6' : cam.status === 'offline' ? '#2a2a3d' : '#2a2a3d'}`,
        boxShadow: isSelected ? '0 0 20px rgba(244,114,182,0.2)' : 'none',
      }}>
          ...
          <>
            {[['top-2 left-2', 'border-t-2 border-l-2'], ['top-2 right-2', 'border-t-2 border-r-2'],
              ...
              <div key={i} className={`absolute ${pos} w-4 h-4 ${border}`}
                style={{ borderColor: '#f472b6', opacity: 0.7 }} />
            ))}
          </>
        )}
      ...
      <div className="p-3" style={{ background: '#141420' }}>
        <div className="flex items-center justify-between mb-1">
          ...
          {isSelected && <Eye size={12} style={{ color: '#f472b6' }} />}
        </div>
        <div className="flex items-center gap-2">
          <MapPin size={10} style={{ color: '#6666aa' }} />
          <span className="text-xs" style={{ color: '#8888aa' }}>{cam.zone}</span>
          {cam.tables.length > 0 && (
...
export default function GastroEye() {
  ...
  const [activeTab, setActiveTab] = useState<'live' | 'cameras' | 'analytics'>('live')
    ...
    <div className="space-y-6">
      ...
      <style>{`
        ...
        @keyframes flash-border {
          ...
          50% { box-shadow: 0 0 0 2px rgba(244,114,182,0.6), 0 0 20px rgba(244,114,182,0.2); }
        }
      `}</style>
      ...
      <div className="rounded-2xl p-6 relative overflow-hidden"
        style={{
          background: 'linear-gradient(135deg, rgba(244,114,182,0.08) 0%, rgba(158,127,255,0.06) 50%, rgba(56,189,248,0.04) 100%)',
          border: `1px solid ${newAlertFlash ? 'rgba(244,114,182,0.5)' : 'rgba(244,114,182,0.2)'}`,
        ...
        <div className="relative z-10 flex items-center gap-4 flex-wrap">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 relative"
            style={{ background: 'linear-gradient(135deg, #f472b6, #9E7FFF)', boxShadow: '0 0 30px rgba(244,114,182,0.4)' }}>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button onClick={() => setSoundEnabled(!soundEnabled)}
              ...
              style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid #2a2a3d', color: soundEnabled ? '#9E7FFF' : '#6666aa' }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
        {[
          ...
          { label: 'Confianza IA', value: `${avgConfidence}%`, color: '#9E7FFF', icon: Brain, urgent: false },
          { label: 'T. respuesta', value: `${avgResponseTime.toFixed(1)}m`, color: '#38bdf8', icon: Clock, urgent: false },
        ].map(stat => {
        ...
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id as typeof activeTab)}
            ...
            style={activeTab === tab.id
              ? { background: 'rgba(244,114,182,0.15)', border: '1px solid rgba(244,114,182,0.35)', color: '#f472b6' }
              : { background: 'rgba(255,255,255,0.03)', border: '1px solid #2a2a3d', color: '#8888aa' }}>
            {tab.label}
            {tab.count !== null && tab.count > 0 && (
              <span className="text-xs px-1.5 py-0.5 rounded-full font-black"
                style={{ background: activeTab === tab.id ? 'rgba(244,114,182,0.25)' : 'rgba(255,255,255,0.08)', color: activeTab === tab.id ? '#f472b6' : '#8888aa' }}>
                {tab.count}
              </span>
            )}
          </button>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <div className="flex items-center gap-1.5 text-xs font-semibold" style={{ color: '#8888aa' }}>
            ...
            <div className="space-y-3">
              {filteredAlerts.length === 0 ? (
                <div className="card-gastro text-center py-16">
                  <Eye size={32} className="mx-auto mb-3" style={{ color: '#4a4a6a' }} />
          ...
          <div className="space-y-4">
            ...
            <div className="card-gastro">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-bold text-gastro-text text-sm flex items-center gap-2">
                  <Camera size={14} style={{ color: '#f472b6' }} /> Feed en vivo
                </h3>
                ...
                return (
                  <div>
                        ...
                        <>
                          ...
                          {[['top-2 left-2', 'border-t-2 border-l-2'], ['top-2 right-2', 'border-t-2 border-r-2'],
                            ...
                            <div key={i} className={`absolute ${pos} w-5 h-5 ${border}`}
                              style={{ borderColor: '#f472b6', opacity: 0.8 }} />
                          ))}
                          {/* Detection boxes simulation */}
                          <div className="absolute" style={{ top: '30%', left: '20%', width: '25%', height: '35%', border: '1.5px solid #38bdf8', borderRadius: 4 }}>
                            <div className="absolute -top-4 left-0 text-xs font-bold px-1 rounded"
                              style={{ background: 'rgba(56,189,248,0.8)', color: '#fff', fontSize: '8px', whiteSpace: 'nowrap' }}>
                              🥤 Vaso vacío 94%
                            </div>
                          </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="p-2 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid #2a2a3d' }}>
                      ...
                      <div className="p-2 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid #2a2a3d' }}>
                        ...
                        <div className="font-bold" style={{ color: '#f472b6' }}>{cam.alertsToday}</div>
                      </div>
                    </div>
                  </div>
                )
            ...
            <div className="card-gastro">
              <h3 className="font-bold text-gastro-text text-sm mb-3 flex items-center gap-2">
                <Users size={14} style={{ color: '#9E7FFF' }} /> Mozos en turno
              </h3>
              <div className="space-y-3">
                {WAITERS_PERF.map(w => (
            ...
            <div className="card-gastro">
              <h3 className="font-bold text-gastro-text text-sm mb-3 flex items-center gap-2">
                <Brain size={14} style={{ color: '#f472b6' }} /> Patrones detectados
              </h3>
        ...
        <div className="space-y-6">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {[
              ...
              { label: 'Alertas resueltas hoy', value: `${resolvedCount}/${alerts.length}`, sub: `${Math.round(resolvedCount / alerts.length * 100)}% tasa de resolución`, color: '#9E7FFF', icon: CheckCircle },
              { label: 'Confianza promedio IA', value: `${avgConfidence}%`, sub: 'Modelo v2.4 · 6 cámaras', color: '#f472b6', icon: Brain },
            ].map(kpi => {
              const Icon = kpi.icon
              return (
              ...
              <div className="flex items-end gap-2 h-32">
                {hourlyData.map(d => (
                  <div key={d.hour} className="flex-1 flex flex-col items-center gap-1">
                    <div className="text-xs font-bold" style={{ color: '#f472b6' }}>{d.alerts}</div>
          ...
          <div className="card-gastro">
            <h3 className="font-bold text-gastro-text mb-4 flex items-center gap-2">
              <TrendingUp size={15} style={{ color: '#9E7FFF' }} /> Performance de mozos — Respuesta a alertas
            </h3>
                ...
                <thead>
                  <tr style={{ borderBottom: '1px solid #2a2a3d' }}>
                    {['Mozo', 'Alertas resueltas', 'T. respuesta prom.', 'Rating', 'Eficiencia'].map(h => (
                      <th key={h} className="text-left pb-3 pr-4 font-semibold" style={{ color: '#8888aa' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
          ...
          <div className="rounded-2xl p-5"
            style={{ background: 'linear-gradient(135deg, rgba(244,114,182,0.06), rgba(158,127,255,0.04))', border: '1px solid rgba(244,114,182,0.15)' }}>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {[
                { label: 'Detección de vasos/platos', accuracy: 96, color: '#38bdf8' },
