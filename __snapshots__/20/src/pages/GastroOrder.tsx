...
// Renders the full-screen black backdrop + centered iPhone frame.
// Children are rendered inside the screen area (below status bar).
  ...
  return (
    <div
      ...
      <div className="absolute top-6 left-6 flex items-center gap-2 z-10">
        {labelDot}
        <span className="text-xs font-semibold" style={{ color: '#6666aa' }}>{label}</span>
      </div>
      ...
      <button
        ...
        style={{ border: '1px solid rgba(255,255,255,0.12)', color: '#6666aa', background: 'rgba(255,255,255,0.04)' }}
      ...
      {showWaiterSelect && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="w-80 rounded-3xl overflow-hidden"
            ...
            <div className="p-4 space-y-2">
              {MOCK_WAITERS.map(w => (
                <button key={w.id} onClick={() => handleWaiterSelect(w)}
                  className="w-full flex items-center gap-3 p-4 rounded-2xl transition-all"
                  style={{ background: `${w.color}0d`, border: `1px solid ${w.color}33` }}
                  onMouseEnter={e => (e.currentTarget.style.transform = 'scale(1.01)')}
                  onMouseLeave={e => (e.currentTarget.style.transform = 'scale(1)')}>
                  <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-2xl" style={{ background: `${w.color}18` }}>
                    {w.emoji}
                  </div>
                  <div className="flex-1 text-left">
