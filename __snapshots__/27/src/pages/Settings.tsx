...
const INTEGRATIONS = [
  ...
  { id: 'pedidosya', name: 'PedidosYa', status: 'disconnected', color: '#8888aa', desc: 'No conectado' },
  { id: 'mercadopago', name: 'MercadoPago', status: 'connected', color: '#10b981', desc: 'Pagos QR y online activos' },
  { id: 'stripe', name: 'Stripe', status: 'disconnected', color: '#8888aa', desc: 'No conectado' },
                ...
                return (
                  <div key={key} className="flex items-center justify-between p-4 rounded-xl"
                    ...
                    <button
                      ...
                      style={{ background: value ? 'rgba(158,127,255,0.4)' : 'rgba(255,255,255,0.1)' }}>
                      <div className="absolute top-1 w-4 h-4 rounded-full transition-all duration-300"
                        style={{
                          background: value ? '#9E7FFF' : '#8888aa',
                          left: value ? '26px' : '4px',
                          boxShadow: value ? '0 0 8px rgba(158,127,255,0.6)' : 'none',
                        }} />
                    </button>
                  </div>
                )
              ...
              {INTEGRATIONS.map(integration => (
                <div key={integration.id} className="flex items-center gap-4 p-4 rounded-xl"
                  ...
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                    ...
                    <Plug size={18} style={{ color: integration.status === 'connected' ? '#10b981' : '#8888aa' }} />
                  </div>
                  ...
                  <button className={`text-xs font-semibold px-3 py-1.5 rounded-lg transition-all ${integration.status === 'connected' ? 'text-error hover:bg-error/10' : 'text-primary-400 hover:bg-primary-400/10'}`}
                    style={{ border: `1px solid ${integration.status === 'connected' ? 'rgba(239,68,68,0.3)' : 'rgba(158,127,255,0.3)'}` }}>
          ...
          <div className="space-y-4">
            <div className="card-gastro">
              ...
              <div className="p-4 rounded-xl mb-4"
                style={{ background: 'linear-gradient(135deg, rgba(158,127,255,0.1) 0%, rgba(56,189,248,0.06) 100%)', border: '1px solid rgba(158,127,255,0.25)' }}>
                <div className="flex items-center justify-between">
                  <div>
              ...
              <div className="grid md:grid-cols-3 gap-3">
                {[
                  ...
                  <div key={usage.label} className="p-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid #2a2a3d' }}>
