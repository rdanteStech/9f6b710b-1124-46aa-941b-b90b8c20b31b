...
export default function ServeHoursCountdown({
  ...
  if (compact) {
    return (
      <div
        className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1.5 rounded-lg flex-shrink-0 whitespace-nowrap min-w-[7.5rem]"
        style={{ background: accent.bg, border: `1px solid ${accent.border}` }}
        title={`${targetLabel} a las ${formatTargetTime(state.target)}`}
        role="status"
        aria-live="polite"
        aria-label={`${label} ${formatCountdown(state.remainingMs)}`}
      >
        <Icon size={14} style={{ color: accent.color, flexShrink: 0 }} />
        <span className="text-[11px] sm:text-xs font-semibold" style={{ color: accent.color }}>
          {label}
        </span>
        <span
          className={`text-xs sm:text-sm font-black tabular-nums tracking-wide ${urgent ? 'animate-pulse' : ''}`}
