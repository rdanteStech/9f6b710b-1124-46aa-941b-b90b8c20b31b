...
import { getRecipeForProduct } from '../data/recipeLab'
import { formatCurrency } from '../lib/locale'
import {
  BookOpen, Plus, Search, Globe, Smartphone, UtensilsCrossed,
  Bike, ShoppingBag, Eye, EyeOff, Edit, TrendingUp, Layers, ChefHat, Check,
} from 'lucide-react'
...
type ViewId = 'products' | 'channels' | 'categories'
...
function parseViewTab(tab: string | null): ViewId | 'recipes' {
  if (tab === 'recetas' || tab === 'recipes' || tab === 'lab') return 'recipes'
  if (tab === 'canales' || tab === 'channels') return 'channels'
  if (tab === 'categorias' || tab === 'categories') return 'categories'
  return 'products'
}

export default function GastroMenu() {
  const [searchParams, setSearchParams] = useSearchParams()
  const tabFromUrl = parseViewTab(searchParams.get('tab'))

  if (tabFromUrl === 'recipes') {
    const open = searchParams.get('open')
    return <Navigate to={open ? `/recipe?open=${open}` : '/recipe'} replace />
  }

  const [activeCategory, setActiveCategory] = useState<number | null>(null)
  const [searchQuery, setSearchQuery] = useState('')
  const [activeView, setActiveView] = useState<ViewId>(tabFromUrl)

  useEffect(() => {
    const parsed = parseViewTab(searchParams.get('tab'))
    if (parsed !== 'recipes') {
      setActiveView(prev => (prev === parsed ? prev : parsed))
    }
  }, [searchParams])

  const setView = useCallback((view: ViewId) => {
    setActiveView(view)
    const tabParam = view === 'channels' ? 'canales' : view === 'categories' ? 'categorias' : null
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            <div className="space-y-3">
              {filteredProducts.map(product => {
                const linkedRecipe = getRecipeForProduct(product.id)
                return (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <div className="text-right flex-shrink-0">
                            <div className="font-black text-gastro-text">{formatCurrency(product.price)}</div>
                            <div className="text-xs text-gastro-subtle">Costo: {formatCurrency(product.cost)}</div>
                          </div>
                        </div>
                ...
                {MENU_PRODUCTS.map(product => (
                  <tr key={product.id} style={{ borderBottom: '1px solid rgba(42,42,61,0.5)' }}
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className="font-bold text-gastro-text">{formatCurrency(product.price)}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
