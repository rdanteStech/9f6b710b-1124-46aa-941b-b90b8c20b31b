...
import { useStock, type InventoryItem } from '../context/StockContext'
import {
  Package, AlertTriangle, TrendingDown, ShoppingCart, Plus,
  Search, Filter, RefreshCw, X, Receipt, ArrowRight, CheckCircle2, ChefHat,
} from 'lucide-react'
...
export default function GastroStock() {
  const { items, pendingInvoices, recentAdjustments, addItem } = useStock()
  const [activeView, setActiveView] = useState<'inventory' | 'orders'>('inventory')
  ...
  const supplierOptions = [...new Set(items.map(i => i.supplier))]
  ...
  const handleAddItem = () => {
    ...
    addItem({ name, unit: form.unit, stock, minStock, maxStock, cost, supplier })
    closeModal()
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Pending invoices from GastroSupply */}
      {pendingInvoices.length > 0 && (
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-4 rounded-xl"
          ...
          <Receipt size={18} className="text-primary-400 flex-shrink-0" />
          <div className="flex-1">
            <span className="text-sm font-semibold text-primary-400">
              {pendingInvoices.length} factura{pendingInvoices.length > 1 ? 's' : ''} de compra reconocida{pendingInvoices.length > 1 ? 's' : ''} —{' '}
            </span>
            <span className="text-sm text-gastro-subtle">
              GastroSupply detectó productos listos para ingresar al inventario. Aprueba sin cargar datos manualmente.
            </span>
          </div>
          <Link to="/supply?tab=invoices" className="btn-primary text-xs px-3 py-2 flex-shrink-0">
            Revisar facturas <ArrowRight size={13} />
          </Link>
        </div>
      )}
      ...
      {recentAdjustments.length > 0 && (
        <div className="card-gastro p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-sm font-bold text-gastro-text">Últimos ingresos por factura</h3>
          </div>
          <div className="space-y-2">
            {recentAdjustments.slice(0, 3).map(adj => (
              <div key={adj.id} className="flex items-center gap-3 text-xs p-2.5 rounded-xl"
                style={{ background: 'rgba(16,185,129,0.06)', border: '1px solid rgba(16,185,129,0.15)' }}>
                <span className="font-bold text-success">+{adj.quantity} {adj.unit}</span>
                <span className="text-gastro-text font-semibold">{adj.itemName}</span>
                <span className="text-gastro-subtle hidden sm:inline">· {adj.invoiceNumber}</span>
                <span className="text-gastro-muted ml-auto">{adj.appliedAt}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Link to="/recipe" className="btn-secondary text-sm px-4 py-2">
            <ChefHat size={15} /> Recetas
          </Link>
              ...
              {filteredItems.map(item => {
                ...
                return (
                  <tr key={item.id}>
                    ...
                    <td className="font-semibold text-gastro-text">{formatCurrency(item.cost)}</td>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={order.id} className="flex items-center gap-4 p-4 rounded-xl"
                  ...
                  <div className="font-bold text-gastro-text">{formatCurrency(order.total)}</div>
