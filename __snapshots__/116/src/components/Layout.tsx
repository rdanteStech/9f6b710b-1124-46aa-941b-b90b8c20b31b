...
import {
  ...
  Truck, Plug, GraduationCap, ChevronRight, Zap, X, ScanEye, CalendarCheck, ChefHat, Contact
} from 'lucide-react'
...
const ICON_MAP: Record<string, LucideIcon> = {
  ...
  Package, TrendingUp, Users, ClipboardList, BarChart3,
  Brain, Heart, Globe, Smartphone, Calendar, Network,
  Truck, Plug, GraduationCap, ScanEye, CalendarCheck, ChefHat, Contact,
}
...
export default function Layout() {
  ...
  const currentModule = isSettings
    ...
    : MODULES.find(m => location.pathname === m.path || location.pathname.startsWith(`${m.path}/`))
  ...
  const filteredGroups = SIDEBAR_GROUPS.map(group => ({
    ...
    items: group.items.filter(id => {
      ...
      return !q
        ...
        || (q.includes('receta') && mod?.id === 'recipe')
        || ((q.includes('cliente') || q.includes('crm') || q.includes('gastroclient')) && mod?.id === 'clients')
    })
  ...
  return (
    <div className="min-h-screen bg-gastro-bg flex">
      ...
      <aside className={`flex-shrink-0 flex flex-col transition-all duration-300 ${sidebarCollapsed ? 'w-16' : 'w-64'}`}
        ...
        <div className="h-14 flex items-center px-3 border-b" style={{ borderColor: '#152035' }}>
          <button
            ...
            <Logo size={sidebarCollapsed ? 'sm' : 'sm'} iconOnly={sidebarCollapsed} />
          </button>
          <button
            type="button"
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Topbar — cuenta regresiva siempre visible a la izquierda de "En vivo" */}
        <header
          className="h-14 flex items-center gap-2 sm:gap-3 px-3 sm:px-6 flex-shrink-0"
        ...
        >
          <div className="min-w-0 flex-1 overflow-hidden">
            ...
            <p className="text-xs text-gastro-subtle truncate hidden lg:block">
              {currentModule?.description || 'Vista ejecutiva en tiempo real'}
            </p>
          </div>

          {/* Horario + estado + notificaciones — bloque fijo que no se comprime */}
          <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
            ...
            <div
              className="flex items-center gap-2 px-2.5 sm:px-3 py-1.5 rounded-lg whitespace-nowrap flex-shrink-0"
              style={
                isOpen
