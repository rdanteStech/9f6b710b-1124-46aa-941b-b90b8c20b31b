...
const DELIVERY_ZONES: DeliveryZone[] = [
  { id: 'z1', name: 'Bellavista', radius: '2 km', fee: 2500, minOrder: 15000, active: true },
  { id: 'z2', name: 'Providencia', radius: '3 km', fee: 3000, minOrder: 18000, active: true },
  { id: 'z3', name: 'Las Condes', radius: '5 km', fee: 3500, minOrder: 20000, active: false },
  { id: 'z4', name: 'Ñuñoa', radius: '4 km', fee: 3000, minOrder: 18000, active: true },
]

const ONLINE_ORDERS: OrderOnline[] = [
  { id: 'OL-001', customer: 'Camila Rojas', items: 'Pasta carbonara x2, Tiramisú x1', total: 24100, status: 'delivering', time: '18 min', channel: 'app' },
  { id: 'OL-002', customer: 'Sofía Herrera', items: 'Pizza margherita x1, Ensalada César x1', total: 15000, status: 'preparing', time: '12 min', channel: 'web' },
  { id: 'OL-003', customer: 'Martín Soto', items: 'Risotto x1, Vino Carmenère x1', total: 19000, status: 'pending', time: '2 min', channel: 'whatsapp' },
  { id: 'OL-004', customer: 'Valentina Cruz', items: 'Lomo a la pimienta x2, Papas rosti x2', total: 36000, status: 'ready', time: '5 min', channel: 'app' },
  { id: 'OL-005', customer: 'Diego Ramírez', items: 'Salmón x1, Tiramisú x1', total: 21000, status: 'delivered', time: 'Hace 1h', channel: 'web' },
]
...
const STATUS_CFG = {
  ...
  preparing:  { label: 'Preparando',   color: '#3B82F6', bg: 'rgba(59,130,246,0.12)'  },
  ready:      { label: 'Listo',        color: '#2563EB', bg: 'rgba(37,99,235,0.12)' },
  delivering: { label: 'En camino',    color: '#60A5FA', bg: 'rgba(96,165,250,0.12)' },
  delivered:  { label: 'Entregado',    color: '#10b981', bg: 'rgba(16,185,129,0.12)'  },
}

const CHANNEL_CFG = {
  app:       { label: 'App',       color: '#2563EB', emoji: '📱' },
  web:       { label: 'Web',       color: '#3B82F6', emoji: '🌐' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl p-6 relative overflow-hidden"
        style={{
          background: 'linear-gradient(135deg, rgba(37,99,235,0.08) 0%, rgba(59,130,246,0.06) 50%, rgba(16,185,129,0.04) 100%)',
        ...
        <div className="relative z-10 flex items-center gap-4 flex-wrap">
          ...
          <div className="hidden md:grid grid-cols-3 gap-6 text-center flex-shrink-0">
            {[
              ...
              <div key={s.label}>
                <div className="text-xl font-black" style={{ color: '#2563EB' }}>{s.value}</div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Pedidos hoy', value: orders.length, color: '#2563EB', icon: ShoppingBag, sub: `${activeOrders.length} activos` },
          { label: 'Facturación online', value: `$${(todayRevenue / 1000).toFixed(1)}K`, color: '#10b981', icon: DollarSign, sub: 'hoy' },
          { label: 'Tiempo promedio', value: '28 min', color: '#3B82F6', icon: Timer, sub: 'entrega' },
          { label: 'Clientes recurrentes', value: '68%', color: '#60A5FA', icon: Repeat, sub: 'este mes' },
        ].map(stat => {
        ...
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id as typeof activeTab)}
            ...
            style={activeTab === tab.id
              ? { background: 'rgba(37,99,235,0.15)', border: '1px solid rgba(37,99,235,0.35)', color: '#2563EB' }
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="card-gastro flex flex-col items-center">
            ...
            <div className="w-48 rounded-3xl overflow-hidden shadow-2xl"
              style={{ background: '#000000', border: '3px solid #1A2540', boxShadow: '0 0 40px rgba(37,99,235,0.2)' }}>
              {/* Status bar */}
              <div className="flex items-center justify-between px-4 py-2" style={{ background: '#111118' }}>
                ...
                <div className="flex gap-1">
                  ...
                  <div className="w-1 h-1.5 rounded-sm" style={{ background: '#8899BB' }} />
                </div>
              </div>
              {/* App header */}
              <div className="px-3 py-3" style={{ background: 'linear-gradient(135deg, #2563EB22, #3B82F622)' }}>
                <div className="text-xs font-black text-white">Trattoria Bellavista</div>
              ...
              {/* Featured item */}
              <div className="mx-2 mb-2 p-2 rounded-xl" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid #1A2540' }}>
                <div className="text-xs font-bold text-gastro-text" style={{ fontSize: '10px' }}>🔥 Más pedido</div>
                <div className="text-xs text-gastro-subtle" style={{ fontSize: '9px' }}>Pasta carbonara · $9.800</div>
              </div>
            </div>
          ...
          <div className="card-gastro">
            ...
            <div className="space-y-3">
              {[
                { name: 'App GastroGo', emoji: '📱', orders: 48, revenue: 124000, active: true, color: '#2563EB' },
                { name: 'Web propia', emoji: '🌐', orders: 31, revenue: 87000, active: true, color: '#3B82F6' },
                ...
                { name: 'Rappi', emoji: '🟠', orders: 19, revenue: 54000, active: false, color: '#60A5FA' },
              ].map(ch => (
                <div key={ch.name} className="flex items-center gap-3 p-2.5 rounded-xl"
                  ...
                  <div className={`w-2 h-2 rounded-full flex-shrink-0`}
                    style={{ background: ch.active ? '#10b981' : '#4A5A7A' }} />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-2 rounded-2xl p-5 relative overflow-hidden"
            style={{ background: 'linear-gradient(135deg, rgba(37,99,235,0.1), rgba(59,130,246,0.06))', border: '1px solid rgba(37,99,235,0.25)' }}>
            ...
            <div className="relative z-10">
              ...
              <p className="text-sm text-gastro-subtle mb-4">
                Con GastroGo tienes tu propio canal de delivery. Ahorras hasta un <strong className="text-gastro-text">30% en comisiones</strong> comparado con PedidosYa o Rappi.
              </p>
              <div className="grid grid-cols-3 gap-4">
                {[
                  { label: 'Ahorro estimado', value: '$48.000/mes', color: '#10b981' },
                  { label: 'Comisión GastroGo', value: '0%', color: '#2563EB' },
          ...
          <div className="space-y-4">
            ...
            {zones.map(zone => (
              <div key={zone.id} className="card-gastro flex items-center gap-4"
                style={{ border: `1px solid ${zone.active ? 'rgba(37,99,235,0.25)' : '#1A2540'}` }}>
                <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                  style={{ background: zone.active ? 'rgba(37,99,235,0.12)' : 'rgba(255,255,255,0.04)' }}>
                  <MapPin size={18} style={{ color: zone.active ? '#2563EB' : '#4A5A7A' }} />
                </div>
                <div className="flex-1 min-w-0">
          ...
          <div className="card-gastro">
            ...
            <div className="space-y-4">
              {[
                ...
                { label: 'Pedido mínimo general', value: '$15.000', icon: DollarSign },
              ...
              <div className="pt-2">
                <button className="w-full py-3 rounded-xl text-sm font-bold"
                  style={{ background: 'linear-gradient(135deg, rgba(37,99,235,0.15), rgba(59,130,246,0.1))', border: '1px solid rgba(37,99,235,0.3)', color: '#2563EB' }}>
            ...
            <div className="space-y-2">
              {sections.map(section => (
                <div key={section.id} className="flex items-center gap-3 p-3 rounded-xl transition-all"
                  style={{ background: section.active ? 'rgba(37,99,235,0.05)' : 'rgba(255,255,255,0.02)', border: `1px solid ${section.active ? 'rgba(37,99,235,0.2)' : '#1A2540'}` }}>
                  ...
                  <button onClick={() => toggleSection(section.id)}>
                    {section.active
                      ...
                      : <ToggleLeft size={22} style={{ color: '#4A5A7A' }} />
                    }
                  </button>
                </div>
              ))}
                  ...
                  <div key={notif.label} className="flex items-center justify-between">
                    ...
                    <div className="w-8 h-4 rounded-full relative cursor-pointer"
                      ...
                      <div className="w-3 h-3 rounded-full absolute top-0.5 transition-all"
                        style={{ background: notif.active ? '#10b981' : '#4A5A7A', left: notif.active ? '18px' : '2px' }} />
                    </div>
                  </div>
                ))}
              </div>
      ...
      {activeTab === 'analytics' && (
        <div className="grid lg:grid-cols-2 gap-6">
          <div className="card-gastro">
            ...
            <div className="space-y-3">
              {[
                { label: 'App GastroGo', value: 312, total: 580, color: '#2563EB', emoji: '📱' },
                { label: 'Web propia', value: 168, total: 580, color: '#3B82F6', emoji: '🌐' },
          ...
          <div className="card-gastro">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: 'Descargas totales', value: '4.284', color: '#2563EB' },
                { label: 'Usuarios activos/mes', value: '1.842', color: '#3B82F6' },
                { label: 'Tasa de conversión', value: '7.3%', color: '#10b981' },
                { label: 'Ticket promedio', value: '$18.450', color: '#f59e0b' },
                { label: 'Reseñas positivas', value: '94%', color: '#60A5FA' },
                { label: 'Tiempo en app', value: '4.2 min', color: '#2563EB' },
              ].map(m => (
              ...
              {[42, 68, 55, 89, 74, 96, 81].map((val, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full rounded-t-lg transition-all"
                    style={{ height: `${(val / 96) * 100}%`, background: `linear-gradient(to top, #2563EB, #3B82F6)`, opacity: i === 5 ? 1 : 0.5 }} />
