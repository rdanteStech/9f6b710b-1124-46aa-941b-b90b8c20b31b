...
// Clamp table inside polygon with wallDistPx margin using iterative edge pushing
function clampInsidePolygon(
  ...
  // Iterative push: run 3 passes to settle
...
// Table-to-table overlap check — with 0 pad when snapped
...
function snapToNeighbor(moving: TableItem, others: TableItem[], snapDist: number): SnapResult {
  ...
  for (const other of others) {
    ...
    // right edge of other → left edge of moving (moving goes to right of other)
    ...
    // left edge of other → right edge of moving (moving goes to left of other)
...
// Wall snap: checks ALL 4 edges of the table against ALL polygon edges
// Returns the best snap position that places the nearest table edge at wallDistPx from the wall
function snapToWall(
  ...
  // The 4 "probe points" — midpoint of each table edge, and what snap position they produce
  const probes = [
    {
      // Top edge midpoint
      px: table.x + table.width / 2,
      py: table.y,
      getSnap: (wallX: number, wallY: number, nx: number, ny: number) => {
        // Push table so top edge is wallDistPx from wall
        // Wall normal points inward; top edge should be wallDistPx inside
        const len = Math.sqrt(nx * nx + ny * ny) || 1
        return { x: table.x, y: wallY + ny / len * wallDistPx }
      },
    },
    {
      // Bottom edge midpoint
      px: table.x + table.width / 2,
      py: table.y + table.height,
      getSnap: (wallX: number, wallY: number, nx: number, ny: number) => {
        const len = Math.sqrt(nx * nx + ny * ny) || 1
        return { x: table.x, y: wallY + ny / len * wallDistPx - table.height }
      },
    },
    {
      // Left edge midpoint
      px: table.x,
      py: table.y + table.height / 2,
      getSnap: (wallX: number, wallY: number, nx: number, ny: number) => {
        const len = Math.sqrt(nx * nx + ny * ny) || 1
        return { x: wallX + nx / len * wallDistPx, y: table.y }
      },
    },
    {
      // Right edge midpoint
      px: table.x + table.width,
      py: table.y + table.height / 2,
      getSnap: (wallX: number, wallY: number, nx: number, ny: number) => {
        const len = Math.sqrt(nx * nx + ny * ny) || 1
        return { x: wallX + nx / len * wallDistPx - table.width, y: table.y }
      },
    },
  ]
  ...
  for (let i = 0; i < poly.length; i++) {
    ...
    for (const probe of probes) {
      ...
      if (gap < bestGap) {
        bestGap = gap
        // Normal from wall toward probe point
        const nx = probe.px - nearX
        const ny = probe.py - nearY
...
function TableSVG({ table, selected, onSelect, onDragStart, minDist, onUnfuse, wallDistPx }: {
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    e.stopPropagation()
    // Fused tables ARE draggable now — no early return
...
export default function GastroLayout() {
  ...
  // Track drag: also remember if the table was locked before drag (for VIP auto-relock)
  ...
  const handleDragStart = useCallback((id: string, e: React.MouseEvent) => {
    ...
    if (!table) return
    // If locked, temporarily unlock for dragging — we'll re-lock on mouseUp
    const wasLocked = table.locked
    if (wasLocked) {
      setTables(prev => prev.map(t => t.id === id ? { ...t, locked: false } : t))
    }
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    setTables(prev => {
      ...
      // 1. Table-to-table magnetic snap (works for fused tables too)
      ...
      // 2. Wall magnetic snap — all 4 edges, only if not already snapped to table
      ...
      // 3. Hard boundary clamp (always applied)
      ...
      // 4. Pending fuse detection (only for non-fused tables)
      ...
      // 5. Collision check — 0 gap allowed when magnetically snapped
      const candidate = { x: finalX, y: finalY, width: moving.width, height: moving.height }
      const minPad = tableSnap.snappedTo ? 0 : minTableDist  // 0 gap when snapped
      ...
      // 6. Wall distance warning (only when not wall-snapped)
      ...
      // Block move if hard collision (not snapped)
  ...
  const handleMouseUp = useCallback(() => {
    ...
    // Auto-relock if the table was locked before drag (VIP behavior)
      ...
      <div className="flex gap-4 flex-1 min-h-0">
        ...
        <div className="flex flex-col gap-2 flex-shrink-0 w-44 overflow-y-auto">
          ...
          <div className="card-gastro p-3">
            ...
            <div className="mt-3 pt-2 border-t border-gastro-border">
              <p className="text-xs text-gastro-subtle leading-relaxed">
                🔒 <strong className="text-gastro-text">VIP:</strong> Presiona el candado para mover. Se bloquea al soltar.
              </p>
              <p className="text-xs text-gastro-subtle leading-relaxed mt-1">
                🔗 <strong className="text-gastro-text">Fusión:</strong> Arrastra una mesa sobre otra para unirlas. Usa "Desacoplar" para separar.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 rounded-2xl overflow-hidden relative" style={{ background: '#0d0d18', border: '1px solid #2a2a3d' }}>
          ...
          >
            ...
            {/* Wall distance inset guide during drag */}
            {dragging && spacePath.points.length > 2 && (
              <polygon points={spacePolygon} fill="none"
