  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Visitas este mes', value: '4.284', color: '#38bdf8', icon: Globe },
          { label: 'Pedidos online', value: '312', color: '#9E7FFF', icon: ShoppingCart },
      ...
      <div className="grid lg:grid-cols-4 gap-6">
            ...
            <div className="space-y-2">
              {WEBSITE_SECTIONS.map(section => (
                <div key={section.id}
                  ...
                  style={activeSection === section.id ? { background: 'rgba(158,127,255,0.12)' } : {}}>
                  ...
                  <div className={`w-8 h-4 rounded-full transition-all ${section.active ? '' : ''}`}
                    ...
                    <div className={`w-3 h-3 rounded-full mt-0.5 transition-all ${section.active ? 'ml-4.5' : 'ml-0.5'}`}
                      style={{ background: section.active ? '#10b981' : '#8888aa', marginLeft: section.active ? '18px' : '2px' }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="lg:col-span-3 card-gastro">
          ...
          <div className={`mx-auto rounded-2xl overflow-hidden transition-all duration-300 ${activeView === 'mobile' ? 'max-w-xs' : 'w-full'}`}
            style={{ background: '#0a0a0f', border: '1px solid #2a2a3d' }}>
            {/* Browser bar */}
            <div className="flex items-center gap-2 px-4 py-2.5" style={{ background: '#1a1a26', borderBottom: '1px solid #2a2a3d' }}>
            ...
            {/* Info bar */}
            <div className="flex items-center justify-around p-4" style={{ background: '#1a1a26', borderTop: '1px solid #2a2a3d' }}>
              {[
                { icon: MapPin, text: 'Palermo, CABA' },
                { icon: Clock, text: 'Mar-Dom 12-24h' },
                { icon: Phone, text: '+54 11 4444-5555' },
