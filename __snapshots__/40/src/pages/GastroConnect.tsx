...
const CATEGORY_META: Record<IntegrationCategory, { label: string; icon: typeof Truck; color: string }> = {
  ...
  pos: { label: 'POS & Hardware', icon: Monitor, color: '#38bdf8' },
  marketing: { label: 'Marketing', icon: Megaphone, color: '#f472b6' },
  accounting: { label: 'Contabilidad', icon: Calculator, color: '#9E7FFF' },
  reservations: { label: 'Reservas', icon: Calendar, color: '#38bdf8' },
}
...
const INTEGRATIONS: Integration[] = [
  ...
  },
  {
    ...
    category: 'delivery', status: 'disconnected', color: '#8888aa',
  ...
  },
  {
    ...
    category: 'payments', status: 'disconnected', color: '#8888aa',
  ...
  },
  {
    ...
    category: 'marketing', status: 'disconnected', color: '#8888aa',
  ...
  },
  {
    ...
    category: 'reservations', status: 'disconnected', color: '#8888aa',
...
const WEBHOOKS: WebhookEndpoint[] = [
  { id: 'w1', url: 'https://api.osteriamoderna.com/webhooks/gastro360', events: ['order.created', 'order.updated', 'payment.completed'], status: 'active', lastDelivery: 'Hace 45 seg', successRate: 99.2 },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl p-6 relative overflow-hidden"
        style={{ background: 'linear-gradient(135deg, rgba(136,136,170,0.12) 0%, rgba(158,127,255,0.08) 100%)', border: '1px solid rgba(136,136,170,0.25)' }}>
        <div className="absolute top-0 right-0 w-56 h-56 opacity-15"
          style={{ background: 'radial-gradient(circle, #8888aa 0%, transparent 70%)', transform: 'translate(30%, -30%)' }} />
        <div className="relative z-10 flex flex-col md:flex-row md:items-center gap-4">
          <div className="flex items-center gap-4 flex-1">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0"
              style={{ background: 'linear-gradient(135deg, #8888aa, #9E7FFF)', boxShadow: '0 0 30px rgba(136,136,170,0.35)' }}>
          ...
          <div className="grid grid-cols-3 gap-4 md:gap-6 text-center">
            {[
              ...
              <div key={stat.label}>
                <div className="text-xl font-black" style={{ color: '#8888aa' }}>{stat.value}</div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'API keys activas', value: API_KEYS.filter(k => k.active).length, color: '#9E7FFF', icon: Key },
          { label: 'Uptime sync', value: '99.7%', color: '#38bdf8', icon: Activity },
        ].map(stat => {
                ...
                <div key={integration.id} className="card-gastro group">
                  <div className="flex items-start gap-3 mb-3">
                    <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
                      ...
                      <Plug size={20} style={{ color: integration.status === 'connected' ? '#10b981' : integration.status === 'error' ? '#ef4444' : '#8888aa' }} />
                    </div>
        ...
        <div className="space-y-4">
              ...
              <div key={key.id} className="card-gastro">
                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                    style={{ background: key.active ? 'rgba(158,127,255,0.12)' : 'rgba(255,255,255,0.04)' }}>
                    <Key size={18} style={{ color: key.active ? '#9E7FFF' : '#8888aa' }} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="card-gastro">
            ...
            <pre className="p-4 rounded-xl text-xs overflow-x-auto leading-relaxed"
              style={{ background: '#0d0d18', border: '1px solid #2a2a3d', color: '#8888aa' }}>
  ...
  -H "Content-Type: application/json"`}
      ...
      {activeTab === 'logs' && (
        <div className="card-gastro overflow-hidden p-0">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {ACTIVITY_LOGS.map(log => (
                  <tr key={log.id} className="hover:bg-white/[0.02] transition-colors" style={{ borderBottom: '1px solid #2a2a3d' }}>
          ...
          {/* Mobile cards */}
          <div className="md:hidden divide-y" style={{ borderColor: '#2a2a3d' }}>
            {ACTIVITY_LOGS.map(log => (
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4"
          ...
          <div className="rounded-2xl p-6 w-full max-w-md shadow-2xl animate-slide-down"
            ...
            <div className="flex items-center gap-3 p-4 rounded-xl mb-5"
              style={{ background: 'rgba(158,127,255,0.08)', border: '1px solid rgba(158,127,255,0.15)' }}>
              <div className="w-10 h-10 rounded-xl flex items-center justify-center"
                style={{ background: 'rgba(158,127,255,0.15)' }}>
