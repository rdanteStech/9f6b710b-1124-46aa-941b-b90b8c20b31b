...
function NewClientModal({ onClose, onSave }: { onClose: () => void; onSave: (client: Client) => void }) {
  ...
  const handleSave = () => {
    ...
    if (!trimmedName || !email.trim() || !phone.trim()) return
    const initials = trimmedName.split(' ').slice(0, 2).map(n => n[0]?.toUpperCase() ?? '').join('')
    ...
    onSave({
      ...
      lastVisit: today,
      firstVisit: today,
      location: location.trim() || 'Santiago',
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.75)' }}>
      <div className="w-full max-w-md rounded-2xl animate-scale-in" style={{ background: '#0F1628', border: '1px solid #1A2540' }}>
        <div className="px-6 py-4 border-b flex items-center justify-between" style={{ borderColor: '#1A2540' }}>
          <h2 className="text-sm font-bold text-gastro-text">Nuevo cliente</h2>
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-white/5 text-gastro-muted">
            <X size={16} />
          </button>
        </div>
        <div className="p-6 space-y-4">
          {[
            { label: 'Nombre completo', value: name, set: setName, placeholder: 'Ej. María González' },
            { label: 'Email', value: email, set: setEmail, placeholder: 'maria@email.cl' },
            { label: 'Teléfono', value: phone, set: setPhone, placeholder: '+56 9 1234 5678' },
            { label: 'Comuna', value: location, set: setLocation, placeholder: 'Providencia, Santiago' },
          ].map(field => (
            <div key={field.label}>
              <label className="text-xs font-semibold text-gastro-subtle mb-1.5 block">{field.label}</label>
              <input
                value={field.value}
        ...
        <div className="px-6 py-4 border-t flex gap-2" style={{ borderColor: '#1A2540' }}>
          <button onClick={onClose} className="btn-secondary flex-1 text-xs py-2.5">Cancelar</button>
          <button
            onClick={handleSave}
...
export default function GastroClients() {
  ...
  const handleAddNote = (clientId: string, text: string) => {
    const today = new Date().toISOString().slice(0, 10)
    const note = { id: `n${Date.now()}`, date: today, author: 'Usuario actual', text }
    setClients(prev => prev.map(c => (c.id === clientId ? { ...c, notes: [note, ...c.notes] } : c)))
    setSelectedClient(prev => (prev?.id === clientId ? { ...prev, notes: [note, ...prev.notes] } : prev))
  }
  ...
  const stats = useMemo(() => ({
    ...
    avgVisits: clients.length ? Math.round(clients.reduce((s, c) => s + c.totalVisits, 0) / clients.length) : 0,
  }), [clients])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <button onClick={() => setShowNewClient(true)} className="btn-primary text-xs px-4 py-2.5 flex items-center gap-1.5">
      ...
      <div className="card-gastro">
        ...
        <div className="grid md:grid-cols-3 gap-3">
          {CLIENT_AI_INSIGHTS.map(insight => {
            const targetClient = insight.clients?.[0] ? clients.find(c => c.id === insight.clients![0]) : null
            return (
              <div key={insight.id} className="p-4 rounded-xl hover:bg-white/5 transition-colors"
                ...
                <button
                  onClick={() => targetClient && setSelectedClient(targetClient)}
        ...
        <div className="lg:col-span-2 card-gastro">
          <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setShowNewClient(true)} className="btn-primary text-xs px-3 py-2 flex items-center gap-1.5">
      ...
      {showNewClient && (
        <NewClientModal onClose={() => setShowNewClient(false)} onSave={handleNewClient} />
      )}
    </div>
  )
}
