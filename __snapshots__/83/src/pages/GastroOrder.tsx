...
import {
  ...
  ChefHat, GlassWater, Share2, Check,
  BadgeCheck, ClipboardList, Tag, Smartphone,
} from 'lucide-react'
import { formatTime, DEMO_RESTAURANT_NAME } from '../lib/locale'

// ─── Types ────────────────────────────────────────────────────────────────────

interface MenuItem {
  id: string; name: string; description: string; price: number
  category: string; image: string; tags: string[]
  popular?: boolean; destination: 'kitchen' | 'bar'; shareable?: boolean
}

interface CartItem {
  menuItemId: string; name: string; price: number; qty: number
  destination: 'kitchen' | 'bar'; shared?: boolean; sharedWith?: string[]
  assignedTo?: string | 'all'; addedByWaiter?: boolean
}

interface OrderRound {
  id: string; items: CartItem[]; sentAt: string
  destination: 'kitchen' | 'bar' | 'both'
  addedByWaiter?: boolean; waiterId?: string
}

interface Diner {
  id: string; name: string; emoji: string
  cart: CartItem[]; rounds: OrderRound[]
  paid: boolean; paymentMethod?: string; joinedAt: string
}

interface Waiter {
  id: string; name: string; emoji: string; role: 'waiter'; color: string
}

interface TableSession {
  id: string; tableNumber: string; tableLabel: string; zone: string
  status: 'ordering' | 'sent' | 'paying' | 'closed'
  diners: Diner[]; waiters: Waiter[]; tableCapacity: number
  openedAt: string; sentAt?: string; openedByFirstScan: boolean
}

interface TableConfig {
  number: string; label: string; zone: string; capacity: number
}

// ─── Mock Data ────────────────────────────────────────────────────────────────

const MENU_ITEMS: MenuItem[] = [
  { id: 'm1', name: 'Burrata con Tomates', description: 'Burrata fresca, tomates cherry asados, albahaca y aceite de oliva extra virgen', price: 7000, category: 'Entradas', image: 'https://images.pexels.com/photos/1640777/pexels-photo-1640777.jpeg?w=400', tags: ['vegetariano'], popular: true, destination: 'kitchen', shareable: true },
  { id: 'm2', name: 'Ceviche Clásico', description: 'Corvina marinada en limón, cebolla morada, ají amarillo y choclo', price: 8500, category: 'Entradas', image: 'https://images.pexels.com/photos/2097090/pexels-photo-2097090.jpeg?w=400', tags: ['sin gluten'], destination: 'kitchen' },
  { id: 'm3', name: 'Tabla de Embutidos', description: 'Selección de jamones ibéricos, quesos curados y encurtidos artesanales', price: 12000, category: 'Entradas', image: 'https://images.pexels.com/photos/1640772/pexels-photo-1640772.jpeg?w=400', tags: ['para compartir'], destination: 'kitchen', shareable: true },
  { id: 'm4', name: 'Risotto de Hongos', description: 'Arroz arbóreo, mix de hongos silvestres, parmesano 24 meses y trufa negra', price: 10500, category: 'Principales', image: 'https://images.pexels.com/photos/1279330/pexels-photo-1279330.jpeg?w=400', tags: ['vegetariano'], popular: true, destination: 'kitchen' },
  { id: 'm5', name: 'Lomo a la Pimienta', description: 'Lomo de res 300g, salsa de pimienta verde, papas rosti y espárragos', price: 18000, category: 'Principales', image: 'https://images.pexels.com/photos/769289/pexels-photo-769289.jpeg?w=400', tags: ['sin gluten'], destination: 'kitchen' },
  { id: 'm6', name: 'Pasta Carbonara', description: 'Spaghetti artesanal, guanciale, yema de huevo, pecorino y pimienta negra', price: 9800, category: 'Principales', image: 'https://images.pexels.com/photos/1437267/pexels-photo-1437267.jpeg?w=400', tags: [], destination: 'kitchen' },
  { id: 'm7', name: 'Salmón Grillado', description: 'Salmón atlántico, quinoa tricolor, salsa de maracuyá y vegetales de estación', price: 16500, category: 'Principales', image: 'https://images.pexels.com/photos/3655916/pexels-photo-3655916.jpeg?w=400', tags: ['sin gluten', 'saludable'], destination: 'kitchen' },
  { id: 'm8', name: 'Agua Mineral', description: 'Agua mineral sin gas o con gas 500ml', price: 1200, category: 'Bebidas', image: 'https://images.pexels.com/photos/1000084/pexels-photo-1000084.jpeg?w=400', tags: [], destination: 'bar' },
  { id: 'm9', name: 'Vino Tinto Copa', description: 'Carmenère Reserva, Valle de Colchagua. Copa 150ml', price: 4500, category: 'Bebidas', image: 'https://images.pexels.com/photos/2912108/pexels-photo-2912108.jpeg?w=400', tags: [], destination: 'bar' },
  { id: 'm9b', name: 'Vino Tinto Botella', description: 'Carmenère Reserva, Valle de Colchagua. Botella 750ml — ideal para compartir', price: 12000, category: 'Bebidas', image: 'https://images.pexels.com/photos/2912108/pexels-photo-2912108.jpeg?w=400', tags: ['para compartir'], destination: 'bar', shareable: true },
  { id: 'm10', name: 'Limonada Artesanal', description: 'Limón exprimido, menta fresca, jengibre y agua con gas', price: 3500, category: 'Bebidas', image: 'https://images.pexels.com/photos/2109099/pexels-photo-2109099.jpeg?w=400', tags: ['sin alcohol'], popular: true, destination: 'bar' },
  { id: 'm11', name: 'Tiramisú Clásico', description: 'Mascarpone, café espresso, savoiardi y cacao en polvo', price: 4500, category: 'Postres', image: 'https://images.pexels.com/photos/6880219/pexels-photo-6880219.jpeg?w=400', tags: ['vegetariano'], popular: true, destination: 'kitchen' },
  { id: 'm12', name: 'Coulant de Chocolate', description: 'Bizcocho de chocolate 70%, centro fundente y helado de vainilla', price: 5200, category: 'Postres', image: 'https://images.pexels.com/photos/3026804/pexels-photo-3026804.jpeg?w=400', tags: ['vegetariano'], destination: 'kitchen' },
]
...
const MOCK_WAITERS: Waiter[] = [
  ...
  { id: 'w2', name: 'Valentina', emoji: '👩‍🍳', role: 'waiter', color: '#60A5FA' },
  { id: 'w3', name: 'Rodrigo', emoji: '🧑‍🍳', role: 'waiter', color: '#3B82F6' },
]
...
function DestBadge({ dest, small }: { dest: 'kitchen' | 'bar'; small?: boolean }) {
  ...
  if (dest === 'kitchen') return (
    <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-lg font-semibold"
      style={{ background: 'rgba(37,99,235,0.15)', color: '#2563EB', fontSize: small ? '9px' : '10px' }}>
      <ChefHat size={sz} /> Cocina
    </span>
  )
  return (
    <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-lg font-semibold"
      style={{ background: 'rgba(59,130,246,0.15)', color: '#3B82F6', fontSize: small ? '9px' : '10px' }}>
...
function PhoneShell({
  accentColor = '#1A2540', glowColor, label, labelDot, onClose, children,
}: {
  accentColor?: string; glowColor?: string; label: React.ReactNode
  labelDot?: React.ReactNode; onClose: () => void; children: React.ReactNode
}) {
    ...
    <FullscreenOverlay>
      {glowColor && (
        <div style={{
          ...
          background: `radial-gradient(ellipse 600px 500px at 50% 50%, ${glowColor}18 0%, transparent 70%)`,
        }} />
      )}
      <div style={{ position: 'absolute', top: 24, left: 24, display: 'flex', alignItems: 'center', gap: 8, zIndex: 10 }}>
        {labelDot}
        <span style={{ fontSize: 12, fontWeight: 600, color: '#6666aa' }}>{label}</span>
      </div>
      <button onClick={onClose}
        style={{
          position: 'absolute', top: 24, right: 24, width: 40, height: 40, borderRadius: 16,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          border: '1px solid rgba(255,255,255,0.12)', color: '#6666aa',
          background: 'rgba(255,255,255,0.04)', cursor: 'pointer', zIndex: 10,
        }}
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{
          position: 'relative', flexShrink: 0, width: 375, height: 780,
          borderRadius: 48, background: '#0a0a12', border: '10px solid #0F1628',
          boxShadow: [
            `0 0 0 1px ${accentColor}`, `0 0 0 2px #0a0a12`,
            `0 0 60px ${glowColor ?? '#2563EB'}33`, '0 40px 100px rgba(0,0,0,0.9)',
          ...
          <div style={{
            ...
            zIndex: 30, width: 120, height: 32, background: '#0F1628', borderRadius: '0 0 20px 20px',
          }} />
          <div style={{
            position: 'absolute', top: 0, left: 0, right: 0, zIndex: 20,
            height: 44, display: 'flex', alignItems: 'center',
            justifyContent: 'space-between', padding: '8px 24px 0',
          }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: '#fff' }}>{time}</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <Wifi size={12} color="#fff" />
              <div style={{ display: 'flex', gap: 2, alignItems: 'flex-end' }}>
                {[3, 4, 5, 6].map(h => (
                  <div key={h} style={{ width: 4, height: h, borderRadius: 2, background: '#fff' }} />
                ))}
              </div>
            </div>
          </div>
          ...
          <div style={{
            ...
            width: 112, height: 4, borderRadius: 2, background: 'rgba(255,255,255,0.2)',
          }} />
        </div>
      </div>
    </FullscreenOverlay>
...
function WaiterSelectModal({ tableLabel, onSelect, onCancel }: {
  tableLabel: string; onSelect: (waiter: Waiter) => void; onCancel: () => void
}) {
  return (
    <FullscreenOverlay onClose={onCancel}>
      <div style={{
        position: 'absolute', inset: 0, pointerEvents: 'none',
        background: 'radial-gradient(ellipse 500px 400px at 50% 50%, rgba(245,158,11,0.08) 0%, transparent 70%)',
      }} />
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{
          ...
          background: '#141420', border: '1px solid #1A2540',
          boxShadow: '0 24px 64px rgba(0,0,0,0.8)',
        }}>
          <div style={{ padding: '20px 20px 16px', borderBottom: '1px solid #1A2540' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              <BadgeCheck size={16} color="#f59e0b" />
              <span style={{ fontWeight: 900, color: '#fff', fontSize: 15 }}>Identificación de mozo</span>
            </div>
            <p style={{ fontSize: 12, color: '#8899BB', margin: 0 }}>
              {tableLabel} · Selecciona tu perfil para asociarte a la mesa
            </p>
          </div>
            ...
            {MOCK_WAITERS.map(w => (
              <button key={w.id} onClick={() => onSelect(w)}
                style={{
                  ...
                  background: `${w.color}0d`, border: `1px solid ${w.color}33`,
                ...
                <div style={{
                  ...
                  fontSize: 24, background: `${w.color}18`, flexShrink: 0,
                }}>{w.emoji}</div>
                <div style={{ flex: 1, textAlign: 'left' }}>
                  <div style={{ fontWeight: 700, color: '#fff', fontSize: 14 }}>{w.name}</div>
                  <div style={{ fontSize: 12, color: w.color }}>Mozo · {DEMO_RESTAURANT_NAME}</div>
                </div>
                <ArrowRight size={16} color={w.color} />
              </button>
            ))}
          </div>
          <div style={{ padding: '0 16px 16px' }}>
            <button onClick={onCancel}
              style={{
                ...
                background: 'rgba(255,255,255,0.04)', border: '1px solid #1A2540', color: '#8899BB',
              }}>
              Cancelar
            </button>
          </div>
...
function ConfirmSendModal({ items, dinerName, dinerEmoji, onConfirm, onCancel, isWaiter = false }: {
  items: CartItem[]; dinerName: string; dinerEmoji: string
  onConfirm: () => void; onCancel: () => void; isWaiter?: boolean
}) {
  ...
  return (
    <div className="absolute inset-0 z-20 flex flex-col justify-end"
      ...
      <div className="rounded-t-3xl overflow-hidden" style={{ background: '#0F1628', border: '1px solid #1A2540' }}>
        <div className="p-4 border-b" style={{ borderColor: '#1A2540' }}>
          <div className="flex items-center gap-2 mb-1">
            ...
            {isWaiter && (
              <span className="text-xs px-1.5 py-0.5 rounded-lg font-semibold"
                style={{ background: 'rgba(245,158,11,0.15)', color: '#f59e0b' }}>Mozo</span>
            )}
          </div>
          <p className="text-xs" style={{ color: '#8899BB' }}>{dinerEmoji} {dinerName} · revisa antes de enviar</p>
        </div>
        <div className="p-4 space-y-3 max-h-56 overflow-y-auto">
          {kitchenItems.length > 0 && (
            <div>
              <div className="flex items-center gap-1.5 mb-2">
                <ChefHat size={12} style={{ color: '#2563EB' }} />
                <span className="text-xs font-bold uppercase tracking-wider" style={{ color: '#2563EB' }}>Cocina</span>
              </div>
              {kitchenItems.map(item => {
                const splitCount = item.shared && item.sharedWith ? item.sharedWith.length + 1 : 1
                ...
                return (
                  <div key={item.menuItemId} className="flex items-center justify-between py-1.5 border-b last:border-0"
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap flex-1">
                      ...
                      {item.shared && <span className="text-xs px-1 py-0.5 rounded" style={{ background: 'rgba(96,165,250,0.15)', color: '#60A5FA', fontSize: '9px' }}>÷{splitCount}</span>}
                      {item.addedByWaiter && item.assignedTo === 'all' && <span className="text-xs px-1 py-0.5 rounded" style={{ background: 'rgba(245,158,11,0.15)', color: '#f59e0b', fontSize: '9px' }}>÷mesa</span>}
                    </div>
          ...
          {barItems.length > 0 && (
            <div>
              <div className="flex items-center gap-1.5 mb-2">
                <GlassWater size={12} style={{ color: '#3B82F6' }} />
                <span className="text-xs font-bold uppercase tracking-wider" style={{ color: '#3B82F6' }}>Barra</span>
              </div>
              {barItems.map(item => {
                const splitCount = item.shared && item.sharedWith ? item.sharedWith.length + 1 : 1
                const myPrice = Math.round(item.price * item.qty / splitCount)
        ...
        <div className="p-4 border-t space-y-3" style={{ borderColor: '#1A2540' }}>
          {isWaiter && (
            <div className="flex items-start gap-2 p-2 rounded-xl"
              ...
              <p className="text-xs" style={{ color: '#f59e0b' }}>Los ítems "÷mesa" se distribuirán entre todos los comensales</p>
            </div>
          )}
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold" style={{ color: '#8899BB' }}>
              {isWaiter ? 'Total del pedido' : 'Tu parte de este pedido'}
            </span>
            <span className="text-lg font-black" style={{ color: isWaiter ? '#f59e0b' : '#2563EB' }}>
              ${total.toLocaleString()}
            </span>
          </div>
          <div className="flex gap-2">
            <button onClick={onCancel} className="flex-1 py-3 rounded-2xl text-xs font-bold"
              style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid #1A2540', color: '#8899BB' }}>
              Revisar
            </button>
            <button onClick={onConfirm}
              className="flex-1 py-3 rounded-2xl text-xs font-bold text-white flex items-center justify-center gap-1.5"
              style={{
                background: isWaiter ? 'linear-gradient(135deg, #f59e0b, #d97706)' : 'linear-gradient(135deg, #10b981, #059669)',
...
function ShareModal({ item, diners, currentDinerId, onConfirm, onCancel }: {
  item: MenuItem; diners: Diner[]; currentDinerId: string
  onConfirm: (sharedWith: string[]) => void; onCancel: () => void
}) {
  ...
  return (
    <div className="absolute inset-0 z-20 flex flex-col justify-end"
      ...
      <div className="rounded-t-3xl overflow-hidden" style={{ background: '#0F1628', border: '1px solid #1A2540' }}>
        <div className="p-4 border-b" style={{ borderColor: '#1A2540' }}>
          <div className="flex items-center gap-2 mb-1">
            <Share2 size={14} style={{ color: '#60A5FA' }} />
          ...
          <p className="text-xs mt-0.5" style={{ color: '#8899BB' }}>Selecciona con quién compartes el costo</p>
        </div>
        <div className="p-4 space-y-2">
            ...
            return (
              <button key={d.id}
                ...
                style={{
                  ...
                  border: `1px solid ${isSelected ? 'rgba(96,165,250,0.4)' : '#1A2540'}`,
                }}>
                <div className="w-5 h-5 rounded-md flex items-center justify-center flex-shrink-0"
                  style={{ background: isSelected ? '#60A5FA' : 'rgba(255,255,255,0.08)', border: `1px solid ${isSelected ? '#60A5FA' : '#2A3550'}` }}>
          ...
          <div className="p-3 rounded-xl mt-2"
            ...
            <div className="flex items-center justify-between text-xs">
              <span style={{ color: '#8899BB' }}>${item.price.toLocaleString()} ÷ {splitCount} {splitCount === 1 ? 'persona' : 'personas'}</span>
              <span className="font-black" style={{ color: '#60A5FA' }}>${myPrice.toLocaleString()} c/u</span>
            </div>
          </div>
        </div>
        <div className="p-4 border-t flex gap-2" style={{ borderColor: '#1A2540' }}>
          <button onClick={onCancel} className="flex-1 py-3 rounded-2xl text-xs font-bold"
            style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid #1A2540', color: '#8899BB' }}>
            Cancelar
          </button>
...
function WaiterAssignModal({ item, diners, onConfirm, onCancel }: {
  item: MenuItem; diners: Diner[]
  onConfirm: (assignedTo: string | 'all') => void; onCancel: () => void
}) {
    ...
    <div className="absolute inset-0 z-20 flex flex-col justify-end"
      ...
      <div className="rounded-t-3xl overflow-hidden" style={{ background: '#0F1628', border: '1px solid #1A2540' }}>
        <div className="p-4 border-b" style={{ borderColor: '#1A2540' }}>
          ...
          <p className="text-xs mt-0.5" style={{ color: '#8899BB' }}>¿A quién se carga este ítem?</p>
        </div>
        <div className="p-4 space-y-2 max-h-64 overflow-y-auto">
          <button onClick={() => setSelected('all')}
            ...
            style={{
              ...
              border: `1px solid ${selected === 'all' ? 'rgba(245,158,11,0.4)' : '#1A2540'}`,
            }}>
            <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0"
              style={{ background: selected === 'all' ? '#f59e0b' : 'rgba(255,255,255,0.08)', border: `1px solid ${selected === 'all' ? '#f59e0b' : '#2A3550'}` }}>
          ...
          {diners.map(d => (
            <button key={d.id} onClick={() => setSelected(d.id)}
              className="w-full flex items-center gap-3 p-3 rounded-xl transition-all"
              style={{
                background: selected === d.id ? 'rgba(59,130,246,0.1)' : 'rgba(255,255,255,0.03)',
                border: `1px solid ${selected === d.id ? 'rgba(59,130,246,0.4)' : '#1A2540'}`,
              }}>
              <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0"
                style={{ background: selected === d.id ? '#3B82F6' : 'rgba(255,255,255,0.08)', border: `1px solid ${selected === d.id ? '#3B82F6' : '#2A3550'}` }}>
                {selected === d.id && <Check size={11} className="text-white" />}
              </div>
              <span className="text-lg">{d.emoji}</span>
              <div className="flex-1 text-left">
                <div className="text-sm font-semibold text-gastro-text">{d.name}</div>
                <div className="text-xs" style={{ color: '#8899BB' }}>Cargo individual · ${item.price.toLocaleString()}</div>
              </div>
            </button>
          ))}
          {diners.length === 0 && (
...
function WaiterPhoneKiosk({ session, waiter, onSendWaiterRound, onClose }: {
  session: TableSession; waiter: Waiter
  onSendWaiterRound: (items: CartItem[]) => void; onClose: () => void
}) {
  ...
  return (
    <PhoneShell
      accentColor={`${waiter.color}66`} glowColor={waiter.color}
      label={<>{waiter.emoji} {waiter.name} · {session.tableLabel} · <span style={{ color: waiter.color }}>Mozo</span></>}
      labelDot={<div style={{ width: 8, height: 8, borderRadius: '50%', background: waiter.color, boxShadow: `0 0 8px ${waiter.color}` }} />}
      onClose={onClose}>

      {screen === 'overview' && (
        <div className="flex-1 flex flex-col overflow-hidden" style={{ background: '#0A0F1A' }}>
          <div className="flex-shrink-0 p-4"
            style={{ background: `linear-gradient(135deg, ${waiter.color}22, transparent)`, borderBottom: '1px solid #152035' }}>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs px-1.5 py-0.5 rounded-lg font-semibold"
                    style={{ background: `${waiter.color}22`, color: waiter.color }}>Mozo</span>
                </div>
                <div className="text-xs mt-0.5" style={{ color: '#8899BB' }}>{session.tableLabel} · {session.zone}</div>
              </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {session.waiters.length > 0 && (
              <div className="p-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid #1A2540' }}>
                <p className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color: '#8899BB' }}>Mozos en esta mesa</p>
                <div className="flex gap-2 flex-wrap">
                  {session.waiters.map(w => (
                    <div key={w.id} className="flex items-center gap-1.5 px-2 py-1 rounded-lg"
                      ...
                      {w.id === waiter.id && <span className="text-xs" style={{ color: w.color, opacity: 0.7 }}>(vos)</span>}
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div>
              <p className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color: '#8899BB' }}>
                Comensales · {session.diners.length} de {session.tableCapacity}
              </p>
              {session.diners.length === 0 ? (
                <div className="p-3 rounded-xl text-xs text-center"
                  style={{ background: 'rgba(255,255,255,0.02)', border: '1px dashed #1A2540', color: '#6666aa' }}>
                    ...
                    return (
                      <div key={d.id} className="flex items-center gap-3 p-3 rounded-xl"
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs" style={{ color: '#8899BB' }}>{d.rounds.length} ronda{d.rounds.length !== 1 ? 's' : ''}</div>
                        </div>
                        <div className="text-right">
                          <div className="text-sm font-black" style={{ color: dTotal > 0 ? '#2563EB' : '#4A5A7A' }}>${dTotal.toLocaleString()}</div>
          ...
          <div className="p-4 border-t flex-shrink-0" style={{ borderColor: '#152035' }}>
            <button onClick={() => setScreen('menu')}
              ...
              <ClipboardList size={15} /> Registrar pedido de la mesa
            </button>
          </div>
        </div>
      )}
        ...
        <div className="flex-1 flex flex-col overflow-hidden" style={{ background: '#0A0F1A' }}>
          <div className="flex items-center justify-between px-4 py-3 flex-shrink-0"
            ...
            <button onClick={() => setScreen('overview')}
              className="flex items-center gap-1 text-xs font-semibold" style={{ color: waiter.color }}>
            ...
            {CATEGORIES.map(cat => (
              <button key={cat} onClick={() => setActiveCategory(cat)}
                ...
                style={{
                  ...
                  border: `1px solid ${activeCategory === cat ? `${waiter.color}55` : '#152035'}`,
                  color: activeCategory === cat ? waiter.color : '#8899BB',
                }}>
            ...
            {filteredItems.map(item => {
              ...
              return (
                <div key={item.id} className="rounded-2xl overflow-hidden"
                  style={{ background: '#141420', border: '1px solid #152035' }}>
                  <div className="flex gap-3 p-3">
                    <div className="relative flex-shrink-0">
                      <img src={item.image} alt={item.name} className="rounded-xl object-cover" style={{ width: 72, height: 72 }} />
                      <div className="absolute bottom-1 left-1"><DestBadge dest={item.destination} small /></div>
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs leading-relaxed line-clamp-2 mb-2" style={{ color: '#6666aa' }}>{item.description}</p>
                      <div className="flex items-center justify-between">
                        {totalQty > 0 && (
                          <div className="flex items-center gap-1 text-xs font-semibold" style={{ color: waiter.color }}>
                            <Check size={10} />{totalQty} en pedido
                          </div>
                        )}
                        <button onClick={() => setAssignItem(item)}
                          className="ml-auto flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-bold"
                          style={{ background: `${waiter.color}18`, border: `1px solid ${waiter.color}44`, color: waiter.color }}>
                          ...
                          {inCart.map((c, ci) => (
                            <div key={ci} className="flex items-center justify-between text-xs px-2 py-1 rounded-lg"
                              ...
                              <span style={{ color: '#8899BB' }}>
                                {c.qty}× {c.assignedTo === 'all' ? '÷ toda la mesa' : `→ ${session.diners.find(d => d.id === c.assignedTo)?.name ?? 'comensal'}`}
                              </span>
          ...
          {waiterCart.length > 0 && (
            <div className="p-4 border-t flex-shrink-0" style={{ borderColor: '#152035' }}>
              <button onClick={() => setScreen('cart')}
                ...
                style={{ background: `linear-gradient(135deg, ${waiter.color}, ${waiter.color}cc)`, boxShadow: `0 4px 20px ${waiter.color}44` }}>
                <ShoppingCart size={15} /> Ver pedido · ${cartTotal.toLocaleString()}
              </button>
            </div>
          )}
        </div>
      ...
      {screen === 'cart' && (
        <div className="flex-1 flex flex-col overflow-hidden" style={{ background: '#0A0F1A' }}>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {waiterCart.length === 0 ? (
              <div className="text-center py-12">
                <ClipboardList size={28} className="mx-auto mb-3" style={{ color: '#2A3550' }} />
                <p className="text-sm" style={{ color: '#8899BB' }}>No hay ítems en el pedido</p>
              ...
              <>
                <div className="p-3 rounded-xl" style={{ background: `${waiter.color}0a`, border: `1px solid ${waiter.color}22` }}>
                  <div className="flex items-center gap-1.5 mb-2">
                    <Tag size={11} style={{ color: waiter.color }} />
                    <span className="text-xs font-bold uppercase tracking-wider" style={{ color: waiter.color }}>Distribución del pedido</span>
                  </div>
                  {waiterCart.map((item, idx) => {
                    ...
                    const splitPrice = item.assignedTo === 'all' && session.diners.length > 0
                      ? Math.round(item.price / session.diners.length) : item.price
                    return (
          ...
          <div className="p-4 border-t space-y-2 flex-shrink-0" style={{ borderColor: '#152035' }}>
            {waiterCart.length > 0 && (
              <button onClick={() => setShowConfirm(true)}
                className="w-full py-4 rounded-2xl font-bold text-white text-sm flex items-center justify-center gap-2"
                style={{ background: `linear-gradient(135deg, ${waiter.color}, ${waiter.color}cc)`, boxShadow: `0 4px 20px ${waiter.color}44` }}>
                <Send size={15} /> Enviar a cocina / barra
              </button>
            )}
            <button onClick={() => setScreen('menu')}
              className="w-full py-3 rounded-2xl font-bold text-sm flex items-center justify-center gap-2"
              style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid #1A2540', color: '#8899BB' }}>
      ...
      {assignItem && (
        <WaiterAssignModal item={assignItem} diners={session.diners}
          onConfirm={(assignedTo) => handleAssignConfirm(assignItem, assignedTo)}
          onCancel={() => setAssignItem(null)} />
      )}
      {showConfirm && (
        <ConfirmSendModal items={waiterCart} dinerName={waiter.name} dinerEmoji={waiter.emoji}
          onConfirm={handleSendConfirmed} onCancel={() => setShowConfirm(false)} isWaiter />
      )}
    </PhoneShell>
  )
}
...
function PhoneKiosk({ session, currentDinerId, onJoin, onAddItem, onRemoveItem, onSendRound, onRegisterPayment, onClose }: {
  session: TableSession; currentDinerId: string | null
  ...
  return (
    <PhoneShell accentColor="#1A2540" glowColor="#2563EB"
      label={<>Vista del cliente · <span style={{ color: '#2563EB' }}>{session.tableLabel}</span></>}
      labelDot={<div style={{ width: 8, height: 8, borderRadius: '50%', background: '#10b981', boxShadow: '0 0 8px #10b981' }} />}
      onClose={onClose}>

      {screen === 'join' && (
        <div className="flex-1 flex flex-col overflow-y-auto" style={{ background: '#0A0F1A' }}>
          <div className="relative overflow-hidden flex-shrink-0"
            style={{ background: 'linear-gradient(160deg, #172554 0%, #0d1a2e 100%)', paddingTop: 24, paddingBottom: 32 }}>
            <div className="absolute inset-0 opacity-30"
              style={{ backgroundImage: 'radial-gradient(circle at 30% 60%, #2563EB 0%, transparent 50%), radial-gradient(circle at 70% 40%, #3B82F6 0%, transparent 50%)' }} />
            <div className="relative text-center px-6">
              ...
              <p className="text-sm" style={{ color: '#a0a0c0' }}>{session.zone} · {DEMO_RESTAURANT_NAME}</p>
              {session.waiters.length > 0 && (
                <div className="flex items-center justify-center gap-1 mt-2">
                  <BadgeCheck size={11} style={{ color: '#f59e0b' }} />
                  <span className="text-xs" style={{ color: '#f59e0b' }}>Mozo {session.waiters[0].name} asignado</span>
                </div>
              )}
              {session.diners.length > 0 && (
          ...
          <div className="flex-1 p-5 space-y-4">
            {session.diners.length === 0 && (
              <div className="flex items-start gap-2 p-3 rounded-2xl"
                ...
                <p className="text-xs" style={{ color: '#7dd3fc' }}>Sos el primero. La sesión se abre automáticamente al unirte.</p>
              </div>
            )}
            {session.diners.length > 0 && (
              <div className="flex -space-x-2 justify-center">
                {session.diners.slice(0, 6).map(d => (
                  <div key={d.id} className="w-9 h-9 rounded-full flex items-center justify-center text-base"
                    style={{ background: '#152035', border: '2px solid #0A0F1A' }}>{d.emoji}</div>
                ))}
              </div>
            )}
            <div>
              <label className="block text-xs font-semibold mb-2 uppercase tracking-wider" style={{ color: '#8899BB' }}>
                ¿Cómo te llamás?
              </label>
            ...
            <button onClick={() => joinName.trim() && onJoin(joinName.trim())} disabled={!joinName.trim()}
            ...
            <p className="text-xs text-center" style={{ color: '#6666aa' }}>
              Puedes ordenar, pagar tu parte y ver el resumen
            </p>
          </div>
        </div>
      )}
        ...
        <div className="flex-1 flex flex-col overflow-hidden" style={{ background: '#0A0F1A' }}>
            ...
            <div className="flex items-center gap-2">
              {myRounds.length > 0 && (
                <div className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-semibold"
                  ...
                  <CheckCircle size={10} />{myRounds.length} enviado{myRounds.length > 1 ? 's' : ''}
                </div>
              )}
              <button onClick={() => setScreen('cart')}
                className="relative flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold"
          ...
          {sentFeedback && (
            <div className="mx-4 mt-2 flex items-center gap-2 p-3 rounded-xl flex-shrink-0"
              ...
              <span className="text-xs font-semibold text-success">¡Pedido enviado! Puedes seguir agregando</span>
            </div>
          )}
            ...
            {CATEGORIES.map(cat => (
              <button key={cat} onClick={() => setActiveCategory(cat)}
                ...
                style={{
                  ...
                  border: `1px solid ${activeCategory === cat ? 'rgba(37,99,235,0.5)' : '#152035'}`,
                  color: activeCategory === cat ? '#2563EB' : '#8899BB',
                }}>
                {CATEGORY_ICONS[cat]}{cat}
              </button>
            ))}
                ...
                <div key={item.id} className="rounded-2xl overflow-hidden"
                  ...
                  <div className="flex gap-3 p-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs leading-relaxed line-clamp-2 mb-1.5" style={{ color: '#6666aa' }}>{item.description}</p>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1">
                          {item.shareable && (
                            <button onClick={() => setShareItem(item)}
                              className="flex items-center gap-1 px-1.5 py-0.5 rounded-lg text-xs font-semibold"
                              style={{ background: 'rgba(96,165,250,0.12)', color: '#60A5FA', fontSize: '10px' }}>
          ...
          {myCart.length > 0 && (
            <div className="p-4 border-t flex-shrink-0" style={{ borderColor: '#152035' }}>
              <button onClick={() => setScreen('cart')}
                ...
                <ShoppingCart size={15} /> Ver pedido · ${myCartTotal.toLocaleString()}
              </button>
            </div>
          )}
        </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden" style={{ background: '#0A0F1A' }}>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {myCart.length === 0 && myRounds.length === 0 ? (
              <div className="text-center py-12">
                <ShoppingCart size={28} className="mx-auto mb-3" style={{ color: '#2A3550' }} />
                <p className="text-sm" style={{ color: '#8899BB' }}>Tu carrito está vacío</p>
                <button onClick={() => setScreen('menu')} className="mt-2 text-xs font-semibold" style={{ color: '#2563EB' }}>
                  Ver menú →
                </button>
              </div>
            ) : (
              <>
                      ...
                      return (
                        <div key={item.menuItemId} className="flex items-center justify-between py-1.5 pl-5"
                          ...
                          <div>
                            ...
                            {isShared && <span className="text-xs ml-1" style={{ color: '#60A5FA' }}>÷{splitCount}</span>}
                ...
                {myCart.length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs font-bold uppercase tracking-wider" style={{ color: '#f59e0b' }}>Pendiente de envío</span>
                    </div>
                    {myCart.map(item => {
                      const isShared = item.shared && item.sharedWith
                      ...
                      return (
                        <div key={item.menuItemId} className="flex items-center gap-3 p-3 rounded-xl mb-2"
                          style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid #152035' }}>
                          <div className="flex-1">
                            <div className="flex items-center gap-1.5 mb-0.5">
                              ...
                              {isShared && (
                                <span className="text-xs px-1 py-0.5 rounded"
                                  style={{ background: 'rgba(96,165,250,0.15)', color: '#60A5FA', fontSize: '9px' }}>
                                  ÷{splitCount}
                                </span>
                              )}
                            </div>
                            <span className="text-xs" style={{ color: '#8899BB' }}>${myPrice.toLocaleString()} tu parte</span>
                          </div>
                          <div className="flex items-center gap-1.5">
                ...
                <div className="p-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid #152035' }}>
                  <p className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color: '#8899BB' }}>Resumen mesa</p>
                  {session.diners.map(d => {
                    ...
                    return (
                      <div key={d.id} className="flex items-center justify-between py-1">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-xs" style={{ color: d.id === currentDinerId ? '#2563EB' : '#8899BB' }}>
                  ...
                  <div className="border-t mt-2 pt-2 flex items-center justify-between" style={{ borderColor: '#152035' }}>
                    <span className="text-xs font-bold text-white">Total mesa</span>
                    <span className="text-sm font-black" style={{ color: '#2563EB' }}>${sessionTotal.toLocaleString()}</span>
                  </div>
                </div>
              </>
            )}
        ...
        <div className="flex-1 flex flex-col overflow-hidden" style={{ background: '#0A0F1A' }}>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            <div className="p-4 rounded-2xl text-center"
              style={{ background: 'linear-gradient(135deg, rgba(37,99,235,0.12), rgba(59,130,246,0.08))', border: '1px solid rgba(37,99,235,0.2)' }}>
              <div className="text-xs mb-1" style={{ color: '#8899BB' }}>Total de la mesa</div>
              <div className="text-3xl font-black text-white">${sessionTotal.toLocaleString()}</div>
              <div className="text-xs mt-1" style={{ color: '#8899BB' }}>{session.diners.length} comensales</div>
            </div>

            {payStep === 'split' && (
              <>
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: '#8899BB' }}>¿Cómo quieres pagar?</p>
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      <button key={id}
                        ...
                        style={{
                          background: splitMode === id ? 'rgba(37,99,235,0.15)' : 'rgba(255,255,255,0.04)',
                          border: `1px solid ${splitMode === id ? 'rgba(37,99,235,0.4)' : '#152035'}`,
                          color: splitMode === id ? '#2563EB' : '#8899BB',
                        }}>
                        <Icon size={15} />
                ...
                {splitMode === 'individual' && (
                  <div className="flex items-center justify-between p-3 rounded-xl font-bold"
                    ...
                    <span className="text-lg" style={{ color: '#2563EB' }}>${myAllTimeTotal.toLocaleString()}</span>
                  </div>
                )}
                {splitMode === 'equal' && (
                      ...
                      return (
                        <button key={d.id}
                          ...
                          style={{
                            ...
                            border: `1px solid ${isSelected ? 'rgba(37,99,235,0.4)' : '#152035'}`,
                          }}>
                          ...
                          {d.id === currentDinerId && <span className="text-xs" style={{ color: '#2563EB' }}>(vos)</span>}
                        </button>
                      )
                    })}
                  </div>
            ...
            {payStep === 'method' && (
              <div className="space-y-2">
                <p className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: '#8899BB' }}>Método de pago</p>
                ...
                <button onClick={() => {
                  ...
                  className="w-full flex items-center gap-3 p-4 rounded-2xl"
                  style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid #152035', color: '#8899BB' }}>
            ...
            {payStep === 'processing' && (
              <div className="text-center py-10">
                ...
                <p className="text-xs mb-6" style={{ color: '#8899BB' }}>Redirigiendo a WebpayPlus</p>
                <button onClick={() => {
                  onRegisterPayment(
                    splitMode === 'individual' ? [currentDinerId!]
                      : splitMode === 'equal' ? unpaidDiners.map(d => d.id)
            ...
            {payStep === 'done' && (
              <div className="text-center py-10">
                ...
                <p className="text-sm mb-1" style={{ color: '#8899BB' }}>Tu boleta fue enviada por email</p>
                <button onClick={onClose}
      ...
      {showConfirm && screen === 'cart' && currentDiner && (
        <ConfirmSendModal items={myCart} dinerName={currentDiner.name} dinerEmoji={currentDiner.emoji}
          onConfirm={handleSendConfirmed} onCancel={() => setShowConfirm(false)} />
      )}
      {shareItem && currentDinerId && (
        <ShareModal item={shareItem} diners={session.diners} currentDinerId={currentDinerId}
          onConfirm={(sharedWith) => { onAddItem(currentDinerId, shareItem, true, sharedWith); setShareItem(null) }}
...
export default function GastroOrder() {
  const [sessions, setSessions] = useState<TableSession[]>([
    {
      ...
      diners: [
        {
          id: 'd1', name: 'Martina', emoji: '👩', paid: false, joinedAt: '20:15', cart: [],
          rounds: [{ id: 'r1', sentAt: '20:18', destination: 'both', items: [
            { menuItemId: 'm4', name: 'Risotto de Hongos', price: 10500, qty: 1, destination: 'kitchen' },
            { menuItemId: 'm10', name: 'Limonada Artesanal', price: 3500, qty: 1, destination: 'bar' },
          ]}]
        },
        {
          id: 'd2', name: 'Lucas', emoji: '👨', paid: false, joinedAt: '20:16',
          cart: [
            { menuItemId: 'm5', name: 'Lomo a la Pimienta', price: 18000, qty: 1, destination: 'kitchen' },
            { menuItemId: 'm9b', name: 'Vino Tinto Botella', price: 12000, qty: 1, destination: 'bar', shared: true, sharedWith: ['d1', 'd3'] },
          ],
          rounds: []
        },
        {
          id: 'd3', name: 'Sofía', emoji: '👱', paid: false, joinedAt: '20:22', cart: [],
          rounds: [
            { id: 'r2', sentAt: '20:25', destination: 'bar', items: [{ menuItemId: 'm10', name: 'Limonada Artesanal', price: 3500, qty: 1, destination: 'bar' }] },
            { id: 'r3', sentAt: '20:28', destination: 'kitchen', addedByWaiter: true, waiterId: 'w1', items: [
              { menuItemId: 'm3', name: 'Tabla de Embutidos', price: 12000, qty: 1, destination: 'kitchen', addedByWaiter: true, assignedTo: 'all' },
              { menuItemId: 'm8', name: 'Agua Mineral', price: 1200, qty: 2, destination: 'bar', addedByWaiter: true, assignedTo: 'd2' },
            ]}
          ]
        },
      ]
    },
    {
      ...
      waiters: [],
      diners: [
        { id: 'd4', name: 'Ana', emoji: '🧕', paid: false, joinedAt: '20:30', cart: [], rounds: [{ id: 'r4', sentAt: '20:35', destination: 'kitchen', items: [{ menuItemId: 'm1', name: 'Burrata con Tomates', price: 7000, qty: 1, destination: 'kitchen' }] }] },
        { id: 'd5', name: 'Pedro', emoji: '🧔', paid: false, joinedAt: '20:31', cart: [], rounds: [{ id: 'r5', sentAt: '20:35', destination: 'kitchen', items: [{ menuItemId: 'm6', name: 'Pasta Carbonara', price: 9800, qty: 1, destination: 'kitchen' }] }] },
      ]
    },
  ])
  ...
  const handleQRScan = (table: TableConfig) => {
    ...
    if (existing) {
      setKioskSession(existing); setKioskDinerId(null); setKioskMode('customer')
    } else {
      const newSession = createAutoSession(table)
      setSessions(prev => [...prev, newSession])
      setKioskSession(newSession); setKioskDinerId(null); setKioskMode('customer')
    }
  }

  const handleWaiterQRScan = (table: TableConfig) => {
    setPendingWaiterTable(table); setShowWaiterSelect(true)
  }
  ...
  const handleWaiterSelect = (waiter: Waiter) => {
    ...
    if (existing) {
      ...
      if (!alreadyAssigned) {
        setSessions(prev => prev.map(s => s.id === existing.id ? { ...s, waiters: [...s.waiters, waiter] } : s))
    ...
    setKioskSession(targetSession); setKioskWaiter(waiter); setKioskMode('waiter')
  }
  ...
  const handleAddItem = (sessionId: string, dinerId: string, item: MenuItem, shared: boolean, sharedWith: string[]) => {
    const updater = (s: TableSession): TableSession => ({
      ...
      diners: s.diners.map(d => {
        ...
        return { ...d, cart: [...d.cart, { menuItemId: item.id, name: item.name, price: item.price, qty: 1, destination: item.destination, shared, sharedWith: shared ? sharedWith : undefined }] }
      })
    })
  ...
  const handleSendRound = (sessionId: string, dinerId: string) => {
    const updater = (s: TableSession): TableSession => {
      ...
      return { ...s, status: 'sent', diners: s.diners.map(d => d.id === dinerId ? { ...d, cart: [], rounds: [...d.rounds, newRound] } : d) }
    }
    setSessions(prev => prev.map(s => s.id === sessionId ? updater(s) : s))
    setKioskSession(prev => prev?.id === sessionId ? updater(prev) : prev)
  }
    ...
    const updater = (s: TableSession): TableSession => {
      ...
      if (sharedItems.length > 0 && updatedDiners.length > 0) {
        updatedDiners = updatedDiners.map(d => ({
          ...d, rounds: [...d.rounds, { id: uid(), items: sharedItems, sentAt: nowTime(), destination: dest, addedByWaiter: true, waiterId }]
        }))
      }
      individualItems.forEach(item => {
        if (updatedDiners.find(d => d.id === item.assignedTo)) {
          updatedDiners = updatedDiners.map(d =>
            d.id === item.assignedTo
              ? { ...d, rounds: [...d.rounds, { id: uid(), items: [item], sentAt: nowTime(), destination: item.destination, addedByWaiter: true, waiterId }] }
              : d
          )
        }
      })
  ...
  const handleRegisterPayment = (sessionId: string, dinerIds: string[], method: string) => {
    setSessions(prev => prev.map(s => {
      ...
      const updatedDiners = s.diners.map(d => dinerIds.includes(d.id) ? { ...d, paid: true, paymentMethod: method } : d)
  ...
  const totalRevenue = activeSessions.reduce((s, sess) =>
    s + sess.diners.reduce((ss, d) => {
      const cartT = d.cart.reduce((sss, i) => { const base = i.price * i.qty; return sss + (i.shared && i.sharedWith ? Math.round(base / (i.sharedWith.length + 1)) : base) }, 0)
  ...
  const STATUS_CFG = {
    ordering: { label: 'Ordenando', color: '#2563EB', bg: 'rgba(37,99,235,0.15)' },
    sent:     { label: 'En cocina', color: '#3B82F6', bg: 'rgba(59,130,246,0.15)' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Sesiones activas', value: activeSessions.length, color: '#2563EB', icon: Wifi },
          { label: 'Comensales online', value: totalDiners, color: '#3B82F6', icon: Users },
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex gap-2">
          {[{ id: 'sessions', label: 'Sesiones activas' }, { id: 'qr', label: 'QR por mesa' }].map(v => (
            <button key={v.id} onClick={() => setActiveView(v.id as typeof activeView)}
              className="px-4 py-2 rounded-xl text-sm font-semibold transition-all"
              style={activeView === v.id
                ? { background: 'rgba(37,99,235,0.15)', border: '1px solid rgba(37,99,235,0.3)', color: '#2563EB' }
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs"
          style={{ background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.2)', color: '#7dd3fc' }}>
          <QrCode size={12} /> Sesiones se abren automáticamente al primer scan
        </div>
      </div>
        ...
        <div className="space-y-4">
          {activeSessions.length === 0 && (
            <div className="card-gastro text-center py-16">
              <Wifi size={32} className="mx-auto mb-3" style={{ color: '#4A5A7A' }} />
              <p className="text-gastro-subtle mb-2">No hay sesiones activas</p>
              <p className="text-xs mb-4" style={{ color: '#6666aa' }}>Las sesiones se abren automáticamente cuando un comensal escanea el QR</p>
          ...
          {activeSessions.map(session => {
            ...
            const sessionTotal = session.diners.reduce((s, d) => {
              const cartT = d.cart.reduce((ss, i) => { const base = i.price * i.qty; return ss + (i.shared && i.sharedWith ? Math.round(base / (i.sharedWith.length + 1)) : base) }, 0)
              const roundT = d.rounds.reduce((ss, r) => ss + r.items.reduce((sss, i) => {
                const base = i.price * i.qty
                if (i.addedByWaiter && i.assignedTo === 'all') return sss + Math.round(base / Math.max(session.diners.length, 1))
                return sss + (i.shared && i.sharedWith ? Math.round(base / (i.sharedWith.length + 1)) : base)
            ...
            return (
              <div key={session.id} className="card-gastro">
                <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {session.openedByFirstScan && (
                          <span className="text-xs px-1.5 py-0.5 rounded-lg"
                            style={{ background: 'rgba(59,130,246,0.1)', color: '#3B82F6', fontSize: '10px' }}>Auto-QR</span>
                        )}
                        {session.waiters.map(w => (
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-semibold px-2.5 py-1 rounded-lg" style={{ background: cfg.bg, color: cfg.color }}>{cfg.label}</span>
                    <div className="flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-lg"
                      style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid #1A2540', color: '#8899BB' }}>
                      <Users size={11} />{dinerCountLabel(session.diners.length, session.tableCapacity)}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-2 mb-4">
                  {session.diners.map(diner => {
                    const cartT = diner.cart.reduce((s, i) => { const base = i.price * i.qty; return s + (i.shared && i.sharedWith ? Math.round(base / (i.sharedWith.length + 1)) : base) }, 0)
                ...
                {isExpanded && (
                  <div className="mb-4 p-4 rounded-xl space-y-3"
                    ...
                    <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: '#8899BB' }}>Detalle por comensal</p>
                    {session.diners.map(diner => (
                      <div key={diner.id}>
                        <div className="flex items-center gap-2 mb-1.5">
                          ...
                          {diner.paid && (
                            <span className="text-xs px-1.5 py-0.5 rounded-lg text-success" style={{ background: 'rgba(16,185,129,0.12)' }}>
                        ...
                        {diner.rounds.map((round, ri) => (
                          <div key={round.id} className="pl-6 mb-2">
                            <div className="flex items-center gap-1.5 mb-1">
                              {round.addedByWaiter ? <BadgeCheck size={10} style={{ color: '#f59e0b' }} /> : <CheckCircle size={10} className="text-success" />}
                              <span className="text-xs font-semibold" style={{ color: round.addedByWaiter ? '#f59e0b' : '#10b981' }}>
                                {round.addedByWaiter ? 'Mozo' : `Ronda ${ri + 1}`} · {round.sentAt}
                              </span>
                              <DestBadge dest={round.destination === 'both' ? 'kitchen' : round.destination} small />
                            </div>
                            {round.items.map(item => (
                              <div key={item.menuItemId} className="flex items-center justify-between text-xs py-0.5">
                                <span style={{ color: '#8899BB' }}>
                                  {item.qty}× {item.name}
                                  {item.shared && <span style={{ color: '#60A5FA' }}> (compartido)</span>}
                        ...
                        {diner.cart.length > 0 && (
                          <div className="pl-6">
                            ...
                            {diner.cart.map(item => (
                              <div key={item.menuItemId} className="flex items-center justify-between text-xs py-0.5">
                                <span style={{ color: '#8899BB' }}>{item.qty}× {item.name}</span>
                    ...
                    <div className="border-t pt-2 flex items-center justify-between" style={{ borderColor: '#1A2540' }}>
                ...
                <div className="flex items-center justify-between pt-3 border-t" style={{ borderColor: '#1A2540' }}>
                  <div className="flex items-center gap-2">
                    ...
                    {session.diners.some(d => d.paid) && (
                      <span className="text-xs px-1.5 py-0.5 rounded-lg text-success" style={{ background: 'rgba(16,185,129,0.1)' }}>
                        {session.diners.filter(d => d.paid).length} pagaron
                      </span>
                    )}
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => { setKioskSession(session); setKioskDinerId(null); setKioskMode('customer') }}
                    ...
                    <button onClick={() => handleWaiterQRScan(MOCK_TABLES.find(t => t.number === session.tableNumber) ?? MOCK_TABLES[0])}
                      className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold transition-all"
                      style={{ background: 'rgba(245,158,11,0.12)', border: '1px solid rgba(245,158,11,0.3)', color: '#f59e0b' }}>
                      <BadgeCheck size={13} /> Mozo
                    </button>
                    <button onClick={() => handleForceClose(session.id)}
                      className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold transition-all"
            ...
            return (
              <div key={table.number} className="card-gastro">
                <div className="flex items-start justify-between mb-4">
                  ...
                  {activeSession ? (
                    <span className="text-xs font-semibold px-2 py-1 rounded-lg" style={{ background: 'rgba(37,99,235,0.15)', color: '#2563EB' }}>Sesión activa</span>
                  ) : (
                    <span className="text-xs font-semibold px-2 py-1 rounded-lg" style={{ background: 'rgba(16,185,129,0.12)', color: '#10b981' }}>Disponible</span>
                  )}
                </div>
                ...
                {activeSession && (
                  <div className="p-3 rounded-xl mb-3 space-y-1.5"
                    ...
                    <div className="flex items-center justify-between text-xs">
                      <span style={{ color: '#8899BB' }}>Comensales</span>
                      <span className="font-bold text-gastro-text">{dinerCountLabel(activeSession.diners.length, table.capacity)}</span>
                    </div>
                    <div className="flex items-center justify-between text-xs">
                      <span style={{ color: '#8899BB' }}>Mozos</span>
                      <span className="font-bold text-gastro-text">
                        {activeSession.waiters.length > 0 ? activeSession.waiters.map(w => `${w.emoji} ${w.name}`).join(', ') : 'Sin asignar'}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-xs">
                      <span style={{ color: '#8899BB' }}>Estado</span>
                      <span className="font-semibold" style={{ color: STATUS_CFG[activeSession.status].color }}>{STATUS_CFG[activeSession.status].label}</span>
                    </div>
                  </div>
                )}
                {!activeSession && (
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleQRScan(table)}
                    className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-semibold"
                    style={{ background: 'rgba(59,130,246,0.12)', border: '1px solid rgba(59,130,246,0.3)', color: '#3B82F6' }}>
                    <Smartphone size={13} />{activeSession ? 'Kiosco cliente' : 'Simular scan'}
                  </button>
                  <button onClick={() => handleWaiterQRScan(table)}
                    className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-semibold"
                    style={{ background: 'rgba(245,158,11,0.12)', border: '1px solid rgba(245,158,11,0.3)', color: '#f59e0b' }}>
                    <BadgeCheck size={13} /> Mozo
                  </button>
      ...
      {/* Waiter Select Modal — portal via FullscreenOverlay */}
      {showWaiterSelect && pendingWaiterTable && (
        <WaiterSelectModal
          tableLabel={pendingWaiterTable.label}
          onSelect={handleWaiterSelect}
          onCancel={() => { setShowWaiterSelect(false); setPendingWaiterTable(null) }}
        />
      )}
      ...
      {kioskSession && kioskMode === 'customer' && (
        <PhoneKiosk
          session={kioskSession} currentDinerId={kioskDinerId}
      ...
      {kioskSession && kioskMode === 'waiter' && kioskWaiter && (
        <WaiterPhoneKiosk
          session={kioskSession} waiter={kioskWaiter}
