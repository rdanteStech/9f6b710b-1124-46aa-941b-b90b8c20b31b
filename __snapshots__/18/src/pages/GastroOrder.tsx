...
import {
  ...
  ChefHat, GlassWater, Share2, Check, Info, Zap
} from 'lucide-react'
...
interface Diner {
  ...
  name: string
  emoji: string
  cart: CartItem[]          // pending (not yet sent)
  rounds: OrderRound[]      // sent rounds
  paid: boolean
...
function createAutoSession(table: TableConfig): TableSession {
  return {
    ...
    status: 'ordering', diners: [], tableCapacity: table.capacity,
      ...
      <div className="rounded-t-3xl overflow-hidden" style={{ background: '#1a1a26', border: '1px solid #2a2a3d' }}>
        ...
        <div className="p-4 border-b" style={{ borderColor: '#2a2a3d' }}>
          <div className="flex items-center gap-2 mb-1">
            <div className="w-6 h-6 rounded-lg flex items-center justify-center"
              ...
              <Send size={12} className="text-success" />
            </div>
                ...
                return (
                  <div key={item.menuItemId} className="flex items-center justify-between py-1.5 border-b last:border-0"
                    ...
                    <div className="flex-1">
                      ...
                      {isShared && (
                        <div className="text-xs mt-0.5" style={{ color: '#6666aa' }}>
                          Compartido · tu parte: ${myPrice.toLocaleString()}
                        </div>
                      )}
                    </div>
        ...
        <div className="p-4 border-t space-y-3" style={{ borderColor: '#2a2a3d' }}>
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold" style={{ color: '#8888aa' }}>Tu parte de este pedido</span>
            <span className="text-lg font-black text-primary-400">${total.toLocaleString()}</span>
          </div>
          <div className="flex gap-2">
            ...
            <button onClick={onConfirm}
              className="flex-1 py-3 rounded-2xl text-xs font-bold text-white flex items-center justify-center gap-1.5 transition-all"
    ...
    <div className="absolute inset-0 z-20 flex flex-col justify-end" style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)' }}>
      <div className="rounded-t-3xl overflow-hidden" style={{ background: '#1a1a26', border: '1px solid #2a2a3d' }}>
        ...
        <div className="p-4 space-y-2">
          {others.length === 0 ? (
            <p className="text-xs text-center py-4" style={{ color: '#6666aa' }}>
              No hay otros comensales en la mesa aún
            </p>
          ) : (
            others.map(d => {
          ...
          <div className="p-3 rounded-xl mt-2"
            ...
            <div className="flex items-center justify-between text-xs">
              <span style={{ color: '#8888aa' }}>
                ${item.price.toLocaleString()} ÷ {splitCount} {splitCount === 1 ? 'persona' : 'personas'}
              </span>
              <span className="font-black" style={{ color: '#f472b6' }}>
                ${myPrice.toLocaleString()} c/u
              </span>
            </div>
          </div>
        </div>
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center"
        ...
        <div className="absolute inset-0 flex flex-col overflow-hidden" style={{ paddingTop: 44 }}>
          ...
          {screen === 'menu' && (
            <div className="flex-1 flex flex-col overflow-hidden" style={{ background: '#0d0d18' }}>
                  ...
                  return (
                    <div key={item.id} className="rounded-2xl overflow-hidden"
                      ...
                      <div className="flex gap-3 p-3">
                        <div className="relative flex-shrink-0">
                          <img src={item.image} alt={item.name} className="w-18 h-18 rounded-xl object-cover"
              ...
              {/* Send to kitchen/bar CTA */}
              {myCart.length > 0 && (
                <div className="p-4 border-t flex-shrink-0 space-y-2" style={{ borderColor: '#1e1e2e' }}>
                  <button onClick={() => setScreen('cart')}
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                <div className="p-4 rounded-2xl text-center"
                  ...
                  <div className="text-xs mt-1" style={{ color: '#8888aa' }}>
                    {session.diners.length} comensales
                  </div>
                </div>
                  ...
                  <>
                    ...
                    {splitMode === 'individual' && (
                      <div className="space-y-2">
                        <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: '#8888aa' }}>Tu cuenta</p>
      ...
      <div className="ml-8 w-64 space-y-4 hidden xl:block">
        ...
        <div className="p-4 rounded-2xl" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
          <p className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: '#8888aa' }}>Destinos</p>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <ChefHat size={13} style={{ color: '#9E7FFF' }} />
              <span className="text-xs text-white font-semibold">Cocina</span>
              <span className="text-xs" style={{ color: '#6666aa' }}>Entradas, Principales, Postres</span>
            </div>
            <div className="flex items-center gap-2">
              <GlassWater size={13} style={{ color: '#38bdf8' }} />
              <span className="text-xs text-white font-semibold">Barra</span>
              <span className="text-xs" style={{ color: '#6666aa' }}>Bebidas, vinos, cócteles</span>
            </div>
            <div className="flex items-center gap-2">
              <Share2 size={13} style={{ color: '#f472b6' }} />
              <span className="text-xs text-white font-semibold">Compartido</span>
              <span className="text-xs" style={{ color: '#6666aa' }}>Divide el costo entre comensales</span>
            </div>
          </div>
        </div>
      </div>
      ...
      diners: [
        {
          ...
          cart: [],
          rounds: [{ id: 'r3', sentAt: '20:35', destination: 'kitchen', items: [{ menuItemId: 'm1', name: 'Burrata con Tomates', price: 1850, qty: 1, destination: 'kitchen' }] }]
        },
        {
          id: 'd5', name: 'Pedro', emoji: '🧔', paid: false, joinedAt: '20:31',
          cart: [],
          rounds: [{ id: 'r4', sentAt: '20:35', destination: 'kitchen', items: [{ menuItemId: 'm6', name: 'Pasta Carbonara', price: 2900, qty: 1, destination: 'kitchen' }] }]
        },
      ]
    },
  ])
            ...
            return (
              <div key={session.id} className="card-gastro">
                        ...
                        {diner.rounds.map((round, ri) => (
                          <div key={round.id} className="pl-6 mb-2">
                            <div className="flex items-center gap-1.5 mb-1">
                              <CheckCircle size={10} className="text-success" />
                              <span className="text-xs font-semibold text-success">Ronda {ri + 1} · {round.sentAt}</span>
                              <DestBadge dest={round.destination === 'both' ? 'kitchen' : round.destination} small />
                            </div>
                            {round.items.map(item => (
                ...
                <div className="flex items-center justify-between pt-3 border-t" style={{ borderColor: '#2a2a3d' }}>
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => { setKioskSession(session); setKioskDinerId(null) }}
              ...
              <div key={table.number} className="card-gastro">
                ...
                {activeSession && (
                  <div className="p-3 rounded-xl mb-3 space-y-1.5"
                    ...
                    <div className="flex items-center justify-between text-xs">
                      <span style={{ color: '#8888aa' }}>Rondas enviadas</span>
                      <span className="font-bold text-gastro-text">
                        {activeSession.diners.reduce((s, d) => s + d.rounds.length, 0)}
                      </span>
                    </div>
                  </div>
                )}
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    style={{ background: 'rgba(56,189,248,0.12)', border: '1px solid rgba(56,189,248,0.3)', color: '#38bdf8' }}>
                    <Smartphone size={13} />
                    {activeSession ? 'Abrir kiosco' : 'Simular scan'}
                  </button>
                  <button className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold"
