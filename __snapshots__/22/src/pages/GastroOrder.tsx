...
// ─── FullscreenOverlay — base portal wrapper ──────────────────────────────────
// Mounts directly on document.body via createPortal.
// Guarantees 100vw × 100vh solid black coverage, z-index max.
    ...
    <FullscreenOverlay>
      ...
      <button
        ...
        style={{
          ...
          color: '#6666aa', background: 'rgba(255,255,255,0.04)',
      ...
      <div style={{
        ...
        <div
          style={{
            ...
            borderRadius: 48,
            background: '#0a0a12',
            border: '10px solid #1a1a26',
            boxShadow: [
              `0 0 0 1px ${accentColor}`,
              `0 0 0 2px #0a0a12`,
              `0 0 60px ${glowColor ?? '#9E7FFF'}33`,
          ...
          <div style={{
            ...
            zIndex: 30, width: 120, height: 32,
            background: '#1a1a26', borderRadius: '0 0 20px 20px',
          }} />
...
// ─── Waiter Select Modal ──────────────────────────────────────────────────────
// Uses FullscreenOverlay so it also covers 100% of the viewport.
                  ...
                  <div key={item.menuItemId} className="flex items-center justify-between py-1.5 border-b last:border-0"
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {isShared && <span className="text-xs px-1 py-0.5 rounded" style={{ background: 'rgba(244,114,182,0.15)', color: '#f472b6', fontSize: '9px' }}>÷{splitCount}</span>}
                        ...
                        {item.addedByWaiter && item.assignedTo !== 'all' && <span className="text-xs px-1 py-0.5 rounded" style={{ background: 'rgba(56,189,248,0.15)', color: '#38bdf8', fontSize: '9px' }}>asignado</span>}
                      </div>
                    </div>
...
function WaiterPhoneKiosk({
  ...
  return (
    <PhoneShell
      accentColor={`${waiter.color}66`}
      glowColor={waiter.color}
        ...
        <div className="flex-1 flex flex-col overflow-hidden" style={{ background: '#0d0d18' }}>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            {session.diners.some(d => d.rounds.some(r => r.addedByWaiter)) && (
              <div>
                <p className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color: '#8888aa' }}>Pedidos del mozo</p>
                {session.diners.flatMap(d => d.rounds.filter(r => r.addedByWaiter)).slice(-3).map(round => (
                  <div key={round.id} className="p-3 rounded-xl mb-2"
                    ...
                    <div className="flex items-center gap-1.5 mb-1.5">
                      ...
                      <span className="text-xs font-semibold" style={{ color: waiter.color }}>Mozo · {round.sentAt}</span>
      ...
      {assignItem && (
        <WaiterAssignModal
          item={assignItem} diners={session.diners}
      ...
      {showConfirm && (
        <ConfirmSendModal
          items={waiterCart} dinerName={waiter.name} dinerEmoji={waiter.emoji}
  ...
  return (
    <PhoneShell
      ...
      {showConfirm && screen === 'cart' && currentDiner && (
        <ConfirmSendModal
          items={myCart} dinerName={currentDiner.name} dinerEmoji={currentDiner.emoji}
          onConfirm={handleSendConfirmed} onCancel={() => setShowConfirm(false)}
        />
      )}
      {shareItem && currentDinerId && (
        <ShareModal
          item={shareItem} diners={session.diners} currentDinerId={currentDinerId}
...
export default function GastroOrder() {
  const [sessions, setSessions] = useState<TableSession[]>([
    ...
    },
    {
      ...
      diners: [
        {
          id: 'd4', name: 'Ana', emoji: '🧕', paid: false, joinedAt: '20:30', cart: [],
        ...
        {
          id: 'd5', name: 'Pedro', emoji: '🧔', paid: false, joinedAt: '20:31', cart: [],
  ...
  const handleRegisterPayment = (sessionId: string, dinerIds: string[], method: string) => {
    ...
    if (session) {
      ...
      if (remaining.length === 0) {
        setTimeout(() => { setSessions(prev => prev.filter(s => s.id !== sessionId)); setKioskSession(null); setKioskDinerId(null) }, 2000)
      }
    }
  }
