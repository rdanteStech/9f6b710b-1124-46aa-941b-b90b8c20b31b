...
export default function ServeHoursCountdown({
  ...
  if (compact) {
    ...
    return (
      <div
        className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-1.5 rounded-lg whitespace-nowrap flex-shrink-0"
        style={{
          background: accent.bg,
          border: `1px solid ${accent.border}`,
          boxShadow: `0 0 14px ${accent.border}`,
        }}
        title={`${targetLabel} a las ${formatTargetTime(state.target)} · ${label} ${time}`}
        role="status"
        aria-live="polite"
      ...
      >
        ...
        <span
          className="text-[10px] font-bold uppercase tracking-wide leading-none hidden sm:inline"
          style={{ color: accent.color }}
        >
          {isClosing ? 'Cierre' : 'Apertura'}
        </span>
        <span
          className={`text-xs sm:text-sm font-black tabular-nums tracking-wide leading-none ${urgent ? 'animate-pulse' : ''}`}
          style={{ color: accent.color }}
        >
          {time}
        </span>
