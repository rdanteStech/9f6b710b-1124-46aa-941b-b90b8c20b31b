...
type ViewId = 'products' | 'channels' | 'categories' | 'recipes'
...
function parseViewTab(tab: string | null): ViewId {
...
export default function GastroMenu() {
  ...
  const [activeView, setActiveView] = useState<ViewId>(() => parseViewTab(searchParams.get('tab')))
  ...
  useEffect(() => {
    ...
    setActiveView(prev => (prev === tabFromUrl ? prev : tabFromUrl))
  }, [searchParams])

  const setView = useCallback((view: ViewId) => {
    setActiveView(view)
    const tabParam = view === 'recipes' ? 'recetas'
        ...
        <div className="flex gap-2 flex-wrap">
          {VIEW_TABS.map(view => (
            <button key={view.id}
              ...
              {view.label}
              {view.id === 'recipes' && labStats.total > 0 && (
                <span className="absolute -top-1.5 -right-1.5 min-w-5 h-5 px-1 rounded-full text-[10px] font-bold flex items-center justify-center text-white"
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            <div className="space-y-3">
              {filteredProducts.map(product => {
                const linkedRecipe = labEntries.find(e => e.linkedProductId === product.id)
                return (
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center gap-4"
            ...
            <div className="flex-1">
              <div className="font-bold text-gastro-text text-sm">Recetas de carta</div>
              <div className="text-xs text-gastro-subtle mt-1">
                {labStats.total} recetas con ingredientes, costos en tiempo real desde GastroStock,
                versiones y publicación directa en la carta.
              </div>
            </div>
