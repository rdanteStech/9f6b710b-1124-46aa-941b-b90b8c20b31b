...
/**
 * Find the closest point on a quadratic bezier curve to (px, py).
 */
function closestPointOnQuadBezier(
  px: number, py: number,
  ax: number, ay: number,
  ...
  let bestDist = Infinity, bestT = 0, bestX = ax, bestY = ay
  for (let i = 0; i <= steps; i++) {
    const t = i / steps
    const mt = 1 - t
    const x = mt * mt * ax + 2 * mt * t * cpx + t * t * bx
    const y = mt * mt * ay + 2 * mt * t * cpy + t * t * by
    const dist = Math.sqrt((px - x) ** 2 + (py - y) ** 2)
    if (dist < bestDist) { bestDist = dist; bestT = t; bestX = x; bestY = y }
  }
...
function splitQuadBezier(
  ...
  bx: number, by: number,
  t: number
): { p: { x: number; y: number }; cp1: { x: number; y: number }; cp2: { x: number; y: number } } {
  const mt = 1 - t
  const cp1x = mt * ax + t * cpx
  const cp1y = mt * ay + t * cpy
  const cp2x = mt * cpx + t * bx
  const cp2y = mt * cpy + t * by
  const px = mt * cp1x + t * cp2x
  const py = mt * cp1y + t * cp2y
  return { p: { x: px, y: py }, cp1: { x: cp1x, y: cp1y }, cp2: { x: cp2x, y: cp2y } }
}
...
/**
 * Convert bezier SpacePath → dense polyline for physics.
 */
function approximateSpacePath(sp: SpacePath, steps = 12): { x: number; y: number }[] {
...
export default function GastroLayout() {
  ...
  // ── Refs for arrow-key handler (avoid stale closures) ─────────────────────
  const selectedIdRef = useRef(selectedId)
  const activeZoneIdRef = useRef(activeZoneId)
  const minTableDistRef = useRef(minTableDist)
  const zonesRef = useRef(zones)
  useEffect(() => { selectedIdRef.current = selectedId }, [selectedId])
  useEffect(() => { activeZoneIdRef.current = activeZoneId }, [activeZoneId])
  useEffect(() => { minTableDistRef.current = minTableDist }, [minTableDist])
  useEffect(() => { zonesRef.current = zones }, [zones])

  // ── Arrow-key movement ────────────────────────────────────────────────────
  // Rules:
  //   • Only fires when a table is selected and no input/textarea is focused
  //   • Step: 1px (Shift → 10px)
  //   • NO wall-distance enforcement — only raw SVG canvas bounds (x≥0, y≥0)
  //   • Table-to-table minimum distance IS still enforced
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      setArrowMoving(true)
      setTimeout(() => setArrowMoving(false), 600)

      setZones(prev => {
        const zoneIndex = prev.findIndex(z => z.id === activeZoneIdRef.current)
        if (zoneIndex === -1) return prev
        const zone = prev[zoneIndex]
        const moving = zone.tables.find(t => t.id === id)
        if (!moving || moving.locked) return prev
        const others = zone.tables.filter(t => t.id !== id)

        let nx = moving.x + dx
        let ny = moving.y + dy

        // ── Only raw canvas bounds — no wall distance enforcement ──────────
        // Get SVG dimensions from the ref (fallback to large number)
        const svg = document.querySelector('svg[data-canvas]') as SVGSVGElement | null
        const svgW = svg ? svg.clientWidth / 1 : 2000  // zoom not applied here since coords are pre-zoom
        const svgH = svg ? svg.clientHeight / 1 : 2000
        nx = Math.max(0, Math.min(svgW - moving.width, nx))
        ny = Math.max(0, Math.min(svgH - moving.height, ny))

        // ── Table-to-table minimum distance still enforced ─────────────────
        const minPad = minTableDistRef.current * METER_PX
        const candidate = { x: nx, y: ny, width: moving.width, height: moving.height }
        const collision = others.some(o => rectsOverlapWithPad(candidate, o, minPad))
        if (collision) return prev

        const newZones = [...prev]
        newZones[zoneIndex] = {
          ...zone,
          tables: zone.tables.map(t => t.id === id ? { ...t, x: nx, y: ny } : t),
        }
        return newZones
      })
    }
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if ((tool === 'addPoint' || tool === 'curveEdge') && activeSpacePath.points.length > 1) {
      ...
      for (let i = 0; i < activeSpacePath.points.length; i++) {
        const a = activeSpacePath.points[i], b = activeSpacePath.points[(i + 1) % activeSpacePath.points.length]
        const seg = activeSpacePath.segments[i] || {}
        let dist: number, cx: number, cy: number
        if (seg.controlPoint && tool === 'addPoint') {
          const result = closestPointOnQuadBezier(pt.x, pt.y, a.x, a.y, seg.controlPoint.x, seg.controlPoint.y, b.x, b.y)
          dist = result.dist; cx = result.x; cy = result.y
        } else {
          const result = pointToSegmentDist(pt.x, pt.y, a.x, a.y, b.x, b.y)
          dist = result.dist; cx = result.cx; cy = result.cy
        }
    ...
    setZones(prev => {
      ...
      const hasBoundary = zone.spacePath.points.length > 2
      const approxPoly = hasBoundary ? approximateSpacePath(zone.spacePath) : []
      ...
      // 2. Wall magnetic snap — uses curved approxPoly
      if (!tableSnap.snappedTo && moving.wallMagnet && hasBoundary) {
      ...
      // 3. Boundary clamp — uses curved approxPoly
      if (hasBoundary) {
        const clamped = clampInsidePolygon(finalX, finalY, moving.width, moving.height, approxPoly, wallDistPx)
        finalX = clamped.x; finalY = clamped.y
      }

      // 4. Proximity coupling — 5px, applies to ALL tables regardless of magnet
      ...
      // 7. Wall distance warning — uses curved approxPoly
      if (hasBoundary && !snappedToWall) {
        const corners = [
        ...
        const tooClose = corners.some(corner => {
          for (let i = 0; i < approxPoly.length; i++) {
            const a = approxPoly[i], b = approxPoly[(i + 1) % approxPoly.length]
  ...
  const handleCanvasClick = useCallback((e: React.MouseEvent) => {
    if (tool === 'draw') {
      ...
      let bestDist = Infinity, bestIndex = -1, bestT = 0, bestX = 0, bestY = 0
      for (let i = 0; i < activeSpacePath.points.length; i++) {
        const a = activeSpacePath.points[i], b = activeSpacePath.points[(i + 1) % activeSpacePath.points.length]
        const seg = activeSpacePath.segments[i] || {}
        let dist: number, t: number, cx: number, cy: number
        if (seg.controlPoint) {
          const result = closestPointOnQuadBezier(pt.x, pt.y, a.x, a.y, seg.controlPoint.x, seg.controlPoint.y, b.x, b.y)
          dist = result.dist; t = result.t; cx = result.x; cy = result.y
        } else {
          const result = pointToSegmentDist(pt.x, pt.y, a.x, a.y, b.x, b.y)
          dist = result.dist; t = result.t; cx = result.cx; cy = result.cy
        }
        if (dist < bestDist) { bestDist = dist; bestIndex = i; bestT = t; bestX = cx; bestY = cy }
      }
      ...
      setZones(prev => prev.map(z => {
        ...
        const newPoints = [...z.spacePath.points]
        const newSegments = [...z.spacePath.segments]
        const seg = z.spacePath.segments[bestIndex] || {}
        const a = z.spacePath.points[bestIndex]
        const b = z.spacePath.points[(bestIndex + 1) % z.spacePath.points.length]

        if (seg.controlPoint) {
          const split = splitQuadBezier(a.x, a.y, seg.controlPoint.x, seg.controlPoint.y, b.x, b.y, bestT)
          newPoints.splice(bestIndex + 1, 0, split.p)
          newSegments.splice(bestIndex, 1, { controlPoint: split.cp1 }, { controlPoint: split.cp2 })
        } else {
  ...
  }, [tool, isDrawing, getSVGPoint, activeSpacePath.points, activeSpacePath.segments, activeZoneId])
      ...
      <div className="flex gap-4 flex-1 min-h-0">
        ...
        <div className="flex flex-col gap-2 flex-shrink-0 w-44 overflow-y-auto">
          <div className="card-gastro p-3 space-y-1">
            ...
            {tool === 'addPoint' && (
              <p className="text-xs mt-2 leading-relaxed px-1" style={{ color: '#10b981' }}>
                Clic en un borde para insertar un punto. En curvas, el punto se crea sobre la curva visual y la divide en dos sub-curvas.
              </p>
            )}
            {tool === 'curveEdge' && (
              <p className="text-xs mt-2 leading-relaxed px-1" style={{ color: '#f472b6' }}>Arrastra un borde para curvarlo. El límite físico sigue la curva visual.</p>
            )}
          </div>
          ...
          <div className="card-gastro p-3">
            ...
            <div className="mt-3 pt-2 border-t border-gastro-border space-y-1">
              <p className="text-xs text-gastro-subtle leading-relaxed">🔒 <strong className="text-gastro-text">VIP:</strong> Presiona el candado para mover.</p>
              <p className="text-xs text-gastro-subtle leading-relaxed">🔗 <strong className="text-gastro-text">Fusión:</strong> Arrastra una mesa sobre otra.</p>
              <p className="text-xs text-gastro-subtle leading-relaxed">〜 <strong className="text-gastro-text">Curva:</strong> El límite físico sigue la curva visual.</p>
              <p className="text-xs text-gastro-subtle leading-relaxed">➕ <strong className="text-gastro-text">Punto en curva:</strong> Se divide en dos sub-curvas exactas.</p>
              <p className="text-xs text-gastro-subtle leading-relaxed">🧲 <strong className="text-gastro-text">5px:</strong> Auto-acoplamiento a mesas y paredes.</p>
              <p className="text-xs text-gastro-subtle leading-relaxed">⌨️ <strong className="text-gastro-text">Flechas:</strong> Ajuste fino 1px sin límite de pared. Shift = 10px.</p>
              <p className="text-xs text-gastro-subtle leading-relaxed">✏️ <strong className="text-gastro-text">Zona:</strong> Doble clic en la pestaña para renombrar.</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 rounded-2xl overflow-hidden relative" style={{ background: '#0d0d18', border: `1px solid ${activeZone.color}22` }}>
          ...
          <div className="absolute top-3 left-1/2 -translate-x-1/2 z-10 pointer-events-none flex flex-col items-center gap-2">
            ...
            {tool === 'addPoint' && (
              <div className="px-4 py-2 rounded-xl text-xs font-semibold"
                ...
                ➕ Clic en un borde — en curvas, divide la curva en dos sub-curvas exactas
              </div>
            )}
            {tool === 'curveEdge' && (
              <div className="px-4 py-2 rounded-xl text-xs font-semibold"
                style={{ background: 'rgba(244,114,182,0.15)', border: '1px solid rgba(244,114,182,0.4)', color: '#f472b6' }}>
                〜 Arrastra un borde para curvarlo · El límite físico sigue la curva
              </div>
            )}
            {arrowMoving && selectedId && (
              <div className="px-4 py-2 rounded-xl text-xs font-semibold"
                style={{ background: 'rgba(158,127,255,0.2)', border: '1px solid rgba(158,127,255,0.5)', color: '#9E7FFF' }}>
                ⌨️ Ajuste fino con flechas — sin límite de pared · Shift = 10px
              </div>
            )}
            {pendingFuse && (
              <div className="px-4 py-2 rounded-xl text-xs font-semibold"
          ...
          >
            ...
            {/* Tables rendered WITHOUT clipPath so arrow-key movement can go past zone boundary */}
            <g clipPath={activeSpacePath.points.length > 2 && !arrowMoving ? 'url(#space-clip)' : undefined}>
