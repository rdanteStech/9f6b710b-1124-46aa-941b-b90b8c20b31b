 ...
 * When a wall edge is curved, this produces points along the visual curve
 * so that clamping, snapping and warnings all follow the curved boundary.
 */
function approximateSpacePath(sp: SpacePath, steps = 12): { x: number; y: number }[] {
  ...
  for (let i = 0; i < sp.points.length; i++) {
    ...
    result.push({ ...a })
    if (seg.controlPoint) {
      // Quadratic bezier: sample intermediate points along the curve
...
/**
 * Clamp table inside polygon (uses approximated curved poly so curved walls
 * act as the real physical boundary).
 */
...
/**
 * Snap table to wall using 8 probe points against the approximated curved polygon.
 * This makes the magnet follow the visual curved boundary.
 */
function snapToWall(
...
export default function GastroLayout() {
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    setZones(prev => {
      ...
      // Pre-compute the approximated curved polygon ONCE per frame.
      // All physics (clamp, wall snap, wall warning) use this so curved walls
      // are the actual physical boundary, not the original straight segments.
      ...
      // 3. Boundary clamp — uses curved approxPoly so tables can't cross curved walls
      if (hasBoundary) {
      ...
      // Wall proximity coupling — uses curved approxPoly
      ...
      <div className="flex gap-4 flex-1 min-h-0">
        ...
        <div className="flex-1 rounded-2xl overflow-hidden relative" style={{ background: '#0d0d18', border: `1px solid ${activeZone.color}22` }}>
          ...
          >
            ...
            {/* Wall distance inset guide during drag — follows the visual curved path */}
