...
function PhoneShell({
  accentColor = '#2a2a3d', glowColor, label, labelDot, onClose, children,
}: {
    ...
    <FullscreenOverlay>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{
          ...
          borderRadius: 48, background: '#0a0a12', border: '10px solid #1a1a26',
          boxShadow: [
            `0 0 0 1px ${accentColor}`, `0 0 0 2px #0a0a12`,
            `0 0 60px ${glowColor ?? '#9E7FFF'}33`, '0 40px 100px rgba(0,0,0,0.9)',
          ...
          <div style={{
            ...
            zIndex: 30, width: 120, height: 32, background: '#1a1a26', borderRadius: '0 0 20px 20px',
          }} />
          <div style={{
    ...
    <FullscreenOverlay onClose={onCancel}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{
          ...
          <div style={{ padding: '20px 20px 16px', borderBottom: '1px solid #2a2a3d' }}>
          ...
          <div style={{ padding: '0 16px 16px' }}>
            <button onClick={onCancel}
              style={{
                ...
                background: 'rgba(255,255,255,0.04)', border: '1px solid #2a2a3d', color: '#8888aa',
              }}>
              Cancelar
            </button>
          </div>
    ...
    <div className="absolute inset-0 z-20 flex flex-col justify-end"
      ...
      <div className="rounded-t-3xl overflow-hidden" style={{ background: '#1a1a26', border: '1px solid #2a2a3d' }}>
        <div className="p-4 border-b" style={{ borderColor: '#2a2a3d' }}>
          ...
          <p className="text-xs" style={{ color: '#8888aa' }}>{dinerEmoji} {dinerName} · revisá antes de enviar</p>
        </div>
                ...
                return (
                  <div key={item.menuItemId} className="flex items-center justify-between py-1.5 border-b last:border-0"
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap flex-1">
                      ...
                      {item.shared && <span className="text-xs px-1 py-0.5 rounded" style={{ background: 'rgba(244,114,182,0.15)', color: '#f472b6', fontSize: '9px' }}>÷{splitCount}</span>}
        ...
        <div className="flex-1 flex flex-col overflow-hidden" style={{ background: '#0d0d18' }}>
          <div className="flex-shrink-0 p-4"
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="text-xs mt-0.5" style={{ color: '#8888aa' }}>{session.tableLabel} · {session.zone}</div>
              </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {session.waiters.length > 0 && (
              <div className="p-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid #2a2a3d' }}>
                <p className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color: '#8888aa' }}>Mozos en esta mesa</p>
                    ...
                    return (
                      <div key={d.id} className="flex items-center gap-3 p-3 rounded-xl"
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs" style={{ color: '#8888aa' }}>{d.rounds.length} ronda{d.rounds.length !== 1 ? 's' : ''}</div>
                        </div>
                        <div className="text-right">
                          <div className="text-sm font-black" style={{ color: dTotal > 0 ? '#9E7FFF' : '#4a4a6a' }}>${dTotal.toLocaleString()}</div>
...
function PhoneKiosk({ session, currentDinerId, onJoin, onAddItem, onRemoveItem, onSendRound, onRegisterPayment, onClose }: {
  ...
  return (
    <PhoneShell accentColor="#2a2a3d" glowColor="#9E7FFF"
      label={<>Vista del cliente · <span style={{ color: '#9E7FFF' }}>{session.tableLabel}</span></>}
              ...
              <div className="flex -space-x-2 justify-center">
                {session.diners.slice(0, 6).map(d => (
                  <div key={d.id} className="w-9 h-9 rounded-full flex items-center justify-center text-base"
                    style={{ background: '#1e1e2e', border: '2px solid #0d0d18' }}>{d.emoji}</div>
                ))}
              </div>
            )}
            <div>
      ...
      {screen === 'cart' && (
        <div className="flex-1 flex flex-col overflow-hidden" style={{ background: '#0d0d18' }}>
          <div className="flex items-center justify-between px-4 py-3 flex-shrink-0"
            style={{ borderBottom: '1px solid #1e1e2e' }}>
            <button onClick={() => setScreen('menu')}
              className="flex items-center gap-1 text-xs font-semibold" style={{ color: '#9E7FFF' }}>
              <ChevronRight size={14} className="rotate-180" /> Menú
            </button>
              ...
              <div key={session.id} className="card-gastro">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {session.openedByFirstScan && (
                          <span className="text-xs px-1.5 py-0.5 rounded-lg"
                            style={{ background: 'rgba(56,189,248,0.1)', color: '#38bdf8', fontSize: '10px' }}>Auto-QR</span>
                        )}
                        {session.waiters.map(w => (
                ...
                {isExpanded && (
                  <div className="mb-4 p-4 rounded-xl space-y-3"
                    ...
                    <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: '#8888aa' }}>Detalle por comensal</p>
                    {session.diners.map(diner => (
                      <div key={diner.id}>
            ...
            return (
              <div key={table.number} className="card-gastro">
                <div className="flex items-start justify-between mb-4">
                  ...
                  {activeSession ? (
                    <span className="text-xs font-semibold px-2 py-1 rounded-lg" style={{ background: 'rgba(158,127,255,0.15)', color: '#9E7FFF' }}>Sesión activa</span>
                  ) : (
