...
const STOCK_ALERTS: StockAlert[] = [
  ...
  { id: 's4', product: 'Vino Carmenère', category: 'Bebidas', currentStock: 18, predictedNeed: 30, daysUntilOut: 2, urgency: 'warning', suggestedOrder: 24, unit: 'btl' },
...
const DEMAND_PRODUCTS: DemandProduct[] = [
  { name: 'Pasta carbonara', category: 'Pastas', trend: 'up', trendPct: 24, predictedSales: 48, color: '#2563EB' },
  { name: 'Risotto de hongos', category: 'Arroces', trend: 'up', trendPct: 18, predictedSales: 32, color: '#3B82F6' },
  ...
  { name: 'Pizza margherita', category: 'Pizzas', trend: 'down', trendPct: -8, predictedSales: 22, color: '#60A5FA' },
]
...
const WEATHER_ICON = {
  ...
  rain: { icon: CloudRain, color: '#3B82F6', label: 'Lluvia' },
}
...
const URGENCY_CFG = {
  ...
  info:     { color: '#3B82F6', bg: 'rgba(59,130,246,0.1)', label: 'Info',    emoji: '🔵' },
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl p-6 relative overflow-hidden"
        style={{
          background: 'linear-gradient(135deg, rgba(59,130,246,0.08) 0%, rgba(37,99,235,0.06) 50%, rgba(16,185,129,0.04) 100%)',
        ...
        <div className="relative z-10 flex items-center gap-4 flex-wrap">
          ...
          <div className="hidden md:grid grid-cols-3 gap-6 text-center flex-shrink-0">
            {[
              ...
              <div key={s.label}>
                <div className="text-xl font-black" style={{ color: '#3B82F6' }}>{s.value}</div>
            ...
            return (
              <button key={day.day}
                ...
                <div className="text-center">
                  <div className="text-sm font-black" style={{ color: isSelected ? '#3B82F6' : '#e0e0e0' }}>
      ...
      <div className="rounded-2xl p-5"
        style={{ background: 'rgba(59,130,246,0.04)', border: '1px solid rgba(59,130,246,0.2)' }}>
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-xs text-gastro-subtle mt-0.5">
              Confianza del modelo: <span style={{ color: '#3B82F6' }}>{selectedDay.confidence}%</span>
              {selectedDay.events.length > 0 && (
                <> · Eventos: <span style={{ color: '#60A5FA' }}>{selectedDay.events.join(', ')}</span></>
              )}
            </p>
          </div>
          <div className="flex gap-4">
            {[
              ...
              { label: 'Predicci��n', value: `$${(selectedDay.predicted / 1000).toFixed(0)}K`, color: '#3B82F6' },
          ...
          <div className="card-gastro">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <ComposedChart data={HOURLY_DEMAND} margin={{ top: 5, right: 5, bottom: 0, left: -20 }}>
                ...
                <Bar dataKey="actual" name="Real" fill="#2563EB" fillOpacity={0.8} radius={[4, 4, 0, 0]} />
                <Line type="monotone" dataKey="predicted" name="Predicción" stroke="#3B82F6" strokeWidth={2} strokeDasharray="5 5" dot={false} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="lg:col-span-2 card-gastro">
            ...
            <div className="grid md:grid-cols-4 gap-4">
              {[
                ...
                { label: 'Evento programado', value: 'Show de jazz 21hs', impact: '+28% noche', color: '#60A5FA', icon: Star },
                { label: 'Historial viernes', value: 'Pico 20-22hs', impact: 'Patrón confirmado', color: '#2563EB', icon: BarChart3 },
            ...
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={HOURLY_DEMAND} margin={{ top: 5, right: 5, bottom: 0, left: -20 }}>
                ...
                <Tooltip
                  ...
                  labelStyle={{ color: '#e0e0e0', fontWeight: 'bold' }}
                />
                <Area type="monotone" dataKey="predicted" name="Pedidos predichos" stroke="#3B82F6" strokeWidth={2.5} fill="url(#staffGrad)" />
              </AreaChart>
            </ResponsiveContainer>
            <div className="mt-3 p-3 rounded-xl text-xs"
              style={{ background: 'rgba(37,99,235,0.06)', border: '1px solid rgba(37,99,235,0.2)' }}>
              <span style={{ color: '#2563EB' }}>💡 IA sugiere:</span>
      ...
      {activeTab === 'revenue' && (
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="lg:col-span-2 card-gastro">
            ...
            <ResponsiveContainer width="100%" height={240}>
              <ComposedChart data={FORECAST_DAYS} margin={{ top: 5, right: 5, bottom: 0, left: 0 }}>
                ...
                <Area type="monotone" dataKey="upper" name="Máximo" stroke="transparent" fill="#3B82F622" />
                <Area type="monotone" dataKey="lower" name="Mínimo" stroke="transparent" fill="#000000" />
                <Line type="monotone" dataKey="predicted" name="Predicción" stroke="#3B82F6" strokeWidth={2.5} dot={{ fill: '#3B82F6', r: 4 }} />
                <Line type="monotone" dataKey="actual" name="Real" stroke="#2563EB" strokeWidth={2.5} dot={{ fill: '#2563EB', r: 4 }} connectNulls={false} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="card-gastro">
            ...
            <div className="space-y-3">
              {[
                { label: 'Salón (mesas)', value: 2100000, pct: 42, color: '#2563EB' },
                { label: 'Delivery propio', value: 980000, pct: 20, color: '#3B82F6' },
                { label: 'Take away', value: 450000, pct: 9, color: '#10b981' },
                { label: 'Eventos privados', value: 1180000, pct: 24, color: '#60A5FA' },
            ...
            <div className="mt-4 pt-4 border-t flex items-center justify-between" style={{ borderColor: '#1A2540' }}>
              ...
              <span className="text-xl font-black" style={{ color: '#3B82F6' }}>$4.96M</span>
            </div>
          </div>
