...
const DEMAND_PRODUCTS: DemandProduct[] = [
  { name: 'Pasta carbonara', category: 'Pastas', trend: 'up', trendPct: 24, predictedSales: 48, color: '#0066FF' },
  { name: 'Risotto de hongos', category: 'Arroces', trend: 'up', trendPct: 18, predictedSales: 32, color: '#3399FF' },
  ...
  { name: 'Pizza margherita', category: 'Pizzas', trend: 'down', trendPct: -8, predictedSales: 22, color: '#00B4FF' },
]
...
const WEATHER_ICON = {
  ...
  cloud: { icon: Wind, color: '#8899BB', label: 'Nublado' },
  rain: { icon: CloudRain, color: '#3399FF', label: 'Lluvia' },
}
...
const URGENCY_CFG = {
  ...
  info:     { color: '#3399FF', bg: 'rgba(51,153,255,0.1)', label: 'Info',    emoji: '🔵' },
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl p-6 relative overflow-hidden"
        style={{
          background: 'linear-gradient(135deg, rgba(51,153,255,0.08) 0%, rgba(0,102,255,0.06) 50%, rgba(16,185,129,0.04) 100%)',
          border: '1px solid rgba(51,153,255,0.25)',
        }}>
        <div className="absolute top-0 right-0 w-80 h-80 opacity-10 pointer-events-none"
          style={{ background: 'radial-gradient(circle, #3399FF 0%, transparent 70%)', transform: 'translate(30%, -30%)' }} />
        ...
        <div className="relative z-10 flex items-center gap-4 flex-wrap">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0"
            style={{ background: 'linear-gradient(135deg, #3399FF, #0066FF)', boxShadow: '0 0 30px rgba(51,153,255,0.4)' }}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              ...
              <span className="text-xs px-2 py-0.5 rounded-lg font-bold"
                style={{ background: 'rgba(51,153,255,0.15)', color: '#3399FF', border: '1px solid rgba(51,153,255,0.3)' }}>
                🧠 IA Predictiva
              </span>
              {criticalAlerts > 0 && (
          ...
          <div className="hidden md:grid grid-cols-3 gap-6 text-center flex-shrink-0">
            {[
              ...
              <div key={s.label}>
                <div className="text-xl font-black" style={{ color: '#3399FF' }}>{s.value}</div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-bold text-gastro-text flex items-center gap-2">
            <Calendar size={16} style={{ color: '#3399FF' }} />
              ...
              <button key={day.day}
                ...
                style={{
                  background: isSelected
                    ? 'linear-gradient(135deg, rgba(51,153,255,0.15), rgba(0,102,255,0.1))'
                    : 'rgba(255,255,255,0.02)',
                  border: isSelected
                    ? '1px solid rgba(51,153,255,0.4)'
                    : '1px solid #1A2540',
                ...
                <div className="text-center">
                  <div className="text-sm font-black" style={{ color: isSelected ? '#3399FF' : '#e0e0e0' }}>
                ...
                {day.events.length > 0 && (
                  <div className="w-1.5 h-1.5 rounded-full" style={{ background: '#00B4FF' }} />
                )}
              </button>
            )
          })}
      ...
      <div className="rounded-2xl p-5"
        style={{ background: 'rgba(51,153,255,0.04)', border: '1px solid rgba(51,153,255,0.2)' }}>
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-xs text-gastro-subtle mt-0.5">
              Confianza del modelo: <span style={{ color: '#3399FF' }}>{selectedDay.confidence}%</span>
              {selectedDay.events.length > 0 && (
                <> · Eventos: <span style={{ color: '#00B4FF' }}>{selectedDay.events.join(', ')}</span></>
              )}
            </p>
          </div>
          <div className="flex gap-4">
            {[
              ...
              { label: 'Predicción', value: `$${(selectedDay.predicted / 1000).toFixed(0)}K`, color: '#3399FF' },
        ...
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id as typeof activeTab)}
            ...
            style={activeTab === tab.id
              ? { background: 'rgba(51,153,255,0.15)', border: '1px solid rgba(51,153,255,0.35)', color: '#3399FF' }
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="card-gastro">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <ComposedChart data={HOURLY_DEMAND} margin={{ top: 5, right: 5, bottom: 0, left: -20 }}>
                ...
                <Tooltip
                  contentStyle={{ background: '#0F1628', border: '1px solid #1A2540', borderRadius: '12px', fontSize: '11px' }}
                ...
                <Bar dataKey="actual" name="Real" fill="#0066FF" fillOpacity={0.8} radius={[4, 4, 0, 0]} />
                <Line type="monotone" dataKey="predicted" name="Predicción" stroke="#3399FF" strokeWidth={2} strokeDasharray="5 5" dot={false} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0 ml-2">
                        ...
                        {p.trend === 'stable' && <Activity size={13} style={{ color: '#8899BB' }} />}
                        <span className="text-xs font-bold"
                          style={{ color: p.trend === 'up' ? '#10b981' : p.trend === 'down' ? '#ef4444' : '#8899BB' }}>
          ...
          <div className="lg:col-span-2 card-gastro">
            <h3 className="font-bold text-gastro-text mb-4 flex items-center gap-2">
              <Sparkles size={15} style={{ color: '#3399FF' }} />
            ...
            <div className="grid md:grid-cols-4 gap-4">
              {[
                ...
                { label: 'Evento programado', value: 'Show de jazz 21hs', impact: '+28% noche', color: '#00B4FF', icon: Star },
                { label: 'Historial viernes', value: 'Pico 20-22hs', impact: 'Patrón confirmado', color: '#0066FF', icon: BarChart3 },
              ...
              <AreaChart data={HOURLY_DEMAND} margin={{ top: 5, right: 5, bottom: 0, left: -20 }}>
                <defs>
                  <linearGradient id="staffGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#3399FF" stopOpacity={0.3} />
                ...
                <Tooltip
                  ...
                  labelStyle={{ color: '#e0e0e0', fontWeight: 'bold' }}
                />
                <Area type="monotone" dataKey="predicted" name="Pedidos predichos" stroke="#3399FF" strokeWidth={2.5} fill="url(#staffGrad)" />
              </AreaChart>
            </ResponsiveContainer>
            <div className="mt-3 p-3 rounded-xl text-xs"
              style={{ background: 'rgba(0,102,255,0.06)', border: '1px solid rgba(0,102,255,0.2)' }}>
              <span style={{ color: '#0066FF' }}>💡 IA sugiere:</span>
      ...
      {activeTab === 'revenue' && (
        <div className="grid lg:grid-cols-2 gap-6">
              ...
              <ComposedChart data={FORECAST_DAYS} margin={{ top: 5, right: 5, bottom: 0, left: 0 }}>
                <defs>
                  <linearGradient id="predGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#3399FF" stopOpacity={0.25} />
                    <stop offset="95%" stopColor="#3399FF" stopOpacity={0} />
                  </linearGradient>
                </defs>
                <CartesianGrid strokeDasharray="3 3" stroke="#1A2540" vertical={false} />
                ...
                <Area type="monotone" dataKey="upper" name="Máximo" stroke="transparent" fill="#3399FF22" />
                <Area type="monotone" dataKey="lower" name="Mínimo" stroke="transparent" fill="#000000" />
                <Line type="monotone" dataKey="predicted" name="Predicción" stroke="#3399FF" strokeWidth={2.5} dot={{ fill: '#3399FF', r: 4 }} />
                <Line type="monotone" dataKey="actual" name="Real" stroke="#0066FF" strokeWidth={2.5} dot={{ fill: '#0066FF', r: 4 }} connectNulls={false} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="card-gastro">
            ...
            <div className="space-y-3">
              {[
                { label: 'Salón (mesas)', value: 2100000, pct: 42, color: '#0066FF' },
                { label: 'Delivery propio', value: 980000, pct: 20, color: '#3399FF' },
                { label: 'Take away', value: 450000, pct: 9, color: '#10b981' },
                { label: 'Eventos privados', value: 1180000, pct: 24, color: '#00B4FF' },
            ...
            <div className="mt-4 pt-4 border-t flex items-center justify-between" style={{ borderColor: '#1A2540' }}>
              <span className="font-bold text-gastro-text">Total predicho</span>
              <span className="text-xl font-black" style={{ color: '#3399FF' }}>$4.96M</span>
            </div>
          </div>
