...
const ZONE_COLORS = ['#0066FF', '#3399FF', '#10b981', '#f59e0b', '#00B4FF', '#ef4444']
    ...
    >
      ...
      {selected && table.wallMagnet && !table.fused && (
        <rect
          ...
          rx={8} fill="none" stroke="#00B4FF" strokeWidth={1} strokeDasharray="3 3" opacity={0.4}
        />
      )}
      {table.fused && (
        <rect x={table.x - 3} y={table.y - 3} width={table.width + 6} height={table.height + 6}
          rx={10} fill="none" stroke="rgba(51,153,255,0.35)" strokeWidth={3} strokeDasharray="8 3" />
      )}
      {table.shape === 'round' ? (
        <ellipse cx={cx} cy={cy} rx={table.width / 2} ry={table.height / 2}
          fill={table.fused ? 'rgba(51,153,255,0.12)' : `${color}22`}
          stroke={selected ? color : table.fused ? '#3399FF' : `${color}88`}
          strokeWidth={selected ? 2.5 : table.fused ? 2 : 1.5}
          style={{ filter: selected || table.fused ? `drop-shadow(0 0 10px ${table.fused ? '#3399FF88' : color + '66'})` : 'none' }}
        />
      ) : (
        <rect x={table.x} y={table.y} width={table.width} height={table.height}
          rx={table.fused ? 10 : table.shape === 'square' ? 8 : 6}
          fill={table.fused ? 'rgba(51,153,255,0.10)' : `${color}22`}
          stroke={selected ? color : table.fused ? '#3399FF' : `${color}88`}
          strokeWidth={selected ? 2.5 : table.fused ? 2 : 1.5}
          style={{ filter: selected || table.fused ? `drop-shadow(0 0 10px ${table.fused ? '#3399FF88' : color + '66'})` : 'none' }}
        />
      ...
      {table.fused && table.fusedFrom?.length === 2 && (() => {
        ...
        return isH
          ? <line x1={table.x + fa.width} y1={table.y + 6} x2={table.x + fa.width} y2={table.y + table.height - 6}
              stroke="rgba(51,153,255,0.4)" strokeWidth={1.5} strokeDasharray="4 3" />
          : <line x1={table.x + 6} y1={table.y + fa.height} x2={table.x + table.width - 6} y2={table.y + fa.height}
              stroke="rgba(51,153,255,0.4)" strokeWidth={1.5} strokeDasharray="4 3" />
      })()}
      <text x={cx} y={cy - 5} textAnchor="middle" dominantBaseline="middle"
        transform={`rotate(${-table.rotation}, ${cx}, ${cy})`}
        fill={table.fused ? '#3399FF' : color} fontSize={table.fused ? 10 : 11} fontWeight="bold"
        style={{ userSelect: 'none' }}>{table.label}</text>
      <text x={cx} y={cy + 9} textAnchor="middle" dominantBaseline="middle"
        transform={`rotate(${-table.rotation}, ${cx}, ${cy})`}
        fill={table.fused ? '#3399FFbb' : `${color}bb`} fontSize={9}
      ...
      {table.fused && (
        ...
        <g transform={`translate(${table.x + table.width - 16}, ${table.y + 4})`}>
          <rect x={-2} y={-2} width={16} height={16} rx={4}
            fill="rgba(51,153,255,0.2)" stroke="rgba(51,153,255,0.5)" strokeWidth={1} />
          <g transform="translate(1,1)" stroke="#3399FF" strokeWidth="1.5" fill="none" strokeLinecap="round">
      ...
      {table.wallMagnet && !table.fused && (
        <g transform={`translate(${table.x + 4}, ${table.y + table.height - 4})`}>
          <circle r={4} fill="rgba(0,180,255,0.3)" stroke="#00B4FF" strokeWidth={1} />
          <circle r={1.5} fill="#00B4FF" />
        </g>
      )}
      {table.locked && (
          ...
          <div className="flex gap-2">
            {([['square', 'Cuadrada', Square], ['round', 'Redonda', Circle], ['rectangle', 'Rectangular', RectangleHorizontal]] as const).map(([s, lbl, Icon]) => (
              <button key={s} onClick={() => setShape(s)}
                ...
                style={{
                  background: shape === s ? 'rgba(0,102,255,0.15)' : 'rgba(255,255,255,0.04)',
                  border: `1px solid ${shape === s ? 'rgba(0,102,255,0.4)' : '#1A2540'}`,
                  color: shape === s ? '#0066FF' : '#8899BB',
                }}>
                <Icon size={18} />{lbl}
              </button>
            ))}
...
export default function GastroLayout() {
  ...
  const [zones, setZones] = useState<Zone[]>([
    {
      id: 'z1', name: 'Salón principal', color: '#0066FF',
      spacePath: {
        points: [
    ...
    {
      id: 'z2', name: 'Terraza', color: '#3399FF',
    ...
    <div className="flex flex-col h-full space-y-3">
      ...
      <div className="grid grid-cols-6 gap-3 flex-shrink-0">
        {[
          ...
          { label: 'Fusionadas', value: fusedCount, color: '#3399FF' },
          { label: 'Disponibles', value: available, color: '#10b981' },
          { label: 'Ocupadas', value: occupied, color: '#0066FF' },
          { label: 'Reservadas', value: reserved, color: '#f59e0b' },
          { label: 'Capacidad', value: `${totalCap.min}–${totalCap.max}p`, color: '#3399FF' },
        ].map(s => (
      ...
      <div className="flex gap-4 flex-1 min-h-0">
        ...
        <div className="flex flex-col gap-2 flex-shrink-0 w-44 overflow-y-auto">
          ...
          <div className="card-gastro p-3 space-y-1">
            ...
            {([
              ...
              <button key={t} onClick={() => setTool(t as ToolMode)}
                ...
                style={{
                  background: tool === t ? 'rgba(0,102,255,0.15)' : 'rgba(255,255,255,0.03)',
                  border: `1px solid ${tool === t ? 'rgba(0,102,255,0.4)' : '#1A2540'}`,
                  color: tool === t ? '#0066FF' : '#8899BB',
                }}>
                <Icon size={14} />{lbl}
              </button>
            ))}
            ...
            {tool === 'curveEdge' && (
              <p className="text-xs mt-2 leading-relaxed px-1" style={{ color: '#00B4FF' }}>
          ...
          {selectedTable && (
            <div className="card-gastro p-3 space-y-2 animate-slide-down">
              ...
              <div className="text-sm font-bold" style={{ color: selectedTable.fused ? '#3399FF' : '#E8F0FF' }}>
                {selectedTable.label}
              </div>
              ...
              {selectedTable.fused ? (
                <>
                  ...
                  <div className="space-y-1 pt-1">
                    <button onClick={toggleWallMagnet} className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-xs transition-all"
                      style={{ color: selectedTable.wallMagnet ? '#00B4FF' : '#8899BB' }}>
                ...
                <div className="space-y-1 pt-1">
                  ...
                  <button onClick={toggleMagnet} className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-xs transition-all"
                    style={{ color: selectedTable.magnetEnabled ? '#0066FF' : '#8899BB' }}>
          ...
          <div className="card-gastro p-3">
            ...
            <div className="flex items-center gap-2 mt-2">
              <div className="w-2.5 h-2.5 rounded-sm border border-dashed" style={{ borderColor: '#3399FF' }} />
              <span className="text-xs text-gastro-subtle">Fusionada</span>
            </div>
            <div className="flex items-center gap-2 mt-1">
              <div className="w-2.5 h-2.5 rounded-full" style={{ background: 'rgba(0,180,255,0.4)', border: '1px solid #00B4FF' }} />
        ...
        <div className="flex-1 rounded-2xl overflow-hidden relative" style={{ background: '#0A0F1A', border: `1px solid ${activeZone.color}22` }}>
          ...
          <div className="absolute top-3 left-1/2 -translate-x-1/2 z-10 pointer-events-none flex flex-col items-center gap-2">
            ...
            {tool === 'curveEdge' && (
              <div className="px-4 py-2 rounded-xl text-xs font-semibold"
                style={{ background: 'rgba(0,180,255,0.15)', border: '1px solid rgba(0,180,255,0.4)', color: '#00B4FF' }}>
                〜 Arrastra un borde para curvarlo · Doble clic en el control para eliminar
              </div>
            )}
            {pendingFuse && (
              <div className="px-4 py-2 rounded-xl text-xs font-semibold"
                style={{ background: 'rgba(51,153,255,0.2)', border: '1px solid rgba(51,153,255,0.5)', color: '#3399FF' }}>
                🔗 Suelta para fusionar mesas
              </div>
            )}
            {wallSnapActive && (
              <div className="px-4 py-2 rounded-xl text-xs font-semibold"
                style={{ background: 'rgba(0,180,255,0.2)', border: '1px solid rgba(0,180,255,0.5)', color: '#00B4FF' }}>
                🧲 Pegada a la pared
              </div>
            )}
            {dragging?.wasLocked && (
          ...
          >
            ...
            {dragging && activeSpacePath.points.length > 2 && (
              <path d={spacePathD} fill="none"
                stroke="rgba(0,180,255,0.12)" strokeWidth={wallDistPx * 2} strokeLinejoin="round" />
            )}
                ...
                {hoveredEdgeIndex !== null && (() => {
                  ...
                  return seg?.controlPoint ? (
                    <path
                      ...
                      fill="none"
                      stroke={tool === 'addPoint' ? '#10b981' : '#00B4FF'}
                  ...
                  return (
                    <g key={`cp-${i}`}>
                      <line x1={a.x} y1={a.y} x2={seg.controlPoint.x} y2={seg.controlPoint.y}
                        stroke="rgba(0,180,255,0.35)" strokeWidth={1} strokeDasharray="4 3" />
                      <line x1={b.x} y1={b.y} x2={seg.controlPoint.x} y2={seg.controlPoint.y}
                        stroke="rgba(0,180,255,0.35)" strokeWidth={1} strokeDasharray="4 3" />
                      <circle
                        cx={seg.controlPoint.x} cy={seg.controlPoint.y}
                        r={isActive ? 7 : 5}
                        fill={isActive ? 'rgba(0,180,255,0.25)' : 'rgba(0,180,255,0.12)'}
                        stroke="#00B4FF" strokeWidth={isActive ? 1.5 : 1}
                      ...
                      <path
                        ...
                        fill="#00B4FF" opacity={0.7} style={{ pointerEvents: 'none' }}
                      />
                    </g>
                  )
                })}
                  ...
                  return (
                    <g style={{ pointerEvents: 'none' }}>
                      <circle cx={mx} cy={my} r={8}
                        fill="rgba(0,180,255,0.2)" stroke="rgba(0,180,255,0.6)" strokeWidth={1.5} strokeDasharray="3 2" />
                      <text x={mx} y={my + 1} textAnchor="middle" dominantBaseline="middle"
                        fill="#00B4FF" fontSize={10} fontWeight="bold">↕</text>
                    </g>
                  )
                })()}
              </>
            ...
            {snapHighlight && (() => {
              ...
              return (
                <rect x={t.x - 5} y={t.y - 5} width={t.width + 10} height={t.height + 10}
                  rx={12} fill="none"
                  stroke={pendingFuse ? '#3399FF' : activeZone.color}
