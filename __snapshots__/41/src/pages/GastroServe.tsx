...
const TABLE_STATUS_CONFIG = {
  ...
  occupied: { label: 'Ocupada', color: '#0066FF', bg: 'rgba(0,102,255,0.12)', border: 'rgba(0,102,255,0.3)' },
  reserved: { label: 'Reservada', color: '#f59e0b', bg: 'rgba(245,158,11,0.12)', border: 'rgba(245,158,11,0.3)' },
  cleaning: { label: 'Limpieza', color: '#3399FF', bg: 'rgba(51,153,255,0.12)', border: 'rgba(51,153,255,0.3)' },
}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Mesas ocupadas', value: stats.occupied, color: '#0066FF', icon: Users },
          { label: 'Reservadas', value: stats.reserved, color: '#f59e0b', icon: Clock },
          { label: 'Pedidos activos', value: LIVE_ORDERS.filter(o => o.status !== 'delivered').length, color: '#3399FF', icon: UtensilsCrossed },
        ].map(stat => {
      ...
      <div className="flex items-center gap-2">
        {[
          ...
          <button key={view.id}
            ...
            style={activeView === view.id ? { background: 'rgba(0,102,255,0.15)', border: '1px solid rgba(0,102,255,0.3)' } : { background: 'rgba(255,255,255,0.03)', border: '1px solid #1A2540' }}>
            {view.label}
          </button>
        ))}
      </div>
            ...
            <div className="flex gap-2 flex-wrap">
              {ZONES.map(zone => (
                <button key={zone}
                  ...
                  style={activeZone === zone ? { background: 'rgba(0,102,255,0.15)' } : { background: 'rgba(255,255,255,0.03)' }}>
                  {zone}
                </button>
              ))}
            </div>
            ...
            <tbody>
              {LIVE_ORDERS.map(order => {
                const STATUS_CONFIG_LOCAL = {
                  ...
                  cooking: { label: 'Cocinando', color: '#3399FF', bg: 'rgba(51,153,255,0.15)' },
