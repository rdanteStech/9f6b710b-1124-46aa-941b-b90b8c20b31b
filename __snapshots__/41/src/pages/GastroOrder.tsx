...
const MOCK_WAITERS: Waiter[] = [
  ...
  { id: 'w2', name: 'Valentina', emoji: '👩‍🍳', role: 'waiter', color: '#00B4FF' },
  { id: 'w3', name: 'Rodrigo', emoji: '🧑‍🍳', role: 'waiter', color: '#3399FF' },
]
...
function DestBadge({ dest, small }: { dest: 'kitchen' | 'bar'; small?: boolean }) {
  ...
  if (dest === 'kitchen') return (
    <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-lg font-semibold"
      style={{ background: 'rgba(0,102,255,0.15)', color: '#0066FF', fontSize: small ? '9px' : '10px' }}>
      <ChefHat size={sz} /> Cocina
    </span>
  )
  return (
    <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-lg font-semibold"
      style={{ background: 'rgba(51,153,255,0.15)', color: '#3399FF', fontSize: small ? '9px' : '10px' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{
          ...
          boxShadow: [
            ...
            `0 0 60px ${glowColor ?? '#0066FF'}33`, '0 40px 100px rgba(0,0,0,0.9)',
        ...
        <div style={{
          ...
          <div style={{ padding: '20px 20px 16px', borderBottom: '1px solid #1A2540' }}>
            ...
            <p style={{ fontSize: 12, color: '#8899BB', margin: 0 }}>
              {tableLabel} · Seleccioná tu perfil para asociarte a la mesa
            </p>
          </div>
            ...
            {MOCK_WAITERS.map(w => (
              <button key={w.id} onClick={() => onSelect(w)}
                ...
                <div style={{ flex: 1, textAlign: 'left' }}>
                  ...
                  <div style={{ fontSize: 12, color: w.color }}>Mozo · Osteria Moderna</div>
                </div>
    ...
    <div className="absolute inset-0 z-20 flex flex-col justify-end"
      ...
      <div className="rounded-t-3xl overflow-hidden" style={{ background: '#0F1628', border: '1px solid #1A2540' }}>
        <div className="p-4 border-b" style={{ borderColor: '#1A2540' }}>
          ...
          <p className="text-xs" style={{ color: '#8899BB' }}>{dinerEmoji} {dinerName} · revisá antes de enviar</p>
        </div>
        <div className="p-4 space-y-3 max-h-56 overflow-y-auto">
          {kitchenItems.length > 0 && (
            <div>
              <div className="flex items-center gap-1.5 mb-2">
                <ChefHat size={12} style={{ color: '#0066FF' }} />
                <span className="text-xs font-bold uppercase tracking-wider" style={{ color: '#0066FF' }}>Cocina</span>
              </div>
                ...
                return (
                  <div key={item.menuItemId} className="flex items-center justify-between py-1.5 border-b last:border-0"
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap flex-1">
                      ...
                      {item.shared && <span className="text-xs px-1 py-0.5 rounded" style={{ background: 'rgba(0,180,255,0.15)', color: '#00B4FF', fontSize: '9px' }}>÷{splitCount}</span>}
          ...
          {barItems.length > 0 && (
            <div>
              <div className="flex items-center gap-1.5 mb-2">
                <GlassWater size={12} style={{ color: '#3399FF' }} />
                <span className="text-xs font-bold uppercase tracking-wider" style={{ color: '#3399FF' }}>Barra</span>
              </div>
              {barItems.map(item => {
                const splitCount = item.shared && item.sharedWith ? item.sharedWith.length + 1 : 1
                const myPrice = Math.round(item.price * item.qty / splitCount)
        ...
        <div className="p-4 border-t space-y-3" style={{ borderColor: '#1A2540' }}>
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-lg font-black" style={{ color: isWaiter ? '#f59e0b' : '#0066FF' }}>
    ...
    <div className="absolute inset-0 z-20 flex flex-col justify-end"
      ...
      <div className="rounded-t-3xl overflow-hidden" style={{ background: '#0F1628', border: '1px solid #1A2540' }}>
        <div className="p-4 border-b" style={{ borderColor: '#1A2540' }}>
          ...
          <p className="text-xs mt-0.5" style={{ color: '#8899BB' }}>Seleccioná con quién compartís el costo</p>
        </div>
        <div className="p-4 space-y-2">
          {others.length === 0 ? (
            ...
            return (
              <button key={d.id}
                ...
                style={{
                  background: isSelected ? 'rgba(0,180,255,0.1)' : 'rgba(255,255,255,0.03)',
                  border: `1px solid ${isSelected ? 'rgba(0,180,255,0.4)' : '#1A2540'}`,
                }}>
                <div className="w-5 h-5 rounded-md flex items-center justify-center flex-shrink-0"
                  style={{ background: isSelected ? '#00B4FF' : 'rgba(255,255,255,0.08)', border: `1px solid ${isSelected ? '#00B4FF' : '#2A3550'}` }}>
          ...
          <div className="p-3 rounded-xl mt-2"
            ...
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-black" style={{ color: '#00B4FF' }}>${myPrice.toLocaleString()} c/u</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 space-y-2 max-h-64 overflow-y-auto">
          ...
          {diners.map(d => (
            <button key={d.id} onClick={() => setSelected(d.id)}
              className="w-full flex items-center gap-3 p-3 rounded-xl transition-all"
              style={{
                background: selected === d.id ? 'rgba(51,153,255,0.1)' : 'rgba(255,255,255,0.03)',
                border: `1px solid ${selected === d.id ? 'rgba(51,153,255,0.4)' : '#1A2540'}`,
              }}>
              <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0"
                style={{ background: selected === d.id ? '#3399FF' : 'rgba(255,255,255,0.08)', border: `1px solid ${selected === d.id ? '#3399FF' : '#2A3550'}` }}>
                  ...
                  {session.diners.map(d => {
                    ...
                    return (
                      <div key={d.id} className="flex items-center gap-3 p-3 rounded-xl"
                        ...
                        <div className="text-right">
                          <div className="text-sm font-black" style={{ color: dTotal > 0 ? '#0066FF' : '#4A5A7A' }}>${dTotal.toLocaleString()}</div>
...
function PhoneKiosk({ session, currentDinerId, onJoin, onAddItem, onRemoveItem, onSendRound, onRegisterPayment, onClose }: {
  ...
  return (
    <PhoneShell accentColor="#1A2540" glowColor="#0066FF"
      label={<>Vista del cliente · <span style={{ color: '#0066FF' }}>{session.tableLabel}</span></>}
      ...
      {screen === 'join' && (
        <div className="flex-1 flex flex-col overflow-y-auto" style={{ background: '#0A0F1A' }}>
          <div className="relative overflow-hidden flex-shrink-0"
            style={{ background: 'linear-gradient(160deg, #0A1E40 0%, #0d1a2e 100%)', paddingTop: 24, paddingBottom: 32 }}>
            <div className="absolute inset-0 opacity-30"
              style={{ backgroundImage: 'radial-gradient(circle at 30% 60%, #0066FF 0%, transparent 50%), radial-gradient(circle at 70% 40%, #3399FF 0%, transparent 50%)' }} />
            <div className="relative text-center px-6">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-3"
                style={{ background: 'linear-gradient(135deg, #0066FF, #3399FF)', boxShadow: '0 8px 32px rgba(0,102,255,0.5)' }}>
          ...
          <div className="flex-1 p-5 space-y-4">
            {session.diners.length === 0 && (
              <div className="flex items-start gap-2 p-3 rounded-2xl"
                style={{ background: 'rgba(51,153,255,0.08)', border: '1px solid rgba(51,153,255,0.2)' }}>
            ...
            <button onClick={() => joinName.trim() && onJoin(joinName.trim())} disabled={!joinName.trim()}
              ...
              style={{ background: 'linear-gradient(135deg, #0066FF, #0052cc)', boxShadow: '0 4px 20px rgba(0,102,255,0.4)' }}>
              <UserPlus size={16} />
        ...
        <div className="flex-1 flex flex-col overflow-hidden" style={{ background: '#0A0F1A' }}>
          <div className="flex items-center justify-between px-4 py-3 flex-shrink-0"
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setScreen('cart')}
                className="relative flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold"
                style={{ background: 'rgba(0,102,255,0.15)', border: '1px solid rgba(0,102,255,0.3)', color: '#0066FF' }}>
                <ShoppingCart size={13} />
                {myCart.length > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full text-white flex items-center justify-center font-black"
                    style={{ background: '#0066FF', fontSize: '9px' }}>
            ...
            {CATEGORIES.map(cat => (
              <button key={cat} onClick={() => setActiveCategory(cat)}
                ...
                style={{
                  ...
                  border: `1px solid ${activeCategory === cat ? 'rgba(0,102,255,0.5)' : '#152035'}`,
                  color: activeCategory === cat ? '#0066FF' : '#8899BB',
                }}>
                {CATEGORY_ICONS[cat]}{cat}
              </button>
            ))}
                        ...
                        <div className="flex items-center gap-1">
                          {item.shareable && (
                            <button onClick={() => setShareItem(item)}
                              ...
                              style={{ background: 'rgba(0,180,255,0.12)', color: '#00B4FF', fontSize: '10px' }}>
          ...
          {myCart.length > 0 && (
            <div className="p-4 border-t flex-shrink-0" style={{ borderColor: '#152035' }}>
              <button onClick={() => setScreen('cart')}
                className="w-full py-3.5 rounded-2xl font-bold text-white text-sm flex items-center justify-center gap-2"
                style={{ background: 'linear-gradient(135deg, #0066FF, #0052cc)', boxShadow: '0 4px 20px rgba(0,102,255,0.35)' }}>
      ...
      {screen === 'cart' && (
        <div className="flex-1 flex flex-col overflow-hidden" style={{ background: '#0A0F1A' }}>
          <div className="flex items-center justify-between px-4 py-3 flex-shrink-0"
            style={{ borderBottom: '1px solid #152035' }}>
            <button onClick={() => setScreen('menu')}
              className="flex items-center gap-1 text-xs font-semibold" style={{ color: '#0066FF' }}>
              <ChevronRight size={14} className="rotate-180" /> Menú
            </button>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {myCart.length === 0 && myRounds.length === 0 ? (
              <div className="text-center py-12">
                ...
                <button onClick={() => setScreen('menu')} className="mt-2 text-xs font-semibold" style={{ color: '#0066FF' }}>
                  Ver menú →
                </button>
              </div>
            ) : (
              <>
                      ...
                      return (
                        <div key={item.menuItemId} className="flex items-center justify-between py-1.5 pl-5"
                          ...
                          <div>
                            ...
                            {isShared && <span className="text-xs ml-1" style={{ color: '#00B4FF' }}>÷{splitCount}</span>}
                            ...
                            <div className="flex items-center gap-1.5 mb-0.5">
                              ...
                              {isShared && (
                                <span className="text-xs px-1 py-0.5 rounded"
                                  style={{ background: 'rgba(0,180,255,0.15)', color: '#00B4FF', fontSize: '9px' }}>
                                  ÷{splitCount}
                                </span>
                              )}
                            </div>
                ...
                <div className="p-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid #152035' }}>
                    ...
                    return (
                      <div key={d.id} className="flex items-center justify-between py-1">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-xs" style={{ color: d.id === currentDinerId ? '#0066FF' : '#8899BB' }}>
                  ...
                  <div className="border-t mt-2 pt-2 flex items-center justify-between" style={{ borderColor: '#152035' }}>
                    ...
                    <span className="text-sm font-black" style={{ color: '#0066FF' }}>${sessionTotal.toLocaleString()}</span>
                  </div>
                </div>
              </>
            )}
          ...
          <div className="p-4 border-t space-y-2 flex-shrink-0" style={{ borderColor: '#152035' }}>
            ...
            {(myRounds.length > 0 || myCart.length > 0) && (
              <button onClick={() => { setScreen('payment'); setPayStep('split') }}
                className="w-full py-3 rounded-2xl font-bold text-sm flex items-center justify-center gap-2"
                style={{ background: 'rgba(0,102,255,0.1)', border: '1px solid rgba(0,102,255,0.25)', color: '#0066FF' }}>
        ...
        <div className="flex-1 flex flex-col overflow-hidden" style={{ background: '#0A0F1A' }}>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            <div className="p-4 rounded-2xl text-center"
              style={{ background: 'linear-gradient(135deg, rgba(0,102,255,0.12), rgba(51,153,255,0.08))', border: '1px solid rgba(0,102,255,0.2)' }}>
            ...
            {payStep === 'split' && (
              <>
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: '#8899BB' }}>¿Cómo querés pagar?</p>
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      <button key={id}
                        ...
                        style={{
                          background: splitMode === id ? 'rgba(0,102,255,0.15)' : 'rgba(255,255,255,0.04)',
                          border: `1px solid ${splitMode === id ? 'rgba(0,102,255,0.4)' : '#152035'}`,
                          color: splitMode === id ? '#0066FF' : '#8899BB',
                        }}>
                        <Icon size={15} />
                ...
                {splitMode === 'individual' && (
                  <div className="flex items-center justify-between p-3 rounded-xl font-bold"
                    ...
                    <span className="text-lg" style={{ color: '#0066FF' }}>${myAllTimeTotal.toLocaleString()}</span>
                  </div>
                )}
                {splitMode === 'equal' && (
                  <div className="p-3 rounded-xl text-xs"
                    style={{ background: 'rgba(51,153,255,0.08)', border: '1px solid rgba(51,153,255,0.2)', color: '#7dd3fc' }}>
                      ...
                      return (
                        <button key={d.id}
                          ...
                          style={{
                            ...
                            border: `1px solid ${isSelected ? 'rgba(0,102,255,0.4)' : '#152035'}`,
                          }}>
                          <div className="w-4 h-4 rounded flex items-center justify-center"
                            style={{ background: isSelected ? '#0066FF' : 'rgba(255,255,255,0.08)' }}>
                            {isSelected && <Check size={10} className="text-white" />}
                          </div>
                          <span>{d.emoji}</span>
                          <span className="text-sm font-semibold text-white flex-1 text-left">{d.name}</span>
                          {d.id === currentDinerId && <span className="text-xs" style={{ color: '#0066FF' }}>(vos)</span>}
                        </button>
                      )
                    })}
                  </div>
                ...
                <button onClick={() => setPayStep('method')}
                  ...
                  className="w-full py-4 rounded-2xl font-bold text-white text-sm flex items-center justify-center gap-2 disabled:opacity-40"
                  style={{ background: 'linear-gradient(135deg, #0066FF, #0052cc)', boxShadow: '0 4px 20px rgba(0,102,255,0.3)' }}>
            ...
            {payStep === 'processing' && (
              <div className="text-center py-10">
                <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-5 animate-pulse"
                  style={{ background: 'rgba(0,102,255,0.15)', border: '2px solid rgba(0,102,255,0.3)' }}>
                  <CreditCard size={28} style={{ color: '#0066FF' }} />
                </div>
...
export default function GastroOrder() {
  const [sessions, setSessions] = useState<TableSession[]>([
    {
      ...
      diners: [
        ...
        },
        {
          ...
          rounds: [
            { id: 'r2', sentAt: '20:25', destination: 'bar', items: [{ menuItemId: 'm10', name: 'Limonada Artesanal', price: 850, qty: 1, destination: 'bar' }] },
    ...
    },
    {
      ...
      waiters: [],
      diners: [
        { id: 'd4', name: 'Ana', emoji: '🧕', paid: false, joinedAt: '20:30', cart: [], rounds: [{ id: 'r4', sentAt: '20:35', destination: 'kitchen', items: [{ menuItemId: 'm1', name: 'Burrata con Tomates', price: 1850, qty: 1, destination: 'kitchen' }] }] },
        { id: 'd5', name: 'Pedro', emoji: '🧔', paid: false, joinedAt: '20:31', cart: [], rounds: [{ id: 'r5', sentAt: '20:35', destination: 'kitchen', items: [{ menuItemId: 'm6', name: 'Pasta Carbonara', price: 2900, qty: 1, destination: 'kitchen' }] }] },
      ]
    },
  ])
  ...
  const STATUS_CFG = {
    ordering: { label: 'Ordenando', color: '#0066FF', bg: 'rgba(0,102,255,0.15)' },
    sent:     { label: 'En cocina', color: '#3399FF', bg: 'rgba(51,153,255,0.15)' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Sesiones activas', value: activeSessions.length, color: '#0066FF', icon: Wifi },
          { label: 'Comensales online', value: totalDiners, color: '#3399FF', icon: Users },
          ...
          {[{ id: 'sessions', label: 'Sesiones activas' }, { id: 'qr', label: 'QR por mesa' }].map(v => (
            <button key={v.id} onClick={() => setActiveView(v.id as typeof activeView)}
              ...
              style={activeView === v.id
                ? { background: 'rgba(0,102,255,0.15)', border: '1px solid rgba(0,102,255,0.3)', color: '#0066FF' }
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {session.openedByFirstScan && (
                          <span className="text-xs px-1.5 py-0.5 rounded-lg"
                            style={{ background: 'rgba(51,153,255,0.1)', color: '#3399FF', fontSize: '10px' }}>Auto-QR</span>
                        )}
                        {session.waiters.map(w => (
                          ...
                          <div key={round.id} className="pl-6 mb-2">
                            ...
                            {round.items.map(item => (
                              <div key={item.menuItemId} className="flex items-center justify-between text-xs py-0.5">
                                <span style={{ color: '#8899BB' }}>
                                  {item.qty}× {item.name}
                                  {item.shared && <span style={{ color: '#00B4FF' }}> (compartido)</span>}
                ...
                <div className="flex items-center justify-between pt-3 border-t" style={{ borderColor: '#1A2540' }}>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { setKioskSession(session); setKioskDinerId(null); setKioskMode('customer') }}
                      ...
                      style={{ background: 'rgba(51,153,255,0.12)', border: '1px solid rgba(51,153,255,0.3)', color: '#3399FF' }}>
            ...
            return (
              <div key={table.number} className="card-gastro">
                <div className="flex items-start justify-between mb-4">
                  ...
                  {activeSession ? (
                    <span className="text-xs font-semibold px-2 py-1 rounded-lg" style={{ background: 'rgba(0,102,255,0.15)', color: '#0066FF' }}>Sesión activa</span>
                  ) : (
