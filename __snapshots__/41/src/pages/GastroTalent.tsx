...
const INITIAL_ORG: OrgNode = {
  ...
  department: 'Dirección', avatar: 'MR', color: '#0066FF', status: 'active',
  reports: [
    {
      id: '2', name: 'Sofía Herrera', role: 'Gerente General',
      department: 'Gerencia', avatar: 'SH', color: '#3399FF', status: 'active',
      reports: [
        {
        ...
        },
        {
          ...
          department: 'Salón', avatar: 'VC', color: '#00B4FF', status: 'active',
          reports: [
            { id: '7', name: 'María González', role: 'Moza Senior', department: 'Salón', avatar: 'MG', color: '#00B4FF', status: 'active' },
            { id: '8', name: 'Roberto Sosa', role: 'Mozo', department: 'Salón', avatar: 'RS', color: '#00B4FF', status: 'vacation' },
            { id: '9', name: 'Camila Torres', role: 'Moza', department: 'Salón', avatar: 'CT', color: '#00B4FF', status: 'active' },
          ]
        },
        {
  ...
  return createPortal(
    ...
    >
      <div
        ...
        style={{ background: '#0F1628', border: '1px solid #1A2540', boxShadow: '0 24px 80px rgba(0,0,0,0.6)' }}
      >
        {/* Header */}
        ...
        <div className="space-y-3">
          <div>
            ...
            <input
              ...
              style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid #1A2540', focusRingColor: '#0066FF' }}
              value={form.name}
              ...
              {DEPARTMENTS.map(dept => (
                <button
                  ...
                  style={form.department === dept
                    ...
                    : { background: 'rgba(255,255,255,0.03)', border: '1px solid #1A2540', color: '#6b7280' }
                  }
                >
                  {dept}
                </button>
        ...
        <div className="flex gap-3 pt-1">
          ...
          <button
            ...
            style={{ background: 'linear-gradient(135deg, #0066FF, #7c5ce0)' }}
          >
...
function AddPositionModal({
  ...
  const handleSubmit = () => {
    ...
    const color = DEPT_COLORS[form.department] ?? '#0066FF'
      ...
      >
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <select
              className="w-full px-4 py-2.5 rounded-xl text-sm text-gastro-text outline-none"
              style={{ background: '#152035', border: '1px solid #1A2540' }}
              value={form.parentId}
              onChange={e => setForm(f => ({ ...f, parentId: e.target.value }))}
            >
              {allNodes.map(n => (
                <option key={n.id} value={n.id} style={{ background: '#0F1628' }}>
                  {n.name} — {n.role}
                </option>
              ))}
            </select>
        ...
        <div className="flex gap-3 pt-1">
          ...
          <button
            ...
            style={{
              background: isValid ? 'linear-gradient(135deg, #0066FF, #7c5ce0)' : 'rgba(0,102,255,0.2)',
...
function EditStructurePanel({
  ...
  return createPortal(
    ...
    >
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between sticky top-0 pb-2" style={{ background: '#0F1628' }}>
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'rgba(0,102,255,0.15)' }}>
...
function NodeEditRow({
  ...
  return (
    <div
      ...
      style={{ border: '1px solid #1A2540', background: 'rgba(255,255,255,0.02)' }}
    >
      {/* Row header */}
      ...
      {/* Inline edit form */}
      {editing && (
        <div className="px-3 pb-3 space-y-2 border-t" style={{ borderColor: '#1A2540' }}>
    ...
    <div className="flex flex-col items-center">
      <div className="relative group">
        <div
          ...
          style={{
            background: isRoot
              ? `linear-gradient(135deg, rgba(0,102,255,0.18), rgba(51,153,255,0.10))`
              : 'rgba(255,255,255,0.03)',
            border: `1px solid ${isRoot ? 'rgba(0,102,255,0.4)' : '#1A2540'}`,
            minWidth: isRoot ? '200px' : '160px',
            boxShadow: isRoot ? '0 0 24px rgba(0,102,255,0.15)' : 'none',
          }}
      ...
      {hasReports && expanded && (
        <div className="flex flex-col items-center mt-6">
          <div className="w-px h-6" style={{ background: '#1A2540' }} />
...
function DeptLegend({ orgData }: { orgData: OrgNode }) {
  const allNodes = flattenNodes(orgData)
  const depts = [
    { name: 'Dirección', color: '#0066FF', icon: Crown },
    { name: 'Gerencia', color: '#3399FF', icon: Building2 },
    { name: 'Cocina', color: '#f59e0b', icon: ChefHat },
    { name: 'Salón', color: '#00B4FF', icon: UtensilsCrossed },
...
export default function GastroTalent() {
  ...
  const deptStats = [
    { dept: 'Dirección', color: '#0066FF', icon: Crown },
    { dept: 'Gerencia', color: '#3399FF', icon: Building2 },
    { dept: 'Cocina', color: '#f59e0b', icon: ChefHat },
    { dept: 'Salón', color: '#00B4FF', icon: UtensilsCrossed },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Horas esta semana', value: '284', color: '#0066FF', icon: Clock },
          { label: 'Satisfacción promedio', value: '4.7', color: '#f59e0b', icon: Star },
          { label: 'Vacantes abiertas', value: JOB_LISTINGS.length, color: '#3399FF', icon: Briefcase },
        ].map(stat => {
      ...
      {activeView === 'recruitment' && (
        <div className="space-y-4">
          ...
          <div className="rounded-2xl p-6 relative overflow-hidden"
            style={{ background: 'linear-gradient(135deg, rgba(0,180,255,0.08) 0%, rgba(0,102,255,0.06) 100%)', border: '1px solid rgba(0,180,255,0.2)' }}>
      ...
      {activeView === 'organigrama' && (
        <div className="space-y-4">
          <div className="card-gastro">
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div>
                <div className="flex items-center gap-3 mb-1">
                  <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: 'rgba(0,102,255,0.15)' }}>
            ...
            <div className="mt-4 pt-4" style={{ borderTop: '1px solid #1A2540' }}>
