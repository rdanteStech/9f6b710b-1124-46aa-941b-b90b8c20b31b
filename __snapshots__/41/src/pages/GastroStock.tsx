...
import {
  ...
  Search, Filter, ArrowUpRight, Truck, BarChart3, RefreshCw
} from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Insumos totales', value: INVENTORY_ITEMS.length, color: '#0066FF', icon: Package },
          ...
          { label: 'Órdenes pendientes', value: 2, color: '#3399FF', icon: ShoppingCart },
        ].map(stat => {
        ...
        <div className="flex items-center gap-3 p-4 rounded-xl"
          ...
          <div className="flex-1">
            ...
            <span className="text-sm text-gastro-subtle">
              {INVENTORY_ITEMS.filter(i => i.status === 'critical').map(i => i.name).join(', ')} — Se recomienda generar orden de compra inmediata.
            </span>
          </div>
                    ...
                    <td className="w-32">
                      <div className="progress-bar">
                        <div className="progress-fill" style={{
                          width: `${pct}%`,
                          background: item.status === 'critical' ? '#ef4444' : item.status === 'low' ? '#f59e0b' : 'linear-gradient(90deg, #0066FF, #3399FF)'
                        }} />
                      </div>
