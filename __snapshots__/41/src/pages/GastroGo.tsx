...
const STATUS_CFG = {
  ...
  preparing:  { label: 'Preparando',   color: '#3399FF', bg: 'rgba(51,153,255,0.12)'  },
  ready:      { label: 'Listo',        color: '#0066FF', bg: 'rgba(0,102,255,0.12)' },
  delivering: { label: 'En camino',    color: '#00B4FF', bg: 'rgba(0,180,255,0.12)' },
  delivered:  { label: 'Entregado',    color: '#10b981', bg: 'rgba(16,185,129,0.12)'  },
}

const CHANNEL_CFG = {
  app:       { label: 'App',       color: '#0066FF', emoji: '📱' },
  web:       { label: 'Web',       color: '#3399FF', emoji: '🌐' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl p-6 relative overflow-hidden"
        style={{
          background: 'linear-gradient(135deg, rgba(0,102,255,0.08) 0%, rgba(51,153,255,0.06) 50%, rgba(16,185,129,0.04) 100%)',
        ...
        <div className="relative z-10 flex items-center gap-4 flex-wrap">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0"
            style={{ background: 'linear-gradient(135deg, #0066FF, #3399FF)', boxShadow: '0 0 30px rgba(0,102,255,0.4)' }}>
          ...
          <div className="hidden md:grid grid-cols-3 gap-6 text-center flex-shrink-0">
            {[
              ...
              <div key={s.label}>
                <div className="text-xl font-black" style={{ color: '#0066FF' }}>{s.value}</div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Pedidos hoy', value: orders.length, color: '#0066FF', icon: ShoppingBag, sub: `${activeOrders.length} activos` },
          { label: 'Facturación online', value: `$${(todayRevenue / 1000).toFixed(1)}K`, color: '#10b981', icon: DollarSign, sub: 'hoy' },
          { label: 'Tiempo promedio', value: '28 min', color: '#3399FF', icon: Timer, sub: 'entrega' },
          { label: 'Clientes recurrentes', value: '68%', color: '#00B4FF', icon: Repeat, sub: 'este mes' },
        ].map(stat => {
        ...
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id as typeof activeTab)}
            ...
            style={activeTab === tab.id
              ? { background: 'rgba(0,102,255,0.15)', border: '1px solid rgba(0,102,255,0.35)', color: '#0066FF' }
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="card-gastro flex flex-col items-center">
            ...
            <div className="w-48 rounded-3xl overflow-hidden shadow-2xl"
              style={{ background: '#000000', border: '3px solid #1A2540', boxShadow: '0 0 40px rgba(0,102,255,0.2)' }}>
              {/* Status bar */}
              ...
              {/* App header */}
              <div className="px-3 py-3" style={{ background: 'linear-gradient(135deg, #0066FF22, #3399FF22)' }}>
            ...
            <div className="flex gap-2 mt-4 w-full">
              ...
              <button className="flex-1 py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5"
                style={{ background: 'rgba(51,153,255,0.1)', border: '1px solid rgba(51,153,255,0.25)', color: '#3399FF' }}>
          ...
          <div className="card-gastro">
            ...
            <div className="space-y-3">
              {[
                { name: 'App GastroGo', emoji: '📱', orders: 48, revenue: 124000, active: true, color: '#0066FF' },
                { name: 'Web propia', emoji: '🌐', orders: 31, revenue: 87000, active: true, color: '#3399FF' },
                ...
                { name: 'Rappi', emoji: '🟠', orders: 19, revenue: 54000, active: false, color: '#00B4FF' },
              ].map(ch => (
          ...
          <div className="lg:col-span-2 rounded-2xl p-5 relative overflow-hidden"
            style={{ background: 'linear-gradient(135deg, rgba(0,102,255,0.1), rgba(51,153,255,0.06))', border: '1px solid rgba(0,102,255,0.25)' }}>
            <div className="absolute right-0 top-0 w-48 h-48 opacity-10"
              style={{ background: 'radial-gradient(circle, #0066FF, transparent)', transform: 'translate(20%, -20%)' }} />
            <div className="relative z-10">
              ...
              <div className="grid grid-cols-3 gap-4">
                {[
                  ...
                  { label: 'Comisión GastroGo', value: '0%', color: '#0066FF' },
          ...
          <div className="space-y-4">
            ...
            {zones.map(zone => (
              <div key={zone.id} className="card-gastro flex items-center gap-4"
                style={{ border: `1px solid ${zone.active ? 'rgba(0,102,255,0.25)' : '#1A2540'}` }}>
                <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                  style={{ background: zone.active ? 'rgba(0,102,255,0.12)' : 'rgba(255,255,255,0.04)' }}>
                  <MapPin size={18} style={{ color: zone.active ? '#0066FF' : '#4A5A7A' }} />
                </div>
                <div className="flex-1 min-w-0">
            ...
            <div className="space-y-4">
              ...
              <div className="pt-2">
                <button className="w-full py-3 rounded-xl text-sm font-bold"
                  style={{ background: 'linear-gradient(135deg, rgba(0,102,255,0.15), rgba(51,153,255,0.1))', border: '1px solid rgba(0,102,255,0.3)', color: '#0066FF' }}>
            ...
            <div className="space-y-2">
              {sections.map(section => (
                <div key={section.id} className="flex items-center gap-3 p-3 rounded-xl transition-all"
                  style={{ background: section.active ? 'rgba(0,102,255,0.05)' : 'rgba(255,255,255,0.02)', border: `1px solid ${section.active ? 'rgba(0,102,255,0.2)' : '#1A2540'}` }}>
      ...
      {activeTab === 'analytics' && (
        <div className="grid lg:grid-cols-2 gap-6">
          <div className="card-gastro">
            ...
            <div className="space-y-3">
              {[
                { label: 'App GastroGo', value: 312, total: 580, color: '#0066FF', emoji: '📱' },
                { label: 'Web propia', value: 168, total: 580, color: '#3399FF', emoji: '🌐' },
          ...
          <div className="card-gastro">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: 'Descargas totales', value: '4.284', color: '#0066FF' },
                { label: 'Usuarios activos/mes', value: '1.842', color: '#3399FF' },
                { label: 'Tasa de conversión', value: '7.3%', color: '#10b981' },
                { label: 'Ticket promedio', value: '$3.840', color: '#f59e0b' },
                { label: 'Reseñas positivas', value: '94%', color: '#00B4FF' },
                { label: 'Tiempo en app', value: '4.2 min', color: '#0066FF' },
              ].map(m => (
              ...
              {[42, 68, 55, 89, 74, 96, 81].map((val, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full rounded-t-lg transition-all"
                    style={{ height: `${(val / 96) * 100}%`, background: `linear-gradient(to top, #0066FF, #3399FF)`, opacity: i === 5 ? 1 : 0.5 }} />
