...
const CATEGORY_META: Record<IntegrationCategory, { label: string; icon: typeof Truck; color: string }> = {
  ...
  pos: { label: 'POS & Hardware', icon: Monitor, color: '#3399FF' },
  marketing: { label: 'Marketing', icon: Megaphone, color: '#00B4FF' },
  accounting: { label: 'Contabilidad', icon: Calculator, color: '#0066FF' },
  reservations: { label: 'Reservas', icon: Calendar, color: '#3399FF' },
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl p-6 relative overflow-hidden"
        style={{ background: 'linear-gradient(135deg, rgba(136,136,170,0.12) 0%, rgba(0,102,255,0.08) 100%)', border: '1px solid rgba(136,136,170,0.25)' }}>
        <div className="absolute top-0 right-0 w-56 h-56 opacity-15"
          style={{ background: 'radial-gradient(circle, #8899BB 0%, transparent 70%)', transform: 'translate(30%, -30%)' }} />
        <div className="relative z-10 flex flex-col md:flex-row md:items-center gap-4">
          <div className="flex items-center gap-4 flex-1">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0"
              style={{ background: 'linear-gradient(135deg, #8899BB, #0066FF)', boxShadow: '0 0 30px rgba(136,136,170,0.35)' }}>
          ...
          <div className="grid grid-cols-3 gap-4 md:gap-6 text-center">
            {[
              ...
              <div key={stat.label}>
                <div className="text-xl font-black" style={{ color: '#8899BB' }}>{stat.value}</div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'API keys activas', value: API_KEYS.filter(k => k.active).length, color: '#0066FF', icon: Key },
          { label: 'Uptime sync', value: '99.7%', color: '#3399FF', icon: Activity },
        ].map(stat => {
              ...
              <div key={key.id} className="card-gastro">
                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                    style={{ background: key.active ? 'rgba(0,102,255,0.12)' : 'rgba(255,255,255,0.04)' }}>
                    <Key size={18} style={{ color: key.active ? '#0066FF' : '#8899BB' }} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
