...
const WAITERS_PERF: WaiterPerf[] = [
  ...
  { id: 'w2', name: 'Valentina', emoji: '👩‍🍳', color: '#00B4FF', avgResponseTime: 2.1, alertsResolved: 28, rating: 4.7 },
  { id: 'w3', name: 'Rodrigo', emoji: '🧑‍🍳', color: '#3399FF', avgResponseTime: 2.6, alertsResolved: 21, rating: 4.5 },
]
...
const ALERT_CONFIG: Record<AlertType, { label: string; icon: React.ComponentType<{size?:number;style?:React.CSSProperties}>; color: string; bg: string; emoji: string }> = {
  empty_glass:  { label: 'Vaso vacío',        icon: GlassWater,      color: '#3399FF', bg: 'rgba(51,153,255,0.12)',   emoji: '🥤' },
  empty_plate:  { label: 'Plato vacío',        icon: UtensilsCrossed, color: '#0066FF', bg: 'rgba(0,102,255,0.12)', emoji: '🍽️' },
...
const PRIORITY_CONFIG: Record<AlertPriority, { label: string; color: string; bg: string; pulse: boolean }> = {
  ...
  medium:   { label: 'Media',    color: '#0066FF', bg: 'rgba(0,102,255,0.15)', pulse: false },
...
const INITIAL_ALERTS: EyeAlert[] = [
  ...
  },
  {
    ...
    assignedTo: 'w2', assignedWaiter: { name: 'Valentina', emoji: '👩‍🍳', color: '#00B4FF' },
  ...
  },
  {
    ...
    assignedTo: 'w3', assignedWaiter: { name: 'Rodrigo', emoji: '🧑‍🍳', color: '#3399FF' },
    thumbnail: 'https://images.pexels.com/photos/1267320/pexels-photo-1267320.jpeg?w=300',
  },
  {
      ...
      <div className="p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 p-2 rounded-xl"
              ...
              <Zap size={11} style={{ color: '#0066FF', flexShrink: 0 }} />
              <span className="text-xs font-semibold" style={{ color: '#0066FF' }}>{alert.actionSuggested}</span>
            </div>
          </div>
        ...
        {showAssign && alert.status === 'new' && (
          <div className="mt-3 p-3 rounded-xl space-y-2"
            ...
            <p className="text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: '#0066FF' }}>
              Asignar a mozo
            </p>
            {WAITERS_PERF.map(w => (
              <button key={w.id}
...
function CameraCard({ cam, isSelected, onClick }: {
  ...
  return (
    <button onClick={onClick}
      ...
      style={{
        border: `2px solid ${isSelected ? '#00B4FF' : cam.status === 'offline' ? '#1A2540' : '#1A2540'}`,
        boxShadow: isSelected ? '0 0 20px rgba(0,180,255,0.2)' : 'none',
      }}>
          ...
          <>
            {[['top-2 left-2', 'border-t-2 border-l-2'], ['top-2 right-2', 'border-t-2 border-r-2'],
              ...
              <div key={i} className={`absolute ${pos} w-4 h-4 ${border}`}
                style={{ borderColor: '#00B4FF', opacity: 0.7 }} />
            ))}
          </>
        )}
      ...
      <div className="p-3" style={{ background: '#141420' }}>
        <div className="flex items-center justify-between mb-1">
          ...
          {isSelected && <Eye size={12} style={{ color: '#00B4FF' }} />}
        </div>
    ...
    <div className="space-y-6">
      ...
      <style>{`
        ...
        @keyframes flash-border {
          ...
          50% { box-shadow: 0 0 0 2px rgba(0,180,255,0.6), 0 0 20px rgba(0,180,255,0.2); }
        }
      `}</style>
      ...
      <div className="rounded-2xl p-6 relative overflow-hidden"
        style={{
          background: 'linear-gradient(135deg, rgba(0,180,255,0.08) 0%, rgba(0,102,255,0.06) 50%, rgba(51,153,255,0.04) 100%)',
          border: `1px solid ${newAlertFlash ? 'rgba(0,180,255,0.5)' : 'rgba(0,180,255,0.2)'}`,
          transition: 'border-color 0.3s ease',
        }}>
        <div className="absolute top-0 right-0 w-64 h-64 opacity-10 pointer-events-none"
          style={{ background: 'radial-gradient(circle, #00B4FF 0%, transparent 70%)', transform: 'translate(30%, -30%)' }} />

        <div className="relative z-10 flex items-center gap-4 flex-wrap">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 relative"
            style={{ background: 'linear-gradient(135deg, #00B4FF, #0066FF)', boxShadow: '0 0 30px rgba(0,180,255,0.4)' }}>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button onClick={() => setSoundEnabled(!soundEnabled)}
              ...
              style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid #1A2540', color: soundEnabled ? '#0066FF' : '#6666aa' }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
        {[
          ...
          { label: 'Confianza IA', value: `${avgConfidence}%`, color: '#0066FF', icon: Brain, urgent: false },
          { label: 'T. respuesta', value: `${avgResponseTime.toFixed(1)}m`, color: '#3399FF', icon: Clock, urgent: false },
        ].map(stat => {
        ...
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id as typeof activeTab)}
            ...
            style={activeTab === tab.id
              ? { background: 'rgba(0,180,255,0.15)', border: '1px solid rgba(0,180,255,0.35)', color: '#00B4FF' }
              : { background: 'rgba(255,255,255,0.03)', border: '1px solid #1A2540', color: '#8899BB' }}>
            {tab.label}
            {tab.count !== null && tab.count > 0 && (
              <span className="text-xs px-1.5 py-0.5 rounded-full font-black"
                style={{ background: activeTab === tab.id ? 'rgba(0,180,255,0.25)' : 'rgba(255,255,255,0.08)', color: activeTab === tab.id ? '#00B4FF' : '#8899BB' }}>
                {tab.count}
              </span>
            )}
          </button>
          ...
          <div className="space-y-4">
            ...
            <div className="card-gastro">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-bold text-gastro-text text-sm flex items-center gap-2">
                  <Camera size={14} style={{ color: '#00B4FF' }} /> Feed en vivo
                </h3>
                  ...
                  <div>
                        ...
                        <>
                          ...
                          {[['top-2 left-2', 'border-t-2 border-l-2'], ['top-2 right-2', 'border-t-2 border-r-2'],
                            ...
                            <div key={i} className={`absolute ${pos} w-5 h-5 ${border}`}
                              style={{ borderColor: '#00B4FF', opacity: 0.8 }} />
                          ))}
                          {/* Detection boxes simulation */}
                          <div className="absolute" style={{ top: '30%', left: '20%', width: '25%', height: '35%', border: '1.5px solid #3399FF', borderRadius: 4 }}>
                            <div className="absolute -top-4 left-0 text-xs font-bold px-1 rounded"
                              style={{ background: 'rgba(51,153,255,0.8)', color: '#fff', fontSize: '8px', whiteSpace: 'nowrap' }}>
                              🥤 Vaso vacío 94%
                            </div>
                          </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      ...
                      <div className="p-2 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid #1A2540' }}>
                        ...
                        <div className="font-bold" style={{ color: '#00B4FF' }}>{cam.alertsToday}</div>
                      </div>
                    </div>
                  </div>
                )
            ...
            <div className="card-gastro">
              <h3 className="font-bold text-gastro-text text-sm mb-3 flex items-center gap-2">
                <Users size={14} style={{ color: '#0066FF' }} /> Mozos en turno
              </h3>
              <div className="space-y-3">
                {WAITERS_PERF.map(w => (
            ...
            <div className="card-gastro">
              <h3 className="font-bold text-gastro-text text-sm mb-3 flex items-center gap-2">
                <Brain size={14} style={{ color: '#00B4FF' }} /> Patrones detectados
              </h3>
        ...
        <div className="space-y-6">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {[
              ...
              { label: 'Alertas resueltas hoy', value: `${resolvedCount}/${alerts.length}`, sub: `${Math.round(resolvedCount / alerts.length * 100)}% tasa de resolución`, color: '#0066FF', icon: CheckCircle },
              { label: 'Confianza promedio IA', value: `${avgConfidence}%`, sub: 'Modelo v2.4 · 6 cámaras', color: '#00B4FF', icon: Brain },
            ].map(kpi => {
              const Icon = kpi.icon
              return (
              ...
              <div className="flex items-end gap-2 h-32">
                {hourlyData.map(d => (
                  <div key={d.hour} className="flex-1 flex flex-col items-center gap-1">
                    <div className="text-xs font-bold" style={{ color: '#00B4FF' }}>{d.alerts}</div>
                    <div className="w-full rounded-t-lg transition-all"
                      style={{
                        height: `${(d.alerts / maxHourly) * 100}px`,
                        background: `linear-gradient(180deg, #00B4FF, rgba(0,180,255,0.3))`,
                        minHeight: 4,
                      }} />
          ...
          <div className="card-gastro">
            <h3 className="font-bold text-gastro-text mb-4 flex items-center gap-2">
              <TrendingUp size={15} style={{ color: '#0066FF' }} /> Performance de mozos — Respuesta a alertas
            </h3>
          ...
          <div className="rounded-2xl p-5"
            style={{ background: 'linear-gradient(135deg, rgba(0,180,255,0.06), rgba(0,102,255,0.04))', border: '1px solid rgba(0,180,255,0.15)' }}>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {[
                { label: 'Detección de vasos/platos', accuracy: 96, color: '#3399FF' },
