    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Utilidad neta', value: `$${(profit / 1000).toFixed(0)}K`, change: '+18.7%', positive: true, color: '#0066FF', icon: DollarSign },
          { label: 'Margen neto', value: `${margin}%`, change: '+2.1pp', positive: true, color: '#3399FF', icon: PieChart },
        ].map(kpi => {
          const Icon = kpi.icon
          return (
        ...
        <div className="lg:col-span-2 card-gastro">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={FINANCE_DATA.cashFlow} margin={{ top: 5, right: 5, bottom: 0, left: 0 }} barGap={4}>
              ...
              <Bar dataKey="income" fill="#0066FF" radius={[4, 4, 0, 0]} name="income" />
              <Bar dataKey="expenses" fill="#1A2540" radius={[4, 4, 0, 0]} name="expenses" />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="card-gastro">
          ...
          <div className="space-y-3">
            {FINANCE_DATA.expenses.map((exp, i) => {
              const colors = ['#0066FF', '#3399FF', '#00B4FF', '#f59e0b', '#10b981', '#8899BB']
              return (
