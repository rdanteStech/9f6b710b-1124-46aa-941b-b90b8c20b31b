...
const LEARNING_PATHS: LearningPath[] = [
  ...
  { id: 'lp2', name: 'Maître & Sommelier', description: 'Excelencia en sala y vinos', color: '#2563EB', icon: Wine, courses: 4, duration: '22h', level: 'intermediate', enrolled: 198 },
  { id: 'lp3', name: 'Gerente de Restaurante', description: 'Gestión integral del negocio', color: '#3B82F6', icon: BarChart3, courses: 5, duration: '18h', level: 'intermediate', enrolled: 287 },
  { id: 'lp4', name: 'Pastelero Profesional', description: 'Arte y técnica en pastelería', color: '#60A5FA', icon: Sparkles, courses: 5, duration: '32h', level: 'advanced', enrolled: 156 },
]
...
const CAT_CFG: Record<CourseCategory, { label: string; color: string; emoji: string }> = {
  ...
  servicio:   { label: 'Servicio',        color: '#3B82F6', emoji: '🍽️' },
  gestion:    { label: 'Gestión',         color: '#2563EB', emoji: '📊' },
  bar:        { label: 'Bar & Bebidas',   color: '#60A5FA', emoji: '🍸' },
  pasteleria: { label: 'Pastelería',      color: '#60A5FA', emoji: '🧁' },
...
const STATUS_CFG: Record<CourseStatus, { label: string; color: string; bg: string }> = {
  ...
  enrolled:  { label: 'En curso',    color: '#2563EB', bg: 'rgba(37,99,235,0.12)' },
...
function CourseCard({ course, onEnroll }: { course: Course; onEnroll: (id: string) => void }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden transition-all duration-200 group"
      style={{
        background: '#0F1628',
        border: `1px solid ${course.status === 'enrolled' ? 'rgba(37,99,235,0.3)' : course.status === 'completed' ? 'rgba(16,185,129,0.25)' : '#1A2540'}`,
      ...
      <div className="relative overflow-hidden" style={{ aspectRatio: '16/9' }}>
        ...
        {course.status === 'enrolled' && course.progress !== undefined && (
          <div className="absolute bottom-0 left-0 right-0 h-1" style={{ background: 'rgba(255,255,255,0.1)' }}>
            <div className="h-full transition-all" style={{ width: `${course.progress}%`, background: 'linear-gradient(90deg, #2563EB, #60A5FA)' }} />
          </div>
        )}
      ...
      <div className="p-4">
        ...
        {course.status === 'enrolled' && course.progress !== undefined && (
          <div className="mb-3">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-xs font-bold" style={{ color: '#2563EB' }}>{course.progress}%</span>
            </div>
            <div className="h-1.5 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.06)' }}>
              <div className="h-full rounded-full" style={{ width: `${course.progress}%`, background: 'linear-gradient(90deg, #2563EB, #60A5FA)' }} />
            </div>
          </div>
        )}
        ...
        {course.status === 'locked' && (
          <button className="w-full py-2.5 rounded-xl text-xs font-bold"
            style={{ background: 'rgba(37,99,235,0.08)', border: '1px solid rgba(37,99,235,0.15)', color: '#6666aa' }}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl p-6 relative overflow-hidden"
        style={{
          background: 'linear-gradient(135deg, rgba(245,158,11,0.08) 0%, rgba(37,99,235,0.06) 50%, rgba(59,130,246,0.04) 100%)',
        ...
        <div className="relative z-10 flex items-center gap-4 flex-wrap">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0"
            style={{ background: 'linear-gradient(135deg, #f59e0b, #60A5FA)', boxShadow: '0 0 30px rgba(245,158,11,0.4)' }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: 'Cursos en curso', value: enrolledCourses.length, color: '#2563EB', icon: BookOpen },
          { label: 'Cursos completados', value: completedCourses.length, color: '#10b981', icon: CheckCircle },
          { label: 'Horas aprendidas', value: `${totalHoursLearned}h`, color: '#3B82F6', icon: Clock },
        ...
        <div className="space-y-6">
          ...
          {enrolledCourses.length > 0 && (
            <div>
              <h3 className="font-bold text-gastro-text mb-4 flex items-center gap-2">
                <Play size={16} style={{ color: '#2563EB' }} /> En curso ({enrolledCourses.length})
              </h3>
          ...
          {enrolledCourses.length === 0 && completedCourses.length === 0 && (
            <div className="card-gastro text-center py-16">
              ...
              <p className="text-gastro-subtle text-sm mb-4">Explora el catálogo y comienza tu formación profesional</p>
          ...
          {(enrolledCourses.length > 0 || completedCourses.length > 0) && (
            <div className="rounded-2xl p-5"
              style={{ background: 'linear-gradient(135deg, rgba(245,158,11,0.08), rgba(96,165,250,0.05))', border: '1px solid rgba(245,158,11,0.2)' }}>
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center"
                  style={{ background: 'linear-gradient(135deg, #f59e0b, #60A5FA)' }}>
